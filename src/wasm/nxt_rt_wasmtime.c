...
static void
nxt_wasmtime_err_msg(wasmtime_error_t* error,
                     wasm_trap_t* trap,
                     const char* fmt,
                     ...)
{
...
static wasm_trap_t*
nxt_wasm_get_init_mem_size(void* env,
...
static wasm_trap_t*
nxt_wasm_response_end(void* env,
                      wasmtime_caller_t* caller,
                      const wasmtime_val_t* args,
                      size_t nargs,
                      wasmtime_val_t* results,
...
static wasm_trap_t*
nxt_wasm_send_response(void* env,
                       wasmtime_caller_t* caller,
                       const wasmtime_val_t* args,
                       size_t nargs,
                       wasmtime_val_t* results,
...
static wasm_trap_t*
nxt_wasm_send_headers(void* env,
                      wasmtime_caller_t* caller,
                      const wasmtime_val_t* args,
                      size_t nargs,
                      wasmtime_val_t* results,
...
static wasm_trap_t*
nxt_wasm_set_resp_status(void* env,
                         wasmtime_caller_t* caller,
                         const wasmtime_val_t* args,
                         size_t nargs,
                         wasmtime_val_t* results,
...
nxt_wasmtime_execute_hook(const nxt_wasm_ctx_t* ctx, nxt_wasm_fh_t hook)
{
  ...
  if (error != NULL || trap != NULL) {
    nxt_wasmtime_err_msg(
      error, trap, "failed to call hook function [%s]", name);
  }
}
...
nxt_wasmtime_set_function_imports(nxt_wasm_ctx_t* ctx)
{
  ...
  for (imf = import_functions; imf->func_name != NULL; imf++) {
    ...
    wasmtime_linker_define_func(rt_ctx->linker,
                                "env",
                                3,
                                imf->func_name,
...
nxt_wasmtime_get_function_exports(nxt_wasm_ctx_t* ctx)
{
  ...
  for (i = 0; i < NXT_WASM_FH_NR; i++) {
    ...
    ok = wasmtime_linker_get(rt_ctx->linker,
                             rt_ctx->ctx,
                             "",
                             0,
                             ctx->fh[i].func_name,
                             strlen(ctx->fh[i].func_name),
                             &item);
    if (!ok) {
      nxt_wasmtime_err_msg(NULL,
                           NULL,
...
nxt_wasmtime_init_memory(nxt_wasm_ctx_t* ctx)
{
  ...
  if (error != NULL || trap != NULL) {
    nxt_wasmtime_err_msg(
      error, trap, "failed to call function [->wasm_malloc_handler]");
    return -1;
  }

  ok = wasmtime_linker_get(
    rt_ctx->linker, rt_ctx->ctx, "", 0, "memory", strlen("memory"), &item);
  if (!ok) {
...
nxt_wasmtime_init(nxt_wasm_ctx_t* ctx)
{
  ...
  if (!fp) {
    nxt_wasmtime_err_msg(
      NULL, NULL, "error opening file (%s)", ctx->module_path);
    return -1;
  }
  ...
  error = wasmtime_module_new(
    rt_ctx->engine, (uint8_t*)wasm.data, wasm.size, &rt_ctx->module);
  if (!rt_ctx->module) {
  ...
  error =
    wasmtime_linker_module(rt_ctx->linker, rt_ctx->ctx, "", 0, rt_ctx->module);
  if (error != NULL) {
