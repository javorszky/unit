...
static nxt_buf_t*
nxt_event_conn_job_sendfile_completion(nxt_task_t* task,
                                       nxt_conn_t* c,
                                       nxt_buf_t* b);
...
fast:
  ...
  nxt_work_set(
    &jbs->job.work, nxt_event_conn_job_sendfile_handler, jbs->job.task, jbs, c);
                                       ...
                                       nxt_buf_t* b)
{
  while (b != NULL) {
    ...
    nxt_work_queue_add(
      c->write_work_queue, b->completion_handler, task, b, b->parent);
