...
nxt_external_start(nxt_task_t *task, nxt_process_data_t *data) {
    ...
    if (nxt_slow_path(
            proto_port == NULL || my_port == NULL || router_port == NULL)) {
        return NXT_ERROR;
    }
    ...
    p = nxt_sprintf(buf, end,
        ...
        "%d,%d;"
        "%d,%z,%uD,%Z",
        NXT_VERSION, my_port->process->stream, proto_port->pid, proto_port->id,
        proto_port->pair[1], router_port->pid, router_port->id,
        router_port->pair[1], my_port->pid, my_port->id, my_port->pair[0],
        my_port->pair[1], conf->shared_port_fd, conf->shared_queue_fd, 2,
