...
nxt_sockaddr_t*
nxt_sockaddr_create(nxt_mp_t* mp,
...
nxt_sockaddr_text(nxt_sockaddr_t* sa)
{
  ...
  switch (sa->u.sockaddr.sa_family) {
    ...
    case AF_INET:
      ...
      p = nxt_sprintf(
        start, end, "%ud.%ud.%ud.%ud", octet[0], octet[1], octet[2], octet[3]);
...
nxt_inet6_ntop(u_char* addr, u_char* buf, u_char* end)
{
  ...
  if (ipv6_bytes == 12) {
    p = nxt_sprintf(
      p, end, "%ud.%ud.%ud.%ud", addr[12], addr[13], addr[14], addr[15]);
  }
...
nxt_sockaddr_parse(nxt_mp_t* mp, nxt_str_t* addr)
{
  ...
  if (sa != NULL && sa->u.sockaddr.sa_family != AF_UNIX &&
    ...
    nxt_thread_log_error(
      NXT_LOG_ERR, "The address \"%V\" must specify a port.", addr);
    return NULL;
  }
...
nxt_sockaddr_unix_parse(nxt_mp_t* mp, nxt_str_t* addr)
{
  ...
  nxt_thread_log_error(
    NXT_LOG_ERR, "unix domain socket \"%V\" is not supported", addr);
...
nxt_sockaddr_inet6_parse(nxt_mp_t* mp, nxt_str_t* addr)
{
  ...
  sa =
    nxt_sockaddr_alloc(mp, sizeof(struct sockaddr_in6), NXT_INET6_ADDR_STR_LEN);
  ...
  nxt_thread_log_error(
    NXT_LOG_ERR, "IPv6 socket \"%V\" is not supported", addr);
  return NULL;
