...
static u_char*
nxt_conf_json_parse_value(nxt_mp_t* mp,
                          nxt_conf_value_t* value,
                          u_char* start,
                          u_char* end,
                          nxt_conf_json_error_t* error);
static u_char*
nxt_conf_json_parse_object(nxt_mp_t* mp,
                           nxt_conf_value_t* value,
                           u_char* start,
                           u_char* end,
                           nxt_conf_json_error_t* error);
static nxt_int_t
nxt_conf_object_hash_add(nxt_mp_t* mp,
                         nxt_lvlhsh_t* lvlhsh,
...
static u_char*
nxt_conf_json_parse_array(nxt_mp_t* mp,
                          nxt_conf_value_t* value,
                          u_char* start,
                          u_char* end,
                          nxt_conf_json_error_t* error);
static u_char*
nxt_conf_json_parse_string(nxt_mp_t* mp,
                           nxt_conf_value_t* value,
                           u_char* start,
                           u_char* end,
                           nxt_conf_json_error_t* error);
static u_char*
nxt_conf_json_parse_number(nxt_mp_t* mp,
                           nxt_conf_value_t* value,
                           u_char* start,
                           u_char* end,
                           nxt_conf_json_error_t* error);
static void
nxt_conf_json_parse_error(nxt_conf_json_error_t* error,
                          u_char* pos,
                          const char* detail);

static nxt_int_t
nxt_conf_copy_value(nxt_mp_t* mp,
                    const nxt_conf_op_t* op,
                    nxt_conf_value_t* dst,
                    const nxt_conf_value_t* src);
static nxt_int_t
nxt_conf_copy_array(nxt_mp_t* mp,
                    const nxt_conf_op_t* op,
                    nxt_conf_value_t* dst,
                    const nxt_conf_value_t* src);
static nxt_int_t
nxt_conf_copy_object(nxt_mp_t* mp,
                     const nxt_conf_op_t* op,
                     nxt_conf_value_t* dst,
                     const nxt_conf_value_t* src);
...
static u_char*
nxt_conf_json_print_array(u_char* p,
...
nxt_conf_json_object_length(const nxt_conf_value_t* value,
                            nxt_conf_json_pretty_t* pretty);
static u_char*
nxt_conf_json_print_object(u_char* p,
                           const nxt_conf_value_t* value,
                           nxt_conf_json_pretty_t* pretty);
...
nxt_str_t*
nxt_conf_get_string_dup(const nxt_conf_value_t* value,
                        nxt_mp_t* mp,
                        nxt_str_t* str)
{
  nxt_str_t s;
...
nxt_int_t
nxt_conf_set_string_dup(nxt_conf_value_t* value,
                        nxt_mp_t* mp,
                        const nxt_str_t* str)
{
  nxt_str_t tmp, *ptr;
...
void
nxt_conf_set_member(nxt_conf_value_t* object,
...
nxt_int_t
nxt_conf_set_member_dup(nxt_conf_value_t* object,
                        nxt_mp_t* mp,
                        const nxt_str_t* name,
                        const nxt_conf_value_t* value,
                        uint32_t index)
...
void
nxt_conf_set_member_string(nxt_conf_value_t* object,
                           const nxt_str_t* name,
...
nxt_int_t
nxt_conf_set_member_string_dup(nxt_conf_value_t* object,
                               nxt_mp_t* mp,
                               const nxt_str_t* name,
                               const nxt_str_t* value,
                               uint32_t index)
...
void
nxt_conf_set_member_integer(nxt_conf_value_t* object,
                            const nxt_str_t* name,
                            int64_t value,
                            uint32_t index)
{
...
void
nxt_conf_set_member_null(nxt_conf_value_t* object,
                         const nxt_str_t* name,
                         uint32_t index)
{
  nxt_conf_object_member_t* member;
...
void
nxt_conf_set_element(nxt_conf_value_t* array,
                     nxt_uint_t index,
                     const nxt_conf_value_t* value)
{
...
nxt_int_t
nxt_conf_set_element_string_dup(nxt_conf_value_t* array,
                                nxt_mp_t* mp,
                                nxt_uint_t index,
                                const nxt_str_t* value)
{
...
nxt_int_t
nxt_conf_map_object(nxt_mp_t* mp,
                    const nxt_conf_value_t* value,
...
nxt_conf_value_t*
nxt_conf_next_object_member(const nxt_conf_value_t* value,
                            nxt_str_t* name,
                            uint32_t* next)
{
  uint32_t n;
...
nxt_conf_op_ret_t
nxt_conf_op_compile(nxt_mp_t* mp,
                    nxt_conf_op_t** ops,
                    ...
                    const nxt_conf_value_t* src)
{
  ...
  switch (src->type) {
    ...
    case NXT_CONF_VALUE_STRING:
      ...
      nxt_memcpy(
        dst->u.string.start, src->u.string.start, src->u.string.length);
  ...
  do {
    ...
    while (s != index) {
      rc = nxt_conf_copy_value(
        mp, pass_op, &dst->u.array->elements[d], &src->u.array->elements[s]);
      if (nxt_slow_path(rc != NXT_OK)) {
        return NXT_ERROR;
      }
...
nxt_conf_value_t*
nxt_conf_json_parse(nxt_mp_t* mp,
                    u_char* start,
                    u_char* end,
                    nxt_conf_json_error_t* error)
{
                          ...
                          nxt_conf_json_error_t* error)
{
  ...
  array =
    nxt_mp_get(mp, sizeof(nxt_conf_array_t) + count * sizeof(nxt_conf_value_t));
                           ...
                           nxt_conf_json_error_t* error)
{
  ...
  do {
    ...
    for (;;) {
      for (i = 0; i < 4; i++) {
        utf =
          (utf << 4) | (p[i] >= 'A' ? 10 + ((p[i] & ~0x20) - 'A') : p[i] - '0');
      }
                           ...
                           nxt_conf_json_error_t* error)
{
  ...
  if (nxt_slow_path(end == NULL || *end != '\0')) {
    nxt_thread_log_alert("strtod(\"%s\", %s) failed %E",
                         value->u.number,
...
void
nxt_conf_json_position(u_char* start,
                       const u_char* pos,
                       nxt_uint_t* line,
                       nxt_uint_t* column)
{
