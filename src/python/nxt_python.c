...
static nxt_int_t
nxt_python_set_target(nxt_task_t* task,
                      nxt_python_target_t* target,
                      nxt_conf_value_t* conf);
nxt_inline nxt_int_t
nxt_python_set_prefix(nxt_task_t* task,
                      nxt_python_target_t* target,
                      ...
                      nxt_conf_value_t* conf)
{
  ...
  if (nxt_slow_path(obj == NULL)) {
    nxt_alert(task,
              "Python failed to get \"%s\" from module \"%s\"",
              callable,
              module_name);
    goto fail;
  }
  ...
  if (is_factory) {
    ...
    nxt_alert(task,
              "\"%s\" in module \"%s\" is not a callable object",
              callable,
              module_name);
    goto fail;
  }
...
nxt_python_init_threads(nxt_python_app_conf_t* c)
{
  ...
  if (c->thread_stack_size > 0) {
    ...
    if (nxt_slow_path(res != 0)) {
      nxt_unit_alert(
        NULL, "thread attr init failed: %s (%d)", strerror(res), res);
    ...
    if (nxt_slow_path(res != 0)) {
      nxt_unit_alert(
        NULL, "thread attr set stack size failed: %s (%d)", strerror(res), res);
...
nxt_python_ready_handler(nxt_unit_ctx_t* ctx)
{
  ...
  for (i = 0; i < c->threads - 1; i++) {
    ...
    res = pthread_create(
      &ti->thread, nxt_py_thread_attr, nxt_python_thread_func, ti);
...
nxt_python_thread_func(void* data)
{
  ...
  nxt_unit_debug(
    ti->ctx, "worker thread #%d start", (int)(ti - nxt_py_threads + 1));
