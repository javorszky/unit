...
static PyObject*
nxt_py_asgi_lifespan_send_startup(nxt_py_asgi_lifespan_t* lifespan,
                                  int v,
                                  PyObject* dict);
static PyObject*
nxt_py_asgi_lifespan_send_(nxt_py_asgi_lifespan_t* lifespan,
                           int v,
                           int* sent,
                           PyObject** future);
static PyObject*
nxt_py_asgi_lifespan_send_shutdown(nxt_py_asgi_lifespan_t* lifespan,
                                   int v,
                                   PyObject* dict);
static PyObject*
                                    ...
                                    nxt_python_target_t* target)
{
  ...
  if (!target->asgi_legacy) {
    ...
    if (nxt_slow_path(res == NULL)) {
      nxt_unit_log(
        NULL, NXT_UNIT_LOG_INFO, "ASGI Lifespan processing exception");
      nxt_python_print_exception();
  ...
  res = PyObject_CallMethodObjArgs(
    py_task, nxt_py_add_done_callback_str, done, NULL);
  if (nxt_slow_path(res == NULL)) {
  ...
  res = PyObject_CallFunctionObjArgs(
    ctx_data->loop_run_until_complete, lifespan->startup_future, NULL);
  if (nxt_slow_path(res == NULL)) {
...
nxt_py_asgi_lifespan_target_shutdown(nxt_py_asgi_lifespan_t* lifespan)
{
  ...
  res = PyObject_CallFunctionObjArgs(
    ctx_data->loop_run_until_complete, lifespan->shutdown_future, NULL);
  if (nxt_slow_path(res == NULL)) {
    nxt_unit_alert(NULL, "Python failed to call loop.run_until_complete");
    nxt_python_print_exception();
    return NXT_UNIT_ERROR;
                                  ...
                                  PyObject* message)
{
  ...
  return nxt_py_asgi_lifespan_send_(
    lifespan, v, &lifespan->startup_sent, &lifespan->startup_future);
}
...
{
  return nxt_py_asgi_lifespan_send_(
    lifespan, v, &lifespan->shutdown_sent, &lifespan->shutdown_future);
}
