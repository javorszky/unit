...
nxt_py_asgi_websocket_send(PyObject* self, PyObject* dict)
{
  ...
  nxt_unit_req_debug(
    ws->req, "asgi_websocket_send type is '%.*s'", (int)type_len, type_str);
...
nxt_py_asgi_websocket_accept(nxt_py_asgi_websocket_t* ws, PyObject* dict)
{
  ...
  if (headers != NULL) {
    res =
      nxt_py_asgi_enum_headers(headers, nxt_py_asgi_calc_size, &calc_size_ctx);
  ...
  rc = nxt_unit_response_init(
    ws->req, 101, calc_size_ctx.fields_count, calc_size_ctx.fields_size);
  ...
  if (headers != NULL) {
    res =
      nxt_py_asgi_enum_headers(headers, nxt_py_asgi_add_field, &add_field_ctx);
    if (nxt_slow_path(res == NULL)) {
      return NULL;
    }
  }
...
nxt_py_asgi_websocket_close(nxt_py_asgi_websocket_t* ws, PyObject* dict)
{
  ...
  if (nxt_unit_response_is_websocket(ws->req)) {
    ...
    rc = nxt_unit_websocket_send(
      ws->req, NXT_WEBSOCKET_OP_CLOSE, 1, &status_code, 2);
    if (nxt_slow_path(rc != NXT_UNIT_OK)) {
...
bad_state:
  ...
  exc = PyObject_CallFunctionObjArgs(
    PyExc_RuntimeError, nxt_py_bad_state_str, NULL);
...
too_big:
  ...
  exc = PyObject_CallFunctionObjArgs(
    PyExc_RuntimeError, nxt_py_message_too_big_str, NULL);
  if (nxt_slow_path(exc == NULL)) {
    nxt_unit_req_alert(ws->req, "RuntimeError create failed");
    nxt_python_print_exception();
                              ...
                              nxt_unit_websocket_frame_t* frame)
{
  ...
  switch (opcode) {
    ...
    case NXT_WEBSOCKET_OP_CLOSE:
      ...
      if (nxt_slow_path(data == NULL)) {
        nxt_unit_req_alert(
          ws->req, "Failed to create Long from code %d.", (int)code);
        nxt_python_print_exception();

        return PyErr_Format(
          PyExc_RuntimeError, "Failed to create Long from code %d.", (int)code);
      }
