...
static PyObject*
nxt_py_asgi_create_ip_address(nxt_unit_sptr_t* sptr,
                              uint8_t len,
                              uint16_t port);
static PyObject*
...
static void
nxt_py_asgi_remove_port(nxt_unit_t* lib,
                        nxt_unit_ctx_t* ctx,
...
static PyMethodDef nxt_py_port_read_method = { "unit_port_read",
                                               nxt_py_asgi_port_read,
                                               METH_VARARGS,
                                               "" };
...
nxt_python_asgi_get_event_loop(PyObject* asyncio, const char* event_loop_func)
{
  ...
  if (nxt_slow_path(event_loop == NULL)) {
    nxt_unit_alert(
      NULL, "Python failed to get '%s' from module 'asyncio'", event_loop_func);
    return NULL;
  }

  if (nxt_slow_path(PyCallable_Check(event_loop) == 0)) {
    nxt_unit_alert(
      NULL, "'asyncio.%s' is not a callable object", event_loop_func);
    return NULL;
  }
  ...
  if (nxt_slow_path(loop == NULL)) {
    if (strcmp(event_loop_func, "get_running_loop") != 0) {
      nxt_unit_alert(
        NULL, "Python failed to call 'asyncio.%s'", event_loop_func);
    }
  ...
  for (i = 0; i < nxt_nitems(handlers); i++) {
    ...
    *handlers[i].handler = obj;
    ...
    if (nxt_slow_path(PyCallable_Check(obj) == 0)) {
      nxt_unit_alert(
        NULL, "'loop.%s' is not a callable object", handlers[i].key);
      goto fail;
    }
  }
...
nxt_python_asgi_run(nxt_unit_ctx_t* ctx)
{
  ...
  res = PyObject_CallFunctionObjArgs(
    ctx_data->loop_run_until_complete, ctx_data->quit_future, NULL);
...
nxt_py_asgi_request_handler(nxt_unit_request_info_t* req)
{
  ...
  if (!target->asgi_legacy) {
    ...
    res = PyObject_CallFunctionObjArgs(
      target->application, scope, receive, send, NULL);
  ...
  res =
    PyObject_CallMethodObjArgs(task, nxt_py_add_done_callback_str, done, NULL);
  if (nxt_slow_path(res == NULL)) {
...
#define SET_ITEM(dict, key, value)                                             \
  ...
  v =
    PyString_FromStringAndSize(nxt_unit_sptr_get(&r->method), r->method_length);
  ...
  v = PyUnicode_DecodeUTF8(
    nxt_unit_sptr_get(&r->path), r->path_length, "replace");
  if (nxt_slow_path(v == NULL)) {
...
nxt_py_asgi_add_reader(nxt_unit_ctx_t* ctx, nxt_unit_port_t* port)
{
  ...
  res = PyObject_CallFunctionObjArgs(
    ctx_data->loop_add_reader, fd, nxt_py_port_read, py_ctx, py_port, NULL);
  if (nxt_slow_path(res == NULL)) {
...
nxt_py_asgi_port_read(PyObject* self, PyObject* args)
{
  ...
  if (nxt_slow_path(rc == NXT_UNIT_ERROR)) {
    return PyErr_Format(
      PyExc_RuntimeError, "error processing port %d message", port->id.id);
  }
  ...
  if (rc == NXT_UNIT_OK) {
    ...
    res = PyObject_CallFunctionObjArgs(
      ctx_data->loop_call_soon, nxt_py_port_read, arg0, arg1, NULL);
    if (nxt_slow_path(res == NULL)) {
...
PyObject*
nxt_py_asgi_enum_headers(PyObject* headers,
                         ...
                         void* data)
{
  ...
  for (i = 0; /* void */; i++) {
    ...
    if (nxt_slow_path(h_iter == NULL)) {
      ...
      return PyErr_Format(
        PyExc_TypeError, "'headers' item #%d is not an iterable", i);
    }
    ...
    if (nxt_slow_path(name == NULL || !PyBytes_Check(name))) {
      ...
      return PyErr_Format(
        PyExc_TypeError, "'headers' item #%d 'name' is not a byte string", i);
    }
    ...
    if (nxt_slow_path(val == NULL || !PyBytes_Check(val))) {
      ...
      return PyErr_Format(
        PyExc_TypeError, "'headers' item #%d 'value' is not a byte string", i);
    }
                            ...
                            PyObject* result)
{
  ...
  res = PyObject_CallFunctionObjArgs(
    ctx_data->loop_call_soon, set_result, result, NULL);
  if (nxt_slow_path(res == NULL)) {
...
PyObject*
nxt_py_asgi_new_scope(nxt_unit_request_info_t* req,
                      PyObject* type,
