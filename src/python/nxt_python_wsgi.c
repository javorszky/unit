...
static int
nxt_python_add_sptr(nxt_python_ctx_t* pctx,
                    PyObject* name,
                    nxt_unit_sptr_t* sptr,
                    uint32_t size);
static int
nxt_python_add_char(nxt_python_ctx_t* pctx,
                    PyObject* name,
                    char* src,
                    uint32_t size);
static int
nxt_python_add_py_string(nxt_python_ctx_t* pctx,
                         PyObject* name,
                         PyObject* value);
static int
nxt_python_add_field(nxt_python_ctx_t* pctx,
...
static int
nxt_python_response_add_field(nxt_python_ctx_t* pctx,
                              PyObject* name,
                              PyObject* value,
                              int i);
static int
...
nxt_python_create_environ(nxt_python_app_conf_t* c)
{
  ...
  if (nxt_slow_path(PyDict_SetItemString(environ,
                                         "wsgi.multithread",
...
#define RC(S)                                                                  \
  ...
  RC(nxt_python_add_sptr(
    pctx, nxt_py_request_method_str, &r->method, r->method_length));
  RC(nxt_python_add_sptr(
    pctx, nxt_py_request_uri_str, &r->target, r->target_length));
  RC(nxt_python_add_sptr(
    pctx, nxt_py_query_string_str, &r->query, r->query_length));
  ...
  if (prefix.length > 0 &&
    ...
    RC(nxt_python_add_py_string(
      pctx, nxt_py_script_name_str, app_target->py_prefix));
  ...
  RC(nxt_python_add_sptr(
    pctx, nxt_py_remote_addr_str, &r->remote, r->remote_length));
  RC(nxt_python_add_sptr(
    pctx, nxt_py_server_addr_str, &r->local_addr, r->local_addr_length));
  ...
  RC(nxt_python_add_sptr(
    pctx, nxt_py_server_protocol_str, &r->version, r->version_length));

  RC(nxt_python_add_sptr(
    pctx, nxt_py_server_name_str, &r->server_name, r->server_name_length));
  ...
  if (r->content_length_field != NXT_UNIT_NONE_FIELD) {
    ...
    RC(nxt_python_add_sptr(
      pctx, nxt_py_content_length_str, &f->value, f->value_length));
  }
  ...
  if (r->content_type_field != NXT_UNIT_NONE_FIELD) {
    ...
    RC(nxt_python_add_sptr(
      pctx, nxt_py_content_type_str, &f->value, f->value_length));
  }

#undef RC

  if (nxt_slow_path(PyDict_SetItem(pctx->environ,
                                   nxt_py_wsgi_input_str,
                                   (PyObject*)pctx) != 0)) {
...
nxt_py_start_resp(PyObject* self, PyObject* args)
{
  ...
  for (i = 0; i < fields_count; i++) {
    ...
    if (PyBytes_Check(string)) {
      ...
      return PyErr_Format(
        PyExc_TypeError, "header #%d name is not a string", (int)i);
    }
    ...
    if (PyBytes_Check(string)) {
      ...
      return PyErr_Format(
        PyExc_TypeError, "header #%d value is not a string", (int)i);
    }
  }
  ...
  for (i = 0; i < fields_count; i++) {
    ...
    rc = nxt_python_response_add_field(
      pctx, PyTuple_GET_ITEM(tuple, 0), PyTuple_GET_ITEM(tuple, 1), i);
    if (nxt_slow_path(rc != NXT_UNIT_OK)) {
      return PyErr_Format(
        PyExc_RuntimeError, "failed to add header #%d", (int)i);
    }
  }
                              ...
                              int i)
{
  ...
  rc = nxt_unit_response_add_field(
    pctx->req, name_str, name_length, value_str, value_length);
  if (nxt_slow_path(rc != NXT_UNIT_OK)) {
    goto fail;
  }
...
nxt_py_write(PyObject* self, PyObject* str)
{
  ...
  if (nxt_fast_path(!PyBytes_Check(str))) {
    return PyErr_Format(
      PyExc_TypeError, "the argument is not a %s", NXT_PYTHON_BYTES_TYPE);
  }
...
nxt_python_write(nxt_python_ctx_t* pctx, PyObject* bytes)
{
  ...
  if (nxt_slow_path(str_length > pctx->content_length - pctx->bytes_sent)) {
    nxt_unit_req_error(
      pctx->req, "content length %" PRIu64 " exceeded", pctx->content_length);
