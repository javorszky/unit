...
static void
nxt_gnutls_conn_init(nxt_thread_t* thr,
...
static ssize_t
nxt_gnutls_vec_push(gnutls_transport_ptr_t data,
                    const giovec_t* iov,
                    int iovcnt);
#endif
static void
...
static ssize_t
nxt_gnutls_conn_io_write_chunk(nxt_thread_t* thr,
                               nxt_event_conn_t* c,
                               nxt_buf_t* b,
                               size_t limit);
static ssize_t
...
static nxt_int_t
nxt_gnutls_conn_test_error(nxt_thread_t* thr,
                           nxt_event_conn_t* c,
                           ssize_t err,
                           nxt_work_handler_t handler);
static void nxt_cdecl
nxt_gnutls_conn_log_error(nxt_event_conn_t* c,
                          ssize_t err,
                          const char* fmt,
                          ...);
static nxt_uint_t
...
static void nxt_cdecl
nxt_gnutls_log_error(nxt_uint_t level,
                     nxt_log_t* log,
                     int err,
                     const char* fmt,
                     ...);
...
nxt_gnutls_start(void)
{
  ...
  if (ret != GNUTLS_E_SUCCESS) {
    nxt_gnutls_log_error(
      NXT_LOG_ALERT, nxt_thread_log(), ret, "gnutls_global_init() failed");
    return NXT_ERROR;
  }

  nxt_thread_log_error(
    NXT_LOG_INFO, "GnuTLS version: %s", gnutls_check_version(NULL));
...
nxt_gnutls_server_init(nxt_ssltls_conf_t* conf)
{
  ...
  if (ret != GNUTLS_E_SUCCESS) {
    nxt_gnutls_log_error(NXT_LOG_ALERT,
                         thr->log,
                         ret,
  ...
  ret = gnutls_certificate_set_x509_key_file(
    ctx->certificate, certificate, key, GNUTLS_X509_FMT_PEM);
  if (ret != GNUTLS_E_SUCCESS) {
    nxt_gnutls_log_error(
      NXT_LOG_ALERT,
      thr->log,
...
nxt_gnutls_set_ciphers(nxt_ssltls_conf_t* conf)
{
  ...
  switch (ret) {
    ...
    default:
      nxt_gnutls_log_error(
        NXT_LOG_ALERT, nxt_thread_log(), ret, "gnutls_priority_init() failed");
      return NXT_ERROR;
  }
}
                     ...
                     nxt_event_conn_t* c)
{
  ...
  if (ret != GNUTLS_E_SUCCESS) {
    nxt_gnutls_log_error(
      NXT_LOG_ALERT, c->socket.log, ret, "gnutls_init() failed");
    goto fail;
  }
  ...
  if (ret != GNUTLS_E_SUCCESS) {
    nxt_gnutls_log_error(
      NXT_LOG_ALERT, c->socket.log, ret, "gnutls_priority_set() failed");
    goto fail;
  }
  ...
  if (ret != GNUTLS_E_SUCCESS) {
    nxt_gnutls_log_error(
      NXT_LOG_ALERT, c->socket.log, ret, "gnutls_credentials_set() failed");
    goto fail;
  }
...
fail:
  ...
  nxt_event_conn_io_handle(
    thr, c->read_work_queue, c->read_state->error_handler, c, c->socket.data);
}
...
nxt_gnutls_conn_handshake(nxt_thread_t* thr, void* obj, void* data)
{
  ...
  if (ret == NXT_ERROR) {
    ...
    nxt_event_conn_io_handle(
      thr, c->read_work_queue, c->read_state->error_handler, c, data);
...
nxt_gnutls_conn_io_read(nxt_thread_t* thr, void* obj, void* data)
{
  ...
  if (b != NULL) {
    ...
    n = gnutls_record_recv(
      ssltls->session, b->mem.free, b->mem.end - b->mem.free);

    nxt_log_debug(thr->log,
                  "gnutls_record_recv(%d, %p, %uz): %z",
                  c->socket.fd,
                  b->mem.free,
...
nxt_gnutls_conn_io_send(nxt_event_conn_t* c, void* buf, size_t size)
{
  ...
  if (nxt_slow_path(ret == NXT_ERROR)) {
    nxt_gnutls_conn_log_error(
      c, n, "gnutls_record_send(%d, %p, %uz): failed", c->socket.fd, buf, size);
  }
...
nxt_gnutls_conn_io_shutdown(nxt_thread_t* thr, void* obj, void* data)
{
  ...
  nxt_log_debug(
    c->socket.log, "gnutls_bye(%d, %d): %d", c->socket.fd, how, err);
                     ...
                     ...)
{
  ...
  nxt_log_error(
    level, log, "%*s (%d: %s)", p - msg, msg, err, gnutls_strerror(err));
}
