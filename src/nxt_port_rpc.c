...
static void
nxt_port_rpc_remove_from_peers(nxt_task_t* task,
                               nxt_port_t* port,
...
uint32_t
nxt_port_rpc_register_handler(nxt_task_t* task,
                              nxt_port_t* port,
                              nxt_port_rpc_handler_t ready_handler,
                              nxt_port_rpc_handler_t error_handler,
                              nxt_pid_t peer,
                              void* data)
{
  ...
  ex = nxt_port_rpc_register_handler_ex(
    task, port, ready_handler, error_handler, 0);
...
void*
nxt_port_rpc_register_handler_ex(nxt_task_t* task,
                                 nxt_port_t* port,
                                 nxt_port_rpc_handler_t ready_handler,
                                 nxt_port_rpc_handler_t error_handler,
                                 size_t ex_size)
...
void
nxt_port_rpc_ex_set_peer(nxt_task_t* task,
                         nxt_port_t* port,
                         void* ex,
                         nxt_pid_t peer)
{
  ...
  switch (ret) {
    ...
    case NXT_OK:
      ...
      nxt_debug(task,
                "rpc: stream #%uD assigned uniq pid %PI (%p)",
                reg->stream,
                reg->peer,
                reg->link.next);
      break;
    ...
    case NXT_DECLINED:
      ...
      nxt_debug(task,
                "rpc: stream #%uD assigned duplicate pid %PI (%p)",
                reg->stream,
                reg->peer,
                reg->link.next);
      break;
...
nxt_port_rpc_remove_peer(nxt_task_t* task, nxt_port_t* port, nxt_pid_t peer)
{
  ...
  while (last == 0) {
    ...
    if (nxt_slow_path(ret != NXT_OK)) {
      nxt_log_error(NXT_LOG_ERR,
                    task->log,
                    "rpc: stream #%uD failed to delete handler",
                    stream);
