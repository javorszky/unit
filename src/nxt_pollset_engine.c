...
static nxt_int_t
nxt_pollset_create(nxt_event_engine_t* engine,
                   nxt_uint_t mchanges,
                   nxt_uint_t mevents);
static void
...
static void
nxt_pollset_change(nxt_event_engine_t* engine,
                   nxt_fd_event_t* ev,
                   nxt_uint_t op,
                   nxt_uint_t events);
static nxt_int_t
                   ...
                   nxt_uint_t events)
{
  ...
  nxt_debug(ev->task,
            "pollset %d change fd:%d op:%ui ev:%04Xi",
            engine->u.pollset.ps,
            ev->fd,
            op,
            events);
...
nxt_pollset_commit_changes(nxt_event_engine_t* engine)
{
  ...
  do {
    ...
    if (change->op == NXT_POLLSET_ADD) {
      ...
      nxt_fd_event_hash_delete(
        &engine->task, &engine->u.pollset.fd_hash, ev->fd, 0);
    }
...
nxt_pollset_change_error(nxt_event_engine_t* engine, nxt_fd_event_t* ev)
{
  ...
  nxt_work_queue_add(
    &engine->fast_work_queue, ev->error_handler, ev->task, ev, ev->data);

  nxt_fd_event_hash_delete(
    &engine->task, &engine->u.pollset.fd_hash, ev->fd, 1);
...
nxt_pollset_remove(nxt_event_engine_t* engine, nxt_fd_t fd)
{
  ...
  nxt_debug(
    &engine->task, "pollset_query(%d, %d): %d", engine->u.pollset.ps, fd, n);
...
nxt_pollset_poll(nxt_event_engine_t* engine, nxt_msec_t timeout)
{
  ...
  nxt_debug(&engine->task,
            "pollset_poll(%d) timeout:%M",
            engine->u.pollset.ps,
            timeout);

  nevents = pollset_poll(engine->u.pollset.ps,
                         engine->u.pollset.events,
                         engine->u.pollset.mevents,
                         timeout);
  ...
  nxt_debug(
    &engine->task, "pollset_poll(%d): %d", engine->u.pollset.ps, nevents);
