...
static uint8_t
nxt_port_enqueue_buf(nxt_task_t* task,
                     nxt_port_msg_t* pm,
                     void* qbuf,
                     nxt_buf_t* b);
static nxt_int_t
nxt_port_msg_chk_insert(nxt_task_t* task,
                        nxt_port_t* port,
...
static nxt_buf_t*
nxt_port_buf_completion(nxt_task_t* task,
                        nxt_work_queue_t* wq,
                        nxt_buf_t* b,
                        size_t sent,
...
static void
nxt_port_read_msg_process(nxt_task_t* task,
                          nxt_port_t* port,
...
nxt_int_t
nxt_port_socket_write2(nxt_task_t* task,
                       nxt_port_t* port,
                       nxt_uint_t type,
                       nxt_fd_t fd,
                       nxt_fd_t fd2,
                       ...
                       nxt_buf_t* b)
{
  ...
  if (port->queue != NULL && type != _NXT_PORT_MSG_READ_QUEUE) {
    ...
    if (fd == -1 && nxt_port_can_enqueue_buf(b)) {
      ...
      nxt_debug(task,
                "port{%d,%d} %d: enqueue %d notify %d, %d",
                (int)port->pid,
                (int)port->id,
                port->socket.fd,
                (int)qmsg_size,
      ...
      nxt_debug(task,
                "port{%d,%d} %d: enqueue 1 notify %d, %d",
                (int)port->pid,
                (int)port->id,
                port->socket.fd,
                notify,
  ...
  next_fragment:
    ...
    if (n > 0) {
      ...
      msg->buf = nxt_port_buf_completion(
        task, wq, msg->buf, plain_size, m == NXT_PORT_METHOD_MMAP);

      if (msg->buf != NULL) {
        nxt_debug(task,
                  "port %d: frag stream #%uD",
                  port->socket.fd,
                  msg->port_msg.stream);
...
fail:
  ...
  nxt_work_queue_add(
    wq, nxt_port_error_handler, task, &port->socket, &port->socket);
  ...
  for (;;) {
    ...
    if (port->from_socket == 0) {
      ...
      if (n == 1 && qmsg[0] == _NXT_PORT_MSG_READ_SOCKET) {
        ...
        nxt_debug(task,
                  "port{%d,%d} %d: dequeue 1 read_socket %d",
                  (int)port->pid,
                  (int)port->id,
                  port->socket.fd,
                  port->from_socket);
      ...
      if ((smsg = port->socket_msg) != NULL && smsg->size != 0) {
        ...
        nxt_debug(task,
                  "port{%d,%d} %d: use suspended message %d",
                  (int)port->pid,
                  (int)port->id,
                  port->socket.fd,
                  (int)n);
    ...
    if (n >= (ssize_t)sizeof(nxt_port_msg_t)) {
      ...
      if (n > (ssize_t)sizeof(nxt_port_msg_t)) {
        nxt_memcpy(b->mem.pos,
                   qmsg + sizeof(nxt_port_msg_t),
...
nxt_port_frag_start(nxt_task_t* task,
                    nxt_port_t* port,
                    nxt_port_recv_msg_t* msg)
{
  nxt_int_t res;
...
nxt_port_frag_find(nxt_task_t* task, nxt_port_t* port, nxt_port_recv_msg_t* msg)
{
  ...
  nxt_debug(
    task, "%s frag stream #%uD", last ? "last" : "next", msg->port_msg.stream);
  ...
  switch (res) {
    ...
    default:
      nxt_log(
        task, NXT_LOG_INFO, "frag stream #%uD not found", frag_key.stream);
                          ...
                          nxt_port_recv_msg_t* msg)
{
  ...
  if (nxt_slow_path(msg->size < sizeof(nxt_port_msg_t))) {
    nxt_alert(
      task, "port %d: too small message:%uz", port->socket.fd, msg->size);
