...
static void
nxt_router_conf_send(nxt_task_t* task,
                     nxt_router_temp_conf_t* tmcf,
                     nxt_port_msg_type_t type);

static nxt_int_t
nxt_router_conf_create(nxt_task_t* task,
                       nxt_router_temp_conf_t* tmcf,
                       u_char* start,
                       u_char* end);
static nxt_int_t
nxt_router_conf_process_static(nxt_task_t* task,
                               nxt_router_conf_t* rtcf,
                               nxt_conf_value_t* conf);
static nxt_http_forward_t*
nxt_router_conf_forward(nxt_task_t* task, nxt_mp_t* mp, nxt_conf_value_t* conf);
static nxt_int_t
nxt_router_conf_forward_header(nxt_mp_t* mp,
                               nxt_conf_value_t* conf,
...
static void
nxt_router_listen_socket_ready(nxt_task_t* task,
                               nxt_port_recv_msg_t* msg,
                               void* data);
static void
nxt_router_listen_socket_error(nxt_task_t* task,
                               nxt_port_recv_msg_t* msg,
                               void* data);
#if (NXT_TLS)
static void
nxt_router_tls_rpc_handler(nxt_task_t* task,
                           nxt_port_recv_msg_t* msg,
                           void* data);
static nxt_int_t
...
#if (NXT_HAVE_NJS)
static void
nxt_router_js_module_rpc_handler(nxt_task_t* task,
                                 nxt_port_recv_msg_t* msg,
                                 void* data);
static nxt_int_t
...
nxt_router_app_rpc_create(nxt_task_t* task,
                          ...
                          nxt_app_t* app);
static void
nxt_router_app_prefork_ready(nxt_task_t* task,
                             nxt_port_recv_msg_t* msg,
                             void* data);
static void
nxt_router_app_prefork_error(nxt_task_t* task,
                             nxt_port_recv_msg_t* msg,
                             void* data);
static nxt_socket_conf_t*
nxt_router_socket_conf(nxt_task_t* task,
                       nxt_router_temp_conf_t* tmcf,
                       nxt_str_t* name,
                       int backlog);
static nxt_int_t
...
static nxt_int_t
nxt_router_engines_create(nxt_task_t* task,
                          nxt_router_t* router,
                          nxt_router_temp_conf_t* tmcf,
...
static nxt_int_t
nxt_router_threads_create(nxt_task_t* task,
                          nxt_runtime_t* rt,
                          nxt_router_temp_conf_t* tmcf);
static nxt_int_t
nxt_router_thread_create(nxt_task_t* task,
                         nxt_runtime_t* rt,
                         nxt_event_engine_t* engine);
static void
nxt_router_apps_sort(nxt_task_t* task,
                     nxt_router_t* router,
                     nxt_router_temp_conf_t* tmcf);
...
static void
nxt_router_app_port_ready(nxt_task_t* task,
                          nxt_port_recv_msg_t* msg,
                          void* data);
static void
nxt_router_app_port_error(nxt_task_t* task,
                          nxt_port_recv_msg_t* msg,
                          void* data);
...
static void
nxt_router_app_port_release(nxt_task_t* task,
                            nxt_app_t* app,
                            nxt_port_t* port,
                            nxt_apr_action_t action);
static void
nxt_router_app_port_get(nxt_task_t* task,
                        nxt_app_t* app,
                        nxt_request_rpc_data_t* req_rpc_data);
static void
...
nxt_router_app_prepare_request(nxt_task_t* task,
                               nxt_request_rpc_data_t* req_rpc_data);
static nxt_buf_t*
nxt_router_prepare_msg(nxt_task_t* task,
...
nxt_router_greet_controller(nxt_task_t* task, nxt_port_t* controller_port)
{
  nxt_port_socket_write(
    task, controller_port, NXT_PORT_MSG_PROCESS_READY, -1, 0, 0, NULL);
}

static void
nxt_router_start_app_process_handler(nxt_task_t* task,
                                     nxt_port_t* port,
                                     void* data)
{
  size_t size;
  ...
  if (dport != NULL) {
    ...
    if (app->proto_port_requests > 0) {
      nxt_debug(
        task, "app '%V' %p wait for prototype process", &app->name, app);
  ...
  app_joint_rpc = nxt_port_rpc_register_handler_ex(task,
                                                   port,
  ...
  ret = nxt_port_socket_write2(task,
                               dport,
...
nxt_router_start_app_process(nxt_task_t* task, nxt_app_t* app)
{
  ...
  res =
    nxt_port_post(task, router_port, nxt_router_start_app_process_handler, app);
                            ...
                            nxt_request_rpc_data_t* req_rpc_data)
{
  ...
  if (req_rpc_data->app_port != NULL) {
    nxt_router_app_port_release(
      task, app, req_rpc_data->app_port, req_rpc_data->apr_action);
...
nxt_router_conf_data_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  port = nxt_runtime_port_find(
    task->thread->runtime, msg->port_msg.pid, msg->port_msg.reply_port);
  ...
  if (nxt_buf_mem_used_size(&msg->buf->mem) != sizeof(size_t)) {
    nxt_alert(task,
              "conf_data_handler: unexpected buffer size (%d)",
...
fail:
  ...
  nxt_port_socket_write(
    task, port, NXT_PORT_MSG_RPC_ERROR, -1, msg->port_msg.stream, 0, NULL);
...
nxt_router_app_restart_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  reply_port = nxt_runtime_port_find(
    task->thread->runtime, msg->port_msg.pid, msg->port_msg.reply_port);
  ...
  if (nxt_fast_path(app != NULL)) {
    ...
    if (proto_port != NULL) {
      nxt_debug(task,
                "send QUIT to prototype '%V' pid %PI",
                &app->name,
                proto_port->pid);
    ...
    if (proto_port != NULL) {
      (void)nxt_port_socket_write(
        task, proto_port, NXT_PORT_MSG_QUIT, -1, 0, 0, NULL);
  ...
  nxt_port_socket_write(
    task, reply_port, reply, -1, msg->port_msg.stream, 0, NULL);
}
...
static void
nxt_router_app_process_remove_pid(nxt_task_t* task,
                                  nxt_port_t* port,
                                  void* data)
{
  union
...
nxt_router_remove_pid_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  nxt_queue_each(engine, &nxt_router->engines, nxt_event_engine_t, link0)
  {
    if (nxt_fast_path(engine->port != NULL)) {
      nxt_port_post(
        task, engine->port, nxt_router_app_process_remove_pid, msg->u.data);
    }
  }
  nxt_queue_loop;
...
nxt_router_conf_apply(nxt_task_t* task, void* obj, void* data)
{
  ...
  if (qlk != nxt_queue_head(&tmcf->tls)) {
    ...
    nxt_cert_store_get(
      task, &tls->name, tmcf->mem_pool, nxt_router_tls_rpc_handler, tls);
    return;
  }
#endif
  ...
  if (qlk != nxt_queue_head(&tmcf->js_modules)) {
    ...
    nxt_script_store_get(task,
                         &js_module->name,
                         tmcf->mem_pool,
                         nxt_router_js_module_rpc_handler,
                         js_module);
    return;
  }
#endif
                       ...
                       u_char* end)
{
  ...
  if (settings != NULL) {
    ret = nxt_conf_map_object(
      mp, settings, nxt_router_conf, nxt_nitems(nxt_router_conf), rtcf);
    if (ret != NXT_OK) {
  ...
  if (applications != NULL) {
    ...
    for (;;) {
      ...
      if (apcf.targets_value != NULL) {
        ...
        for (i = 0; i < n; i++) {
          (void)nxt_conf_next_object_member(
            apcf.targets_value, &target, &next_target);
  ...
  if (mtypes_conf != NULL) {
    ...
    for (;;) {
      ...
      if (nxt_conf_type(ext_conf) == NXT_CONF_STRING) {
        ...
        ret =
          nxt_http_static_mtypes_hash_add(mp, &rtcf->mtypes_hash, &exten, type);
        if (nxt_slow_path(ret != NXT_OK)) {
          return NXT_ERROR;
        }
...
nxt_router_app_queue_init(nxt_task_t* task, nxt_port_t* port)
{
  ...
  mem = nxt_mem_mmap(
    NULL, sizeof(nxt_app_queue_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
nxt_router_port_queue_init(nxt_task_t* task, nxt_port_t* port)
{
  ...
  mem = nxt_mem_mmap(
    NULL, sizeof(nxt_port_queue_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  if (nxt_slow_path(mem == MAP_FAILED)) {
    nxt_fd_close(fd);
...
nxt_int_t
nxt_router_application_init(nxt_router_conf_t* rtcf,
                            nxt_str_t* name,
                            nxt_str_t* target,
                                    ...
                                    nxt_socket_conf_t* skcf)
{
  ...
  stream = nxt_port_rpc_register_handler(task,
                                         router_port,
  ...
  ret = nxt_port_socket_write(
    task, main_port, NXT_PORT_MSG_SOCKET, -1, stream, router_port->id, b);
                           ...
                           void* data)
{
  ...
  ret =
    task->thread->runtime->tls->server_init(task, mp, tls->tls_init, tls->last);
  if (nxt_slow_path(ret != NXT_OK)) {
    goto fail;
  }
                          ...
                          nxt_app_t* app)
{
  ...
  rpc = nxt_port_rpc_register_handler_ex(task,
                                         router_port,
                             ...
                             void* data)
{
  ...
  nxt_debug(task,
            "app '%V' move new port %PI:%d to spare_ports",
            &app->name,
            port->pid,
            port->id);
                             ...
                             void* data)
{
  ...
  if (rpc->proto) {
    ...
    nxt_log(
      task, NXT_LOG_WARN, "failed to start application \"%V\"", &app->name);
                          ...
                          const nxt_event_interface_t* interface)
{
  ...
  tmcf->engines =
    nxt_array_create(tmcf->mem_pool, threads, sizeof(nxt_router_engine_conf_t));
  if (nxt_slow_path(tmcf->engines == NULL)) {
    return NXT_ERROR;
  }
                              ...
                              nxt_router_engine_conf_t* recf)
{
  ...
  ret = nxt_router_engine_joints_create(
    tmcf, recf, &creating_sockets, nxt_router_listen_socket_create);
  if (nxt_slow_path(ret != NXT_OK)) {
    return ret;
  }

  ret = nxt_router_engine_joints_create(
    tmcf, recf, &updating_sockets, nxt_router_listen_socket_create);
  if (nxt_slow_path(ret != NXT_OK)) {
    return ret;
  }
                              ...
                              nxt_router_engine_conf_t* recf)
{
  ...
  ret = nxt_router_engine_joints_create(
    tmcf, recf, &updating_sockets, nxt_router_listen_socket_update);
  if (nxt_slow_path(ret != NXT_OK)) {
    return ret;
  }
...
nxt_router_listen_socket_delete(nxt_task_t* task, void* obj, void* data)
{
  ...
  nxt_debug(
    task, "engine %p: listen socket delete: %d", engine, lev->socket.fd);
...
nxt_router_listen_socket_close(nxt_task_t* task, void* obj, void* data)
{
  ...
  nxt_debug(task,
            "engine %p: listen socket close: %d",
            task->thread->engine,
            lev->socket.fd);
...
nxt_router_listen_socket_release(nxt_task_t* task, nxt_socket_conf_t* skcf)
{
  ...
  nxt_debug(task,
            "engine %p: listen socket release: ls->count %D",
            task->thread->engine,
            ls->count);
  ...
  (void)nxt_port_socket_write(
    task, main_port, NXT_PORT_MSG_SOCKET_UNLINK, -1, 0, 0, b);
...
void
nxt_router_listen_event_release(nxt_task_t* task,
                                nxt_listen_event_t* lev,
                                nxt_socket_conf_joint_t* joint)
{
  nxt_event_engine_t* engine;
...
static void
nxt_router_response_ready_handler(nxt_task_t* task,
                                  nxt_port_recv_msg_t* msg,
                                  void* data)
{
  size_t b_size, count;
  ...
  if (r->header_sent) {
    ...
    if (nxt_slow_path(count < resp->fields_count)) {
      nxt_alert(task,
                "response buffer too small for fields count: %D",
                resp->fields_count);
      goto fail;
    }
    ...
    for (f = resp->fields; f < resp->fields + resp->fields_count; f++) {
      ...
      nxt_debug(task,
                "header%s: %*s: %*s",
                (field->skip ? " skipped" : ""),
                (size_t)field->name_length,
                field->name,
                (size_t)field->value_length,
                field->value);
                                   ...
                                   nxt_request_rpc_data_t* req_rpc_data)
{
  ...
  app_port = nxt_port_hash_find(
    &app->port_hash, msg->port_msg.pid, msg->port_msg.reply_port);
  if (nxt_slow_path(app_port == NULL)) {
    nxt_thread_mutex_unlock(&app->mutex);
  ...
  if (nxt_queue_chk_remove(&main_app_port->idle_link)) {
    ...
    nxt_debug(task,
              "app '%V' move port %PI:%d out of %s (ack)",
              &app->name,
              main_app_port->pid,
              main_app_port->id,
  ...
  if (req_rpc_data->msg_info.body_fd != -1 || b != NULL) {
    ...
    res = nxt_port_socket_write(task,
                                app_port,
                                NXT_PORT_MSG_REQ_BODY,
                                req_rpc_data->msg_info.body_fd,
                                req_rpc_data->stream,
                                task->thread->engine->port->id,
                                b);
...
static void
nxt_router_response_error_handler(nxt_task_t* task,
                                  nxt_port_recv_msg_t* msg,
                                  void* data)
{
  nxt_request_rpc_data_t* req_rpc_data;
  ...
  if (req_rpc_data->request != NULL) {
    nxt_http_request_error(
      task, req_rpc_data->request, NXT_HTTP_SERVICE_UNAVAILABLE);
  }
                          ...
                          void* data)
{
  ...
  nxt_debug(task,
            "app '%V' new port ready, pid %PI, %d/%d",
            &app->name,
            port->pid,
            app->processes,
            app->pending_processes);
...
nxt_router_app_get_port_for_quit(nxt_task_t* task, nxt_app_t* app)
{
  ...
  nxt_queue_each(port, &app->ports, nxt_port_t, app_link)
  {
    ...
    if (nxt_queue_chk_remove(&port->idle_link)) {
      ...
      nxt_debug(task,
                "app '%V' move port %PI:%d out of %s for quit",
                &app->name,
                port->pid,
                port->id,
                            ...
                            nxt_apr_action_t action)
{
  ...
  if (main_app_port->pair[1] != -1 && main_app_port->active_requests == 0 &&
    ...
    if (app->idle_processes < app->spare_processes) {
      ...
      nxt_debug(task,
                "app '%V' move port %PI:%d to spare_ports",
                &app->name,
                main_app_port->pid,
                main_app_port->id);
    } else {
      ...
      nxt_debug(task,
                "app '%V' move port %PI:%d to idle_ports",
                &app->name,
                main_app_port->pid,
                main_app_port->id);
    }
  ...
  if (main_app_port->pair[1] == -1) {
    nxt_debug(task,
              "app '%V' %p port %p already closed (pid %PI dead?)",
              &app->name,
              app,
              main_app_port,
              main_app_port->pid);
  ...
  nxt_debug(task,
            "app '%V' %p requests queue is empty, keep the port",
            &app->name,
            app);
...
nxt_router_app_port_close(nxt_task_t* task, nxt_port_t* port)
{
  ...
  if (port->id != 0) {
    ...
    nxt_debug(
      task, "app '%V' port (%PI, %d) closed", &app->name, port->pid, port->id);
  ...
  if (nxt_queue_chk_remove(&port->idle_link)) {
    ...
    nxt_debug(task,
              "app '%V' move port %PI:%d out of %s before close",
              &app->name,
              port->pid,
              port->id,
              (port->idle_start ? "idle_ports" : "spare_ports"));
...
nxt_router_adjust_idle_timer(nxt_task_t* task, void* obj, void* data)
{
  ...
  nxt_debug(task,
            "nxt_router_adjust_idle_timer: app \"%V\", queued %b",
            &app->name,
            queued);
  ...
  nxt_debug(task,
            "app '%V' idle_processes %d, spare_processes %d",
            &app->name,
            (int)app->idle_processes,
  ...
  while (app->idle_processes > app->spare_processes) {
    ...
    nxt_debug(task,
              "app '%V' pid %PI, start %M, timeout %M, threshold %M",
              &app->name,
              port->pid,
              port->idle_start,
              timeout,
    ...
    nxt_debug(task,
              "app '%V' move port %PI:%d out of idle_ports (timeout)",
              &app->name,
              port->pid,
              port->id);
    ...
    nxt_debug(
      task, "app '%V' send QUIT to idle port %PI", &app->name, port->pid);
...
nxt_router_free_app(nxt_task_t* task, void* obj, void* data)
{
  ...
  if (proto_port != NULL) {
    nxt_debug(
      task, "send QUIT to prototype '%V' pid %PI", &app->name, proto_port->pid);
...
void
nxt_router_process_http_request(nxt_task_t* task,
                                nxt_http_request_t* r,
                                nxt_http_action_t* action)
{
  nxt_event_engine_t* engine;
  ...
  req_rpc_data =
    nxt_port_rpc_register_handler_ex(task,
                                     engine->port,
                               ...
                               nxt_request_rpc_data_t* req_rpc_data)
{
  ...
  buf = nxt_router_prepare_msg(
    task, req_rpc_data->request, app, nxt_app_msg_prefix[app->type]);
  if (nxt_slow_path(buf == NULL)) {
    nxt_alert(task,
              "stream #%uD, app '%V': failed to prepare app message",
              req_rpc_data->stream,
              &app->name);

    nxt_http_request_error(
      task, req_rpc_data->request, NXT_HTTP_INTERNAL_SERVER_ERROR);

    return;
  }

  nxt_debug(task,
            "about to send %O bytes buffer to app process port %d",
  ...
  res = nxt_app_queue_send(port->queue,
                           &msg,
                           sizeof(msg),
                           req_rpc_data->stream,
                           &notify,
  ...
  if (nxt_fast_path(res == NXT_OK)) {
    if (notify != 0) {
      (void)nxt_port_socket_write(task,
                                  port,
    ...
    nxt_alert(task,
              "stream #%uD, app '%V': failed to send app message",
              req_rpc_data->stream,
              &app->name);
                       ...
                       const nxt_str_t* prefix)
{
  ...
  if (nxt_slow_path(req_size > PORT_MMAP_DATA_SIZE)) {
    nxt_alert(
      task, "headers to big to fit in shared memory (%d)", (int)req_size);
  ...
  *p++ = '\0';
  ...
  p = nxt_cpymem(
    p, nxt_sockaddr_port(r->local), nxt_sockaddr_port_length(r->local));
  *p++ = '\0';
...
nxt_router_get_mmap_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  if (nxt_slow_path(port == NULL)) {
    nxt_alert(task,
              "get_mmap_handler: reply_port %PI:%d not found",
              msg->port_msg.pid,
              msg->port_msg.reply_port);
  ...
  if (nxt_slow_path(nxt_buf_used_size(msg->buf) <
    ...
    nxt_alert(task,
              "get_mmap_handler: message buffer too small (%d)",
  ...
  if (nxt_slow_path(get_mmap_msg->id >= mmaps->size)) {
    ...
    nxt_alert(
      task, "get_mmap_handler: mmap id is too big (%d)", (int)get_mmap_msg->id);
  ...
  nxt_debug(
    task, "get mmap %PI:%d found", msg->port_msg.pid, (int)get_mmap_msg->id);
...
nxt_router_get_port_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  if (nxt_slow_path(reply_port == NULL)) {
    nxt_alert(task,
              "get_port_handler: reply_port %PI:%d not found",
              msg->port_msg.pid,
              msg->port_msg.reply_port);
  ...
  if (nxt_slow_path(nxt_buf_used_size(msg->buf) <
    ...
    nxt_alert(task,
              "get_port_handler: message buffer too small (%d)",
              (int)nxt_buf_used_size(msg->buf));
  ...
  if (nxt_slow_path(port == NULL)) {
    nxt_alert(task,
              "get_port_handler: port %PI:%d not found",
              get_port_msg->pid,
              get_port_msg->id);
