...
nxt_conn_close(nxt_event_engine_t* engine, nxt_conn_t* c)
{
  ...
  if (c->socket.timedout) {
     ...
     */
    ret = setsockopt(
      c->socket.fd, SOL_SOCKET, SO_LINGER, &linger_off, sizeof(struct linger));

    if (nxt_slow_path(ret != 0)) {
      nxt_alert(c->socket.task,
...
nxt_conn_shutdown_handler(nxt_task_t* task, void* obj, void* data)
{
  ...
  nxt_work_queue_add(
    &engine->close_work_queue, nxt_conn_close_handler, task, c, engine);
}
