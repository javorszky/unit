...
static nxt_int_t
nxt_kqueue_create(nxt_event_engine_t* engine,
                  nxt_uint_t mchanges,
                  nxt_uint_t mevents);
static void
...
static void
nxt_kqueue_fd_set(nxt_event_engine_t* engine,
                  nxt_fd_event_t* ev,
                  nxt_int_t filter,
                  nxt_uint_t flags);
...
nxt_kqueue_oneshot_read(nxt_event_engine_t* engine, nxt_fd_event_t* ev)
{
  ...
  nxt_kqueue_fd_set(
    engine, ev, EVFILT_WRITE, EV_ADD | EV_ENABLE | NXT_KEVENT_ONESHOT);
}
...
nxt_kqueue_enable_file(nxt_event_engine_t* engine, nxt_file_event_t* ev)
{
  ...
  nxt_debug(&engine->task,
            "kevent(%d) set: id:%d ft:%i fl:%04Xd, ff:%04XuD",
            engine->u.kqueue.fd,
            ev->file->fd,
            EVFILT_VNODE,
            flags,
                  ...
                  nxt_uint_t flags)
{
  ...
  nxt_debug(ev->task,
            "kevent(%d) set event: id:%d ft:%i fl:%04Xui",
            engine->u.kqueue.fd,
            ev->fd,
            filter,
            flags);
...
nxt_kqueue_get_kevent(nxt_event_engine_t* engine)
{
  ...
  if (nxt_slow_path(nchanges >= engine->u.kqueue.mchanges)) {
    ...
    nxt_debug(
      &engine->task, "kevent(%d) changes:%d", engine->u.kqueue.fd, nchanges);

    ret = kevent(
      engine->u.kqueue.fd, engine->u.kqueue.changes, nchanges, NULL, 0, NULL);

    if (nxt_slow_path(ret != 0)) {
      nxt_alert(
        &engine->task, "kevent(%d) failed %E", engine->u.kqueue.fd, nxt_errno);
  ...
  for (kev = engine->u.kqueue.changes; kev < end; kev++) {
    ...
    switch (kev->filter) {
      ...
      case EVFILT_WRITE:
        ...
        nxt_work_queue_add(
          wq, nxt_kqueue_fd_error_handler, ev->task, ev, ev->data);
        break;

      case EVFILT_VNODE:
        fev = nxt_kevent_get_udata(kev->udata);
        nxt_work_queue_add(
          wq, nxt_kqueue_file_error_handler, fev->task, fev, fev->data);
        break;
    }
  }
}
...
nxt_kqueue_fd_error_handler(nxt_task_t* task, void* obj, void* data)
{
  ...
  if (ev->kq_eof && ev->kq_errno != 0) {
    ...
    nxt_log(task,
            nxt_socket_error_level(ev->kq_errno),
...
nxt_kqueue_poll(nxt_event_engine_t* engine, nxt_msec_t timeout)
{
  ...
  nxt_debug(&engine->task,
            "kevent(%d) changes:%d timeout:%M",
            engine->u.kqueue.fd,
            engine->u.kqueue.nchanges,
            timeout);

  nevents = kevent(engine->u.kqueue.fd,
                   engine->u.kqueue.changes,
                   engine->u.kqueue.nchanges,
                   engine->u.kqueue.events,
                   engine->u.kqueue.mevents,
                   tp);
  ...
  if (nevents == -1) {
    ...
    nxt_log(
      &engine->task, level, "kevent(%d) failed %E", engine->u.kqueue.fd, err);
...
nxt_kqueue_listen_handler(nxt_task_t* task, void* obj, void* data)
{
  ...
  nxt_debug(
    task, "kevent fd:%d avail:%D", lev->socket.fd, lev->socket.kq_available);
...
nxt_kqueue_conn_io_read(nxt_task_t* task, void* obj, void* data)
{
  ...
  if (c->socket.kq_available == 0 && c->socket.kq_eof) {
    ...
    c->socket.closed = 1;
    nxt_work_queue_add(
      c->read_work_queue, c->read_state->close_handler, task, c, data);
    return;
  }
...
nxt_kqueue_conn_io_recvbuf(nxt_conn_t* c, nxt_buf_t* b)
{
  ...
  if (n > 0) {
    ...
    nxt_debug(c->socket.task,
              "kevent fd:%d avail:%D eof:%d",
              c->socket.fd,
              c->socket.kq_available,
              c->socket.kq_eof);
