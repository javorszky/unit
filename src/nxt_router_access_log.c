...
nxt_router_access_log_create(nxt_task_t *task, nxt_router_conf_t *rtcf,
    ...
    if (nxt_conf_type(value) == NXT_CONF_STRING) {
        ...
        ret = nxt_conf_map_object(rtcf->mem_pool, value,
            nxt_router_access_log_conf, nxt_nitems(nxt_router_access_log_conf),
            &alcf);
        if (ret != NXT_OK) {
...
nxt_router_access_log_open(nxt_task_t *task, nxt_router_temp_conf_t *tmcf) {
    ...
    *b->mem.free++ = '\0';
    ...
    stream = nxt_port_rpc_register_handler(task, router_port,
        nxt_router_access_log_ready, nxt_router_access_log_error, -1, tmcf);
...
nxt_router_access_log_reopen_handler(nxt_task_t *task,
    ...
    *b->mem.free++ = '\0';
    ...
    stream = nxt_port_rpc_register_handler(task, router_port,
        nxt_router_access_log_reopen_ready, nxt_router_access_log_reopen_error,
        -1, reopen);
    if (nxt_slow_path(stream == 0)) {
        goto fail;
    }
