...
nxt_http_static(nxt_task_t* task,
                nxt_http_request_t* r,
                nxt_http_action_t* action);
static void
nxt_http_static_iterate(nxt_task_t* task,
                        nxt_http_request_t* r,
                        nxt_http_static_ctx_t* ctx);
static void
nxt_http_static_send(nxt_task_t* task,
                     nxt_http_request_t* r,
                     nxt_http_static_ctx_t* ctx);
static void
nxt_http_static_next(nxt_task_t* task,
                     nxt_http_request_t* r,
                     nxt_http_static_ctx_t* ctx,
                        ...
                        nxt_http_static_ctx_t* ctx)
{
  ...
  nxt_debug(task,
            "http static: \"%V\", index: \"%V\" (chroot: \"%V\")",
            &shr,
            &idx,
            &chr);
#else
                     ...
                     nxt_http_static_ctx_t* ctx)
{
  ...
  if (conf->resolve != 0 || ctx->chroot.length > 0) {
    ...
    if (nxt_fast_path(ret == NXT_OK)) {
      ...
      ret = nxt_file_openat2(
        task, &file, NXT_FILE_RDONLY, NXT_FILE_OPEN, 0, af.fd, resolve);
  ...
  *f = file;
  ...
  if (nxt_fast_path(nxt_is_file(&fi))) {
    ...
    field->value_length =
      nxt_sprintf(
        p, p + length, "\"%xT-%xO\"", nxt_file_mtime(&fi), nxt_file_size(&fi)) -
      p;
