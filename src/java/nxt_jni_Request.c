...
nxt_java_Request_getHeader(JNIEnv* env,
                           jclass cls,
                           jlong req_ptr,
                           jstring name,
                           jint name_len);
...
static jobject JNICALL
nxt_java_Request_getHeaders(JNIEnv* env,
                            jclass cls,
                            jlong req_ptr,
                            jstring name,
                            jint name_len);
static jint JNICALL
nxt_java_Request_getIntHeader(JNIEnv* env,
                              jclass cls,
                              jlong req_ptr,
                              jstring name,
                              jint name_len);
...
static void JNICALL
nxt_java_Request_log(JNIEnv* env,
                     jclass cls,
                     jlong req_info_ptr,
                     jstring msg,
                     jint msg_len);
static void JNICALL
nxt_java_Request_trace(JNIEnv* env,
                       jclass cls,
                       jlong req_info_ptr,
                       jstring msg,
                       jint msg_len);
...
static void JNICALL
nxt_java_Request_sendWsFrameBuf(JNIEnv* env,
                                jclass cls,
                                jlong req_info_ptr,
                                jobject buf,
                                jint pos,
                                ...
                                jboolean last);
static void JNICALL
nxt_java_Request_sendWsFrameArr(JNIEnv* env,
                                jclass cls,
                                jlong req_info_ptr,
                                jarray arr,
                                jint pos,
                           ...
                           jint name_len)
{
  ...
  f = nxt_java_findHeader(
    r->fields, r->fields + r->fields_count, name_str, name_len);
                            ...
                            jint name_len)
{
  ...
  return nxt_java_newHeadersEnumeration(
    env, r->fields, r->fields_count, f - r->fields);
}
...
nxt_java_Request_getLocalAddr(JNIEnv* env, jclass cls, jlong req_ptr)
{
  ...
  return nxt_java_newString(
    env, nxt_unit_sptr_get(&r->local_addr), r->local_addr_length);
}
...
nxt_java_Request_getRemoteAddr(JNIEnv* env, jclass cls, jlong req_ptr)
{
  ...
  return nxt_java_newString(
    env, nxt_unit_sptr_get(&r->remote), r->remote_length);
}
...
void
nxt_java_Request_websocket(JNIEnv* env,
                           jobject jreq,
                           jobject jbuf,
                           uint8_t opcode,
                           uint8_t fin)
{
  (*env)->CallVoidMethod(
    env, jreq, nxt_java_Request_processWsFrame, jbuf, opcode, fin);
}
