...
static void JNICALL
nxt_java_Response_addHeader(JNIEnv* env,
                            jclass cls,
                            jlong req_info_ptr,
                            jarray name,
                            jarray value);
...
nxt_java_get_response_info(jlong req_info_ptr,
                           uint32_t extra_fields,
                           uint32_t extra_data);

static void JNICALL
nxt_java_Response_addIntHeader(JNIEnv* env,
                               jclass cls,
                               jlong req_info_ptr,
                               jarray name,
                               jint value);
...
static void
nxt_java_add_int_header(nxt_unit_request_info_t* req,
                        const char* name,
                        uint8_t name_len,
                        int value);
...
nxt_java_Response_getHeader(JNIEnv* env,
                            jclass cls,
                            jlong req_info_ptr,
                            jarray name);
...
static jobject JNICALL
nxt_java_Response_getHeaders(JNIEnv* env,
                             jclass cls,
                             jlong req_info_ptr,
                             jarray name);
...
static void JNICALL
nxt_java_Response_sendRedirect(JNIEnv* env,
                               jclass cls,
                               jlong req_info_ptr,
                               jarray loc);
...
static void JNICALL
nxt_java_Response_setHeader(JNIEnv* env,
                            jclass cls,
                            jlong req_info_ptr,
                            jarray name,
                            jarray value);
...
static void JNICALL
nxt_java_Response_removeHeader(JNIEnv* env,
                               jclass cls,
                               jlong req_info_ptr,
                               jarray name);
...
static void JNICALL
nxt_java_Response_setIntHeader(JNIEnv* env,
                               jclass cls,
                               jlong req_info_ptr,
                               jarray name,
                               jint value);
...
static void JNICALL
nxt_java_Response_setStatus(JNIEnv* env,
                            jclass cls,
                            jlong req_info_ptr,
                            jint sc);
...
static void JNICALL
nxt_java_Response_setBufferSize(JNIEnv* env,
                                jclass cls,
                                jlong req_info_ptr,
                                jint size);
...
static void JNICALL
nxt_java_Response_setContentLength(JNIEnv* env,
                                   jclass cls,
                                   jlong req_info_ptr,
                                   jlong len);

static void JNICALL
nxt_java_Response_setContentType(JNIEnv* env,
                                 jclass cls,
                                 jlong req_info_ptr,
                                 jarray type);

static void JNICALL
nxt_java_Response_removeContentType(JNIEnv* env,
                                    jclass cls,
                                    jlong req_info_ptr);
...
static void JNICALL
nxt_java_Response_trace(JNIEnv* env,
                        jclass cls,
                        jlong req_info_ptr,
                        jarray msg);
...
nxt_java_initResponse(JNIEnv* env, jobject cl)
{
  ...
  res = (*env)->RegisterNatives(env,
                                nxt_java_Response_class,
                                resp_methods,
...
nxt_java_newResponse(JNIEnv* env, nxt_unit_request_info_t* req)
{
  return (*env)->NewObject(
    env, nxt_java_Response_class, nxt_java_Response_ctor, nxt_ptr2jlong(req));
}
                            ...
                            jarray value)
{
  ...
  rc =
    nxt_unit_response_add_field(req, name_str, name_len, value_str, value_len);
  if (rc != NXT_UNIT_OK) {
    // throw
  }
                           ...
                           uint32_t extra_data)
{
  ...
  if (extra_fields > req->response_max_fields - req->response->fields_count ||
    ...
    if (max_size > nxt_unit_buf_max()) {
      nxt_unit_req_warn(
        req, "required max_size is too big: %" PRIu32, max_size);
      return NULL;
    }
    ...
    if (rc != NXT_UNIT_OK) {
      nxt_unit_req_warn(req,
                        "reallocation failed: %" PRIu32 ", %" PRIu32,
                        2 * req->response_max_fields,
                        max_size);
      return NULL;
    }
  }
...
nxt_java_Response_getHeaderNames(JNIEnv* env, jclass cls, jlong req_info_ptr)
{
  ...
  return nxt_java_newHeaderNamesEnumeration(
    env, req->response->fields, req->response->fields_count);
}
                             ...
                             jarray name)
{
  ...
  f = nxt_java_findHeader(
    resp->fields, resp->fields + resp->fields_count, name_str, name_len);
  ...
  return nxt_java_newHeadersEnumeration(
    env, resp->fields, resp->fields_count, f - resp->fields);
}
                               ...
                               jarray loc)
{
  ...
  rc = nxt_java_response_set_header(
    req_info_ptr, location, location_len, loc_str, loc_len);
  if (rc != NXT_UNIT_OK) {
    // throw
  }
                            ...
                            jarray value)
{
  ...
  rc = nxt_java_response_set_header(
    req_info_ptr, name_str, name_len, value_str, value_len);
  if (rc != NXT_UNIT_OK) {
    // throw
  }
                                 ...
                                 jarray type)
{
  ...
  rc = nxt_java_response_set_header(
    req_info_ptr, content_type, content_type_len, type_str, type_len);
  if (rc != NXT_UNIT_OK) {
    // throw
  }
...
{
  nxt_java_response_remove_header(
    req_info_ptr, "Content-Type", sizeof("Content-Type") - 1);
}
