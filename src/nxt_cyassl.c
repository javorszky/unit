...
static void
nxt_cyassl_conn_init(nxt_thread_t* thr,
...
static ssize_t
nxt_cyassl_conn_io_write_chunk(nxt_thread_t* thr,
                               nxt_event_conn_t* c,
                               nxt_buf_t* b,
                               size_t limit);
static ssize_t
...
static nxt_int_t
nxt_cyassl_conn_test_error(nxt_thread_t* thr,
                           nxt_event_conn_t* c,
                           int err,
...
static void nxt_cdecl
nxt_cyassl_log_error(nxt_uint_t level,
                     nxt_log_t* log,
                     int ret,
                     const char* fmt,
                     ...);
...
nxt_cyassl_start(void)
{
  ...
  nxt_thread_log_error(
    NXT_LOG_INFO, "CyaSSL version: %s", LIBCYASSL_VERSION_STRING);
...
nxt_cyassl_server_init(nxt_ssltls_conf_t* conf)
{
  ...
  if (err != SSL_SUCCESS) {
    nxt_cyassl_log_error(NXT_LOG_ALERT,
                         thr->log,
                         err,
                     ...
                     nxt_event_conn_t* c)
{
  ...
  s = CyaSSL_new(ctx);
  if (s == NULL) {
    nxt_cyassl_log_error(
      NXT_LOG_ALERT, c->socket.log, 0, "CyaSSL_new() failed");
    goto fail;
  }
...
nxt_cyassl_conn_io_read(nxt_thread_t* thr, void* obj, void* data)
{
  ...
  if (b != NULL) {
    ...
    if (ret > 0) {
      ...
      nxt_cyassl_conn_error(c,
                            err,
                            "CyaSSL_read(%d, %p, %uz) failed",
                            c->socket.fd,
                            b->mem.free,
                            b->mem.end - b->mem.free);
...
nxt_cyassl_conn_io_send(nxt_event_conn_t* c, void* buf, size_t size)
{
  ...
  if (nxt_slow_path(n == NXT_ERROR)) {
    nxt_cyassl_conn_error(
      c, err, "CyaSSL_write(%d, %p, %uz) failed", c->socket.fd, buf, size);
  }
...
nxt_cyassl_conn_io_shutdown(nxt_thread_t* thr, void* obj, void* data)
{
  ...
  nxt_event_conn_io_handle(
    thr, c->write_work_queue, c->write_state->close_handler, c, data);
}
