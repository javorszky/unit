...
static void
nxt_h1p_conn_ws_frame_process(nxt_task_t* task,
                              nxt_conn_t* c,
                              nxt_h1proto_t* h1p,
...
static void
hxt_h1p_send_ws_error(nxt_task_t* task,
...
nxt_h1p_conn_ws_frame_header_read(nxt_task_t* task, void* obj, void* data)
{
  ...
  if ((wsh->opcode & NXT_WEBSOCKET_OP_CTRL) != 0) {
    ...
    if (nxt_slow_path(wsh->payload_len > 125)) {
      hxt_h1p_send_ws_error(task,
                            r,
                            &nxt_ws_err_ctrl_too_big,
...
nxt_h1p_conn_ws_keepalive_enable(nxt_task_t* task, nxt_h1proto_t* h1p)
{
  ...
  nxt_timer_add(
    task->thread->engine, timer, h1p->websocket_timer->keepalive_interval);
}
