...
nxt_socket_t
nxt_socket_create(nxt_task_t* task,
                  nxt_uint_t domain,
                  nxt_uint_t type,
                  nxt_uint_t protocol,
                  nxt_uint_t flags)
...
nxt_int_t
nxt_socket_getsockopt(nxt_task_t* task,
                      nxt_socket_t s,
                      nxt_uint_t level,
                      nxt_uint_t sockopt)
{
  ...
  if (nxt_fast_path(getsockopt(s, level, sockopt, &val, &len) == 0)) {
    nxt_debug(task,
              ...
              s,
              level,
              nxt_socket_sockopt_name(level, sockopt),
              val);
    return val;
  }

  nxt_alert(task,
            "getsockopt(%d, %ui, %s) failed %E",
            s,
            level,
            nxt_socket_sockopt_name(level, sockopt),
            nxt_socket_errno);
...
nxt_int_t
nxt_socket_setsockopt(nxt_task_t* task,
                      nxt_socket_t s,
                      nxt_uint_t level,
                      nxt_uint_t sockopt,
                      int val)
...
{
  nxt_debug(
    task, "bind(%d, %*s)", s, (size_t)sa->length, nxt_sockaddr_start(sa));
...
nxt_socket_connect(nxt_task_t* task, nxt_socket_t s, nxt_sockaddr_t* sa)
{
  ...
  nxt_debug(
    task, "connect(%d, %*s)", s, (size_t)sa->length, nxt_sockaddr_start(sa));
