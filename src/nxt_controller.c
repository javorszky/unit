...
static nxt_int_t
nxt_controller_file_read(nxt_task_t* task,
                         const char* name,
                         nxt_str_t* str,
                         nxt_mp_t* mp);
static nxt_int_t
...
static void
nxt_controller_conf_init_handler(nxt_task_t* task,
                                 nxt_port_recv_msg_t* msg,
                                 void* data);
static void
nxt_controller_flush_requests(nxt_task_t* task);
static nxt_int_t
nxt_controller_conf_send(nxt_task_t* task,
                         nxt_mp_t* mp,
                         nxt_conf_value_t* conf,
...
static nxt_int_t
nxt_controller_request_content_length(void* ctx,
...
static void
nxt_controller_process_config(nxt_task_t* task,
...
static void
nxt_controller_status_handler(nxt_task_t* task,
                              nxt_port_recv_msg_t* msg,
                              void* data);
static void
...
nxt_controller_process_cert(nxt_task_t* task,
                            ...
                            nxt_str_t* path);
static void
nxt_controller_process_cert_save(nxt_task_t* task,
                                 nxt_port_recv_msg_t* msg,
                                 void* data);
static nxt_bool_t
...
nxt_controller_process_script(nxt_task_t* task,
                              ...
                              nxt_str_t* path);
static void
nxt_controller_process_script_save(nxt_task_t* task,
                                   nxt_port_recv_msg_t* msg,
                                   void* data);
static nxt_bool_t
...
nxt_controller_process_control(nxt_task_t* task,
                               ...
                               nxt_str_t* path);
static void
nxt_controller_app_restart_handler(nxt_task_t* task,
                                   nxt_port_recv_msg_t* msg,
                                   void* data);
static void
nxt_controller_conf_handler(nxt_task_t* task,
                            nxt_port_recv_msg_t* msg,
                            void* data);
static void
nxt_controller_conf_store(nxt_task_t* task, nxt_conf_value_t* conf);
static void
nxt_controller_response(nxt_task_t* task,
                        nxt_controller_request_t* req,
                        nxt_controller_response_t* resp);
static u_char*
nxt_controller_date(u_char* buf,
                    nxt_realtime_t* now,
                    struct tm* tm,
                    size_t size,
                    const char* format);
...
nxt_controller_prefork(nxt_task_t* task, nxt_process_t* process, nxt_mp_t* mp)
{
  ...
  nxt_mp_cleanup(
    mp, nxt_controller_script_cleanup, task, ctrl_init.scripts, rt);
#endif
...
nxt_controller_send_current_conf(nxt_task_t* task)
{
  ...
  if (conf != NULL) {
    rc = nxt_controller_conf_send(task,
                                  nxt_controller_conf.pool,
                                  conf,
                                  nxt_controller_conf_init_handler,
                                  NULL);
                         ...
                         void* data)
{
  ...
  stream = nxt_port_rpc_register_handler(
    task, controller_port, handler, handler, router_port->pid, data);
  if (nxt_slow_path(stream == 0)) {
    goto fail;
  }

  rc = nxt_port_socket_write(task,
                             router_port,
                              ...
                              nxt_str_t* path)
{
  ...
  if (post || nxt_str_eq(&req->parser.method, "PUT", 3)) {
    ...
    rc = nxt_controller_conf_send(
      task, mp, value, nxt_controller_conf_handler, req);
  ...
  if (nxt_str_eq(&req->parser.method, "DELETE", 6)) {
    ...
    if (path->length == 1) {
      ...
      rc = nxt_conf_op_compile(
        c->mem_pool, &ops, nxt_controller_conf.root, path, NULL, 0);
                            ...
                            nxt_str_t* path)
{
  ...
  if (nxt_str_eq(&req->parser.method, "PUT", 3)) {
    ...
    nxt_cert_store_get(
      task, &name, c->mem_pool, nxt_controller_process_cert_save, req);
    return;
  }
                              ...
                              nxt_str_t* path)
{
  ...
  if (nxt_str_eq(&req->parser.method, "PUT", 3)) {
    ...
    nxt_script_store_get(
      task, &name, c->mem_pool, nxt_controller_process_script_save, req);
    return;
  }
                               ...
                               nxt_str_t* path)
{
  ...
  rc = nxt_port_socket_write(
    task, router_port, NXT_PORT_MSG_APP_RESTART, -1, stream, 0, b);
  if (nxt_slow_path(rc != NXT_OK)) {
    nxt_port_rpc_cancel(task, controller_port, stream);
                        ...
                        nxt_controller_response_t* resp)
{
  ...
  b->mem.free = nxt_sprintf(
    b->mem.free, b->mem.end, "%uz", nxt_buf_mem_used_size(&body->mem));
