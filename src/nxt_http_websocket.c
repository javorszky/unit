...
nxt_http_websocket_client(nxt_task_t* task, void* obj, void* data)
{
  ...
  frame_size =
    nxt_websocket_frame_header_size(wsh) + nxt_websocket_frame_payload_len(wsh);
  ...
  while (b != NULL && frame_size > 0) {
    ...
    while (copy_size > 0) {
      if (buf == NULL || buf_free_size == 0) {
        ...
        buf = nxt_port_mmap_get_buf(
          task, &req_rpc_data->app->outgoing, buf_free_size);
