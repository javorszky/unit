...
error:
  ...
  nxt_work_queue_add(
    c->write_work_queue, c->write_state->error_handler, task, c, data);
}
...
nxt_conn_io_sendfile(nxt_task_t* task, nxt_sendbuf_t* sb)
{
  ...
  for (;;) {
    ...
    if (nxt_slow_path(n == 0)) {
      nxt_alert(
        task, "sendfile() reported that file was truncated at %O", b->file_pos);
...
nxt_sendfile(int fd, int s, off_t pos, size_t size)
{
  ...
  map = nxt_mem_mmap(
    NULL, size + page_off, PROT_READ, MAP_SHARED, fd, pos - page_off);
...
ssize_t
nxt_conn_io_writev(nxt_task_t* task,
                   nxt_sendbuf_t* sb,
                   struct iovec* iov,
                   nxt_uint_t niov)
{
...
nxt_event_conn_write_limit(nxt_conn_t* c)
{
  ...
  nxt_debug(c->socket.task,
            "event conn correction:%z average:%0.3f",
            correction,
            rate->average);
...
nxt_bool_t
nxt_event_conn_write_delayed(nxt_event_engine_t* engine,
                             nxt_conn_t* c,
                             size_t sent)
{
  return 0;
...
nxt_event_conn_io_send(nxt_conn_t* c, void* buf, size_t size)
{
  ...
  for (;;) {
    ...
    nxt_debug(
      c->socket.task, "send(%d, %p, %uz): %z", c->socket.fd, buf, size, n);
