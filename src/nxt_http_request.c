...
static nxt_int_t
nxt_http_request_forward(nxt_task_t* task,
...
nxt_http_request_client_ip_sockaddr(nxt_http_request_t* r,
                                    u_char* start,
                                    size_t len);
static void
...
static u_char*
nxt_http_date_cache_handler(u_char* buf,
                            nxt_realtime_t* now,
                            struct tm* tm,
                            size_t size,
                            const char* format);
...
nxt_http_argument(nxt_array_t* array,
                  u_char* name,
                  size_t name_length,
                  uint32_t hash,
                  u_char* start,
...
static nxt_http_name_value_t*
nxt_http_cookie(nxt_array_t* array,
                u_char* name,
                size_t name_length,
                u_char* start,
                const u_char* end);
  ...
  (nxt_http_field_hash_end(nxt_http_field_hash_char(                           \
     nxt_http_field_hash_char(                                                 \
       nxt_http_field_hash_char(                                               \
         nxt_http_field_hash_char(                                             \
           nxt_http_field_hash_char(                                           \
             nxt_http_field_hash_char(NXT_HTTP_FIELD_HASH_INIT, 'c'), 'o'),    \
           'o'),                                                               \
         'k'),                                                                 \
       'i'),                                                                   \
     'e')) &                                                                   \
                         ...
                         nxt_http_forward_t* forward)
{
  ...
  nxt_list_each(f, r->fields)
  {
    if (client_ip_fields != NULL && f->hash == client_ip->header_hash &&
        ...
        nxt_memcasecmp(
          f->name, client_ip->header->start, client_ip->header->length) == 0) {
    ...
    if (protocol->header != NULL && protocol_field == NULL &&
        ...
        nxt_memcasecmp(
          f->name, protocol->header->start, protocol->header->length) == 0) {
      protocol_field = f;
    }
  }
  nxt_list_loop;
