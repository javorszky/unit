...
#define nxt_conn_tcp_nodelay_on(task, c)                                       \
  do {                                                                         \
    ...
    if ((c)->remote->u.sockaddr.sa_family != AF_UNIX) {                        \
      ret = nxt_socket_setsockopt(                                             \
        task, (c)->socket.fd, IPPROTO_TCP, TCP_NODELAY, 1);                    \
                                                                               \
...
NXT_EXPORT void
nxt_conn_timer(nxt_event_engine_t* engine,
               nxt_conn_t* c,
...
void
nxt_conn_accept_error(nxt_task_t* task,
...
#define nxt_conn_read(engine, c)                                               \
  do {                                                                         \
    ...
    c->socket.read_work_queue = &e->read_work_queue;                           \
                                                                               \
    nxt_work_queue_add(                                                        \
      &e->read_work_queue, c->io->read, c->socket.task, c, c->socket.data);    \
  } while (0)
...
#define nxt_conn_write(engine, c)                                              \
  do {                                                                         \
    ...
    c->socket.write_work_queue = &e->write_work_queue;                         \
                                                                               \
    nxt_work_queue_add(                                                        \
      &e->write_work_queue, c->io->write, c->socket.task, c, c->socket.data);  \
  } while (0)
