...
ssize_t
nxt_freebsd_event_conn_io_sendfile(nxt_event_conn_t* c,
                                   nxt_buf_t* b,
                                   size_t limit);

static int
nxt_sys_sendfile(int fd,
                 int s,
                 off_t offset,
                 size_t nbytes,
                 struct sf_hdtr* hdtr,
                                   ...
                                   size_t limit)
{
  ...
  nxt_debug(c->socket.task,
            "sendfile(%FD, %d, @%O, %uz) hd:%ui tr:%ui",
            fb->file->fd,
            c->socket.fd,
            fb->file_pos,
            file_size,
  ...
  sent = 0;
  n = nxt_sys_sendfile(
    fb->file->fd, c->socket.fd, fb->file_pos, file_size, ht, &sent, 0);
