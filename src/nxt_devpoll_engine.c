...
static nxt_int_t
nxt_devpoll_create(nxt_event_engine_t* engine,
                   nxt_uint_t mchanges,
                   nxt_uint_t mevents);
static void
...
static void
nxt_devpoll_change(nxt_event_engine_t* engine,
                   nxt_fd_event_t* ev,
                   nxt_uint_t op,
                   nxt_uint_t events);
static nxt_int_t
                   ...
                   nxt_uint_t events)
{
  ...
  nxt_debug(ev->task,
            "devpoll %d change fd:%d op:%ui ev:%04Xi",
            engine->u.devpoll.fd,
            ev->fd,
            op,
            events);
  ...
  do {
    ...
    if (change->op == NXT_DEVPOLL_ADD) {
      ...
      nxt_fd_event_hash_delete(
        &engine->task, &engine->u.devpoll.fd_hash, ev->fd, 0);
    }
...
nxt_devpoll_poll(nxt_event_engine_t* engine, nxt_msec_t timeout)
{
  ...
  nxt_debug(
    &engine->task, "ioctl(%d, DP_POLL): %d", engine->u.devpoll.fd, nevents);
  ...
  for (i = 0; i < nevents; i++) {
    ...
    nxt_debug(ev->task,
              "devpoll: fd:%d ev:%04uXi rd:%d wr:%d",
              fd,
              events,
              ev->read,
              ev->write);
