  ...
  for (;;) {
    ...
    */
    ...
    if (nxt_strcmp(type, PEM_STRING_PKCS8INF) == 0) {
      ...
      if (p8inf == NULL) {
        nxt_openssl_log_error(
          task, NXT_LOG_ALERT, "d2i_PKCS8_PRIV_KEY_INFO() failed");
        goto done;
      }
    ...
    if (suffix != 0) {
      ...
      if (ameth == NULL) {
        nxt_openssl_log_error(
          task, NXT_LOG_ALERT, "EVP_PKEY_asn1_find_str() failed");
        goto done;
      }
  ...
  if (cert->key != NULL) {
    ...
    switch (EVP_PKEY_base_id(key)) {
      case EVP_PKEY_RSA:
          ...
          buf, buf + sizeof(buf), "RSA (%d bits)", EVP_PKEY_bits(key));
      ...
      case EVP_PKEY_DH:
          ...
          buf, buf + sizeof(buf), "DH (%d bits)", EVP_PKEY_bits(key));
...
nxt_cert_name_details(nxt_mp_t* mp, X509* x509, nxt_bool_t issuer)
{
  ...
  for (n = 0, i = 0; n != nxt_nitems(nids) && i != count; n++) {
    ...
    len = X509_NAME_get_text_by_NID(
      x509_name, nids[n].nid, (char*)buf, sizeof(buf));
...
nxt_cert_store_load(nxt_task_t* task, nxt_mp_t* mp)
{
  ...
  for (;;) {
    ...
    ret = nxt_file_open(
      task, &file, NXT_FILE_RDONLY, NXT_FILE_OPEN, NXT_FILE_OWNER_ACCESS);
...
void
nxt_cert_store_get(nxt_task_t* task,
                   nxt_str_t* name,
                   nxt_mp_t* mp,
                   ...
                   void* ctx)
{
  ...
  *b->mem.free++ = '\0';
  ...
  ret = nxt_port_socket_write(
    task, main_port, NXT_PORT_MSG_CERT_GET, -1, stream, recv_port->id, b);
...
nxt_cert_store_get_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  if (nxt_slow_path(port == NULL)) {
    nxt_alert(task,
              "process port not found (pid %PI, reply_port %d)",
              msg->port_msg.pid,
              msg->port_msg.reply_port);
    return;
  }
  ...
  ret = nxt_file_open(
    task, &file, NXT_FILE_RDWR, NXT_FILE_CREATE_OR_OPEN, NXT_FILE_OWNER_ACCESS);
...
nxt_cert_store_delete(nxt_task_t* task, nxt_str_t* name, nxt_mp_t* mp)
{
  ...
  if (nxt_fast_path(b != NULL)) {
    ...
    *b->mem.free++ = '\0';
    ...
    (void)nxt_port_socket_write(
      task, main_port, NXT_PORT_MSG_CERT_DELETE, -1, 0, 0, b);
  }
}
...
nxt_cert_store_delete_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  if (nxt_slow_path(nxt_recv_msg_cmsg_pid(msg) != ctl_port->pid)) {
    nxt_alert(task,
              "process %PI cannot delete certificates",
