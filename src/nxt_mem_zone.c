...
static void*
nxt_mem_zone_alloc_small(nxt_mem_zone_t* zone,
...
nxt_mem_zone_alloc_pages(nxt_mem_zone_t* zone,
                         size_t alignment,
                         uint32_t pages);
static nxt_mem_zone_free_block_t*
nxt_mem_zone_find_free_block(nxt_mem_zone_t* zone,
                             nxt_rbtree_node_t* node,
                             uint32_t alignment,
                             uint32_t pages);
static const char*
nxt_mem_zone_free_chunk(nxt_mem_zone_t* zone,
                        nxt_mem_zone_page_t* page,
                        void* p);
static void
nxt_mem_zone_free_pages(nxt_mem_zone_t* zone,
                        nxt_mem_zone_page_t* page,
                        nxt_uint_t count);
...
static nxt_log_moderation_t nxt_mem_zone_log_moderation = {
  NXT_LOG_ALERT,
  2,
  "mem_zone_alloc() failed, not enough memory",
  NXT_LOG_MODERATION
};
...
nxt_mem_zone_pages(u_char* start, size_t zone_size, nxt_uint_t page_size)
{
  ...
  nxt_thread_log_debug("mem zone pages: %uD, unused:%z",
                       pages,
...
nxt_mem_zone_align(nxt_mem_zone_t* zone, size_t alignment, size_t size)
{
  ...
  if (size <= zone->max_chunk_size && alignment <= zone->max_chunk_size) {
    ...
    nxt_thread_log_debug(
      "mem zone alloc: @%uz:%uz chunk:%uD", alignment, size, slot->size);
                             ...
                             uint32_t pages)
{
  ...
  return nxt_mem_zone_find_free_block(
    zone, block->node.right, alignment, pages);
}
