...
static nxt_log_moderation_t nxt_mem_zone_log_moderation = {NXT_LOG_ALERT, 2,
    "mem_zone_alloc() failed, not enough memory", NXT_LOG_MODERATION};
...
nxt_mem_zone_alloc_pages(nxt_mem_zone_t *zone, size_t alignment,
    ...
    block = nxt_mem_zone_find_free_block(zone,
        nxt_rbtree_root(&zone->free_pages), alignment, pages);
...
nxt_mem_zone_free(nxt_mem_zone_t *zone, void *p) {
    ...
    if (nxt_fast_path(
            zone->start <= (u_char *) p && (u_char *) p < zone->end)) {
        page = nxt_mem_zone_addr_page(zone, p);
