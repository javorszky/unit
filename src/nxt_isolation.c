...
#if (NXT_HAVE_CGROUP)
    if (nxt_slow_path(
            nxt_isolation_set_cgroup(task, isolation, process) != NXT_OK)) {
        return NXT_ERROR;
    }
#endif
...
#if (NXT_HAVE_LINUX_NS)
    if (nxt_slow_path(
            nxt_isolation_set_namespaces(task, isolation, process) != NXT_OK)) {
        return NXT_ERROR;
    }
#endif
...
#if (NXT_HAVE_CLONE_NEWUSER)
    if (nxt_slow_path(
            nxt_isolation_set_creds(task, isolation, process) != NXT_OK)) {
        return NXT_ERROR;
    }
#endif
...
#if (NXT_HAVE_ISOLATION_ROOTFS)
    if (nxt_slow_path(
            nxt_isolation_set_rootfs(task, isolation, process) != NXT_OK)) {
        return NXT_ERROR;
    }

    if (nxt_slow_path(
            nxt_isolation_set_automount(task, isolation, process) != NXT_OK)) {
        return NXT_ERROR;
    }
#endif
...
#if (NXT_HAVE_PR_SET_NO_NEW_PRIVS)
    if (nxt_slow_path(
            nxt_isolation_set_new_privs(task, isolation, process) != NXT_OK)) {
        return NXT_ERROR;
    }
#endif
