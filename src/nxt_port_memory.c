...
nxt_port_incoming_port_mmap(nxt_task_t* task,
                            nxt_process_t* process,
                            nxt_fd_t fd)
{
  void* mem;
  ...
  mem = nxt_mem_mmap(
    NULL, mmap_stat.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
nxt_port_new_port_mmap(nxt_task_t* task,
                       ...
                       nxt_int_t n)
{
  ...
  mem = nxt_mem_mmap(
    NULL, PORT_MMAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
static nxt_port_mmap_handler_t*
nxt_port_mmap_get(nxt_task_t* task,
                  nxt_port_mmaps_t* mmaps,
                  nxt_chunk_id_t* c,
                  nxt_int_t n,
                  nxt_bool_t tracking)
...
nxt_port_mmap_get_buf(nxt_task_t* task, nxt_port_mmaps_t* mmaps, size_t size)
{
  ...
  nxt_debug(task,
            "outgoing mmap buf allocation: %p [%p,%uz] %PI->%PI,%d,%d",
            b,
            b->mem.start,
            b->mem.end - b->mem.start,
            hdr->src_pid,
...
nxt_int_t
nxt_port_mmap_increase_buf(nxt_task_t* task,
                           nxt_buf_t* b,
                           size_t size,
                           size_t min_size)
{
...
static nxt_buf_t*
nxt_port_mmap_get_incoming_buf(nxt_task_t* task,
                               nxt_port_t* port,
                               nxt_pid_t spid,
                               nxt_port_mmap_msg_t* mmap_msg)
{
  ...
  nxt_debug(task,
            "incoming mmap buf allocation: %p [%p,%uz] %PI->%PI,%d,%d",
            b,
            b->mem.start,
            b->mem.end - b->mem.start,
            hdr->src_pid,
...
void
nxt_port_mmap_write(nxt_task_t* task,
                    nxt_port_t* port,
                    ...
                    void* mmsg_buf)
{
  ...
  for (i = 0; i < sb->niov; i++, mmap_msg++) {
    ...
    if (nxt_slow_path(bmem == NULL)) {
      nxt_log_error(
        NXT_LOG_ERR, task->log, "failed to find buf for iobuf[%d]", i);
      return;
  ...
  for (b = msg->buf; b != NULL; b = b->next) {
    ...
    while (mmap_msg < end) {
      ...
      *pb = nxt_port_mmap_get_incoming_buf(
        task, msg->port, msg->port_msg.pid, mmap_msg);
...
nxt_port_broadcast_shm_ack(nxt_task_t* task, nxt_port_t* port, void* data)
{
  ...
  nxt_queue_each(port, &process->ports, nxt_port_t, link)
  {
    (void)nxt_port_socket_write(
      task, port, NXT_PORT_MSG_SHM_ACK, -1, 0, 0, NULL);
  }
  nxt_queue_loop;
}
