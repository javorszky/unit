...
static nxt_int_t
nxt_php_setup(nxt_task_t* task,
              nxt_process_t* process,
              nxt_common_app_conf_t* conf);
static nxt_int_t
nxt_php_start(nxt_task_t* task, nxt_process_data_t* data);
static nxt_int_t
nxt_php_set_target(nxt_task_t* task,
...
#endif
static void
nxt_php_disable(nxt_task_t* task,
                const char* type,
                nxt_str_t* value,
                char** ptr,
...
static void
nxt_zend_stream_init_fp(zend_file_handle* handle,
                        FILE* fp,
                        const char* filename);
#endif
static void
...
static void
nxt_php_set_sptr(nxt_unit_request_info_t* req,
                 const char* name,
                 nxt_unit_sptr_t* v,
                 uint32_t len,
                 zval* track_vars_array TSRMLS_DC);
nxt_inline void
nxt_php_set_str(nxt_unit_request_info_t* req,
                const char* name,
                nxt_str_t* s,
                zval* track_vars_array TSRMLS_DC);
static void
nxt_php_set_cstr(nxt_unit_request_info_t* req,
                 const char* name,
                 const char* str,
                 uint32_t len,
                 zval* track_vars_array TSRMLS_DC);
...
#ifdef NXT_PHP7
  nxt_php_server_ag =
    zend_hash_str_find_ptr(CG(auto_globals), "_SERVER", nxt_length("_SERVER"));
#else
  zend_hash_quick_find(CG(auto_globals),
                       "_SERVER",
                       sizeof("_SERVER"),
  ...
  *p = '\0';
  ...
  if (value != NULL) {
    ...
    *p = '\0';
    ...
    if (!nxt_str_start(
          &target->script_filename, target->root.start, target->root.length)) {
  ...
  if (options != NULL) {
    ...
    for (;;) {
      ...
      if (nxt_str_eq(&name, "disable_classes", 15)) {
        nxt_php_disable(
          task, "class", &value, &PG(disable_classes), zend_disable_class);
        continue;
      }
    }
  }
...
nxt_php_do_301(nxt_unit_request_info_t* req)
{
  ...
  *p = '\0';
  ...
  nxt_unit_response_init(
    req, NXT_HTTP_MOVED_PERMANENTLY, 1, nxt_length("Location") + size);
  nxt_unit_response_add_field(
    req, "Location", nxt_length("Location"), url, size);
...
nxt_php_vcwd_chdir(nxt_unit_request_info_t* req, u_char* dir)
{
  if (nxt_slow_path(VCWD_CHDIR((char*)dir) != 0)) {
    nxt_unit_req_alert(
      req, "VCWD_CHDIR(%s) failed (%d: %s)", dir, errno, strerror(errno));
  }
}
...
nxt_php_send_headers(sapi_headers_struct* sapi_headers TSRMLS_DC)
{
  ...
  for (h = zend_llist_get_first_ex(&sapi_headers->headers, &zpos); h;
    ...
    if (nxt_slow_path(colon == NULL)) {
      nxt_unit_req_warn(
        req, "colon not found in header '%.*s'", (int)h->header_len, h->header);
      continue;
    }
    ...
    nxt_unit_response_add_field(req,
                                h->header,
                                colon - h->header,
                                value,
...
nxt_php_register_variables(zval* track_vars_array TSRMLS_DC)
{
  ...
  if (ctx->path_info.length != 0) {
    nxt_php_set_sptr(
      req, "PHP_SELF", &r->path, r->path_length, track_vars_array TSRMLS_CC);

    nxt_php_set_str(
      req, "PATH_INFO", &ctx->path_info, track_vars_array TSRMLS_CC);

  } else {
    nxt_php_set_str(
      req, "PHP_SELF", &ctx->script_name, track_vars_array TSRMLS_CC);
  }
  ...
  nxt_php_set_str(
    req, "SCRIPT_NAME", &ctx->script_name, track_vars_array TSRMLS_CC);
  ...
  nxt_php_set_str(
    req, "SCRIPT_FILENAME", &ctx->script_filename, track_vars_array TSRMLS_CC);
  ...
  for (f = r->fields; f < f_end; f++) {
    ...
    nxt_php_set_sptr(
      req, name, &f->value, f->value_length, track_vars_array TSRMLS_CC);
  }
                 ...
                 zval* track_vars_array TSRMLS_DC)
{
  ...
  if (sapi_module.input_filter(
        PARSE_SERVER, (char*)name, &str, len, &new_len TSRMLS_CC)) {
    php_register_variable_safe(
      (char*)name, str, new_len, track_vars_array TSRMLS_CC);
  }
}
...
{
  nxt_php_set_cstr(
    req, name, (char*)s->start, s->length, track_vars_array TSRMLS_CC);
}
                 ...
                 zval* track_vars_array TSRMLS_DC)
{
  ...
  php_register_variable_safe(
    (char*)name, (char*)cstr, len, track_vars_array TSRMLS_CC);
}
...
#endif
{
  ...
  if (ctx != NULL) {
    ...
    nxt_unit_log(
      nxt_php_unit_ctx, NXT_UNIT_LOG_NOTICE, "php message: %s", message);
  }
}
