...
static zend_module_entry nxt_php_unit_module = {STANDARD_MODULE_HEADER, "unit",
    ...
    NULL,                   /* information */
    NXT_VERSION, STANDARD_MODULE_PROPERTIES};
...
nxt_php_alter_option(nxt_str_t *name, nxt_str_t *value, int type) {
    ...
    if (ini_entry->on_modify
        && ini_entry->on_modify(ini_entry, zs, ini_entry->mh_arg1,
               ini_entry->mh_arg2, ini_entry->mh_arg3, ZEND_INI_STAGE_ACTIVATE)
               != SUCCESS) {
...
nxt_php_alter_option(nxt_str_t *name, nxt_str_t *value, int type) {
    ...
    if (ini_entry->on_modify
        && ini_entry->on_modify(ini_entry, cstr, value->length,
               ini_entry->mh_arg1, ini_entry->mh_arg2, ini_entry->mh_arg3,
