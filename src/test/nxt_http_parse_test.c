...
static nxt_int_t
nxt_http_parse_test_bench(nxt_thread_t* thr,
                          nxt_str_t* request,
                          nxt_lvlhsh_t* hash,
                          const char* name,
                          nxt_uint_t n);
...
nxt_http_parse_test(nxt_thread_t* thr)
{
  ...
  colls =
    nxt_http_fields_hash_collisions(&hash,
  ...
  rc = nxt_http_fields_hash(
    &hash, nxt_http_test_bench_fields, nxt_nitems(nxt_http_test_bench_fields));
  if (rc != NXT_OK) {
    return NXT_ERROR;
  }

  if (nxt_http_parse_test_bench(
        thr, &nxt_http_test_simple_request, &hash, "simple", 1000000) !=
      NXT_OK) {
    return NXT_ERROR;
  }

  if (nxt_http_parse_test_bench(
        thr, &nxt_http_test_big_request, &hash, "big", 100000) != NXT_OK) {
    return NXT_ERROR;
  }
