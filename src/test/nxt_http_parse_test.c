...
static nxt_http_parse_test_case_t nxt_http_test_cases[] = {
    ...
    {nxt_string("GET / HTTP/2.0\r\n"), NXT_HTTP_PARSE_UNSUPPORTED_VERSION, NULL,
        {NULL}},
    {nxt_string("GET /. HTTP/1.0\r\n\r\n"), NXT_DONE,
        &nxt_http_parse_test_request_line,
        {.request_line = {nxt_string("GET"), nxt_string("/."), nxt_null_string,
    ...
    {nxt_string("GET / HTTP/1.1\r\n"
                ...
                "X-Good-Header: value\r\n"
                "!#$%&'*+.^_`|~: skipped\r\n\r\n"),
        NXT_DONE, &nxt_http_parse_test_fields, {.fields = {NXT_OK, 1}}},
    {nxt_string("GET / HTTP/1.1\r\n"
                "X-Good-Header: value\r\n"
                "X-Unknown-Header: value\r\n"
                "X-Bad-Header: value\r\n\r\n"),
        NXT_DONE, &nxt_http_parse_test_fields, {.fields = {NXT_ERROR, 1}}},
    {nxt_string("GET / HTTP/1.1\r\n"
                "!#$%&'*+.^_`|~: allowed\r\n\r\n"),
...
nxt_http_parse_test(nxt_thread_t *thr) {
    ...
    colls = nxt_http_fields_hash_collisions(&hash, nxt_http_test_bench_fields,
        nxt_nitems(nxt_http_test_bench_fields), 0);

    nxt_memzero(&hash, sizeof(nxt_lvlhsh_t));

    lvl_colls = nxt_http_fields_hash_collisions(&hash,
        nxt_http_test_bench_fields, nxt_nitems(nxt_http_test_bench_fields), 1);

    nxt_log_error(NXT_LOG_NOTICE, thr->log,
        "http parse test hash collisions %ui out of %uz, level: %ui", colls,
...
nxt_http_parse_test_bench(nxt_thread_t *thr, nxt_str_t *request,
    ...
    nxt_log_error(NXT_LOG_NOTICE, thr->log,
        "http parse %s request bench started: %uz bytes, %ui runs", name,
        request->length, n);
    ...
    for (i = 0; nxt_fast_path(i < n); i++) {
        ...
        if (nxt_slow_path(
                nxt_http_fields_process(rp.fields, hash, NULL) != NXT_OK)) {
            nxt_log_alert(thr->log,
                "http parse %s request bench failed "
