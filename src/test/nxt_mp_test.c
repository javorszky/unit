            ...
            size_t max_size)
{
  ...
  nxt_log_error(
    NXT_LOG_NOTICE, thr->log, "mem pool test started, max:%uz", max_size);
  ...
  valid =
    nxt_mp_test_sizes(cluster_size, page_alignment, page_size, min_chunk_size);
  if (!valid) {
    return NXT_ERROR;
  }
  ...
  for (i = 0; i < runs; i++) {
    ...
    for (n = 0; n < nblocks; n++) {
      ...
      if (blocks[n] == NULL) {
        nxt_log_error(
          NXT_LOG_NOTICE, thr->log, "mem pool test failed: %uz", total);
        return NXT_ERROR;
      }
    }
