...
nxt_rbtree1_test(nxt_thread_t* thr, nxt_uint_t n)
{
  ...
  for (i = 0; i < n; i++) {
    if (nxt_rbtree1_test_find(&tree, &nodes[i]) != &nodes[i]) {
      nxt_log_alert(
        thr->log, "rbtree1 test failed: %08XD not found", nodes[i].key);
      goto fail;
    }
  }
  ...
  while (nxt_rbtree1_is_there_successor(&tree, node)) {
    ...
    if (keys[i] != node->key) {
      nxt_log_alert(thr->log,
                    "rbtree1 test failed: %i: %08XD %08XD",
                    i,
                    keys[i],
                    node->key);
      goto fail;
...
nxt_rbtree1_mb_start(nxt_thread_t* thr)
{
  ...
  nxt_log_error(NXT_LOG_NOTICE,
                thr->log,
                "rbtree1 mb cached insert: %L cycles",
                end - start);
...
nxt_rbtree1_mb_insert(nxt_thread_t* thr)
{
  ...
  nxt_log_error(
    NXT_LOG_NOTICE, thr->log, "rbtree1 mb insert: %L cycles", end - start);
}
...
nxt_rbtree1_mb_delete(nxt_thread_t* thr)
{
  ...
  nxt_log_error(
    NXT_LOG_NOTICE, thr->log, "rbtree1 mb delete: %L cycles", end - start);
