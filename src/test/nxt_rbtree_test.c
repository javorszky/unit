...
nxt_rbtree_test(nxt_thread_t* thr, nxt_uint_t n)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    if (node != (nxt_rbtree_node_t*)&items[i].node) {
      nxt_log_alert(
        thr->log, "rbtree test failed: %08XD not found", items[i].key);
      goto fail;
    }
  }
  ...
  while (nxt_rbtree_is_there_successor(&tree, node)) {
    ...
    if (keys[i] != item->key) {
      nxt_log_alert(
        thr->log, "rbtree test failed: %i: %08XD %08XD", i, keys[i], item->key);
      goto fail;
    }
...
nxt_rbtree_mb_start(nxt_thread_t* thr)
{
  ...
  nxt_log_error(NXT_LOG_NOTICE,
                thr->log,
                "rbtree mb cached insert: %L cycles",
                end - start);
...
nxt_rbtree_mb_insert(nxt_thread_t* thr)
{
  ...
  nxt_log_error(
    NXT_LOG_NOTICE, thr->log, "rbtree mb insert: %L cycles", end - start);
}
...
nxt_rbtree_mb_delete(nxt_thread_t* thr)
{
  ...
  nxt_log_error(
    NXT_LOG_NOTICE, thr->log, "rbtree mb delete: %L cycles", end - start);
