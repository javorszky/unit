...
nxt_perl_psgi_call_var_application(PerlInterpreter* my_perl,
                                   ...
                                   nxt_unit_request_info_t* req);
static SV*
nxt_perl_psgi_call_method(PerlInterpreter* my_perl,
                          SV* obj,
                          const char* method,
                          nxt_unit_request_info_t* req);
...
nxt_perl_psgi_env_create(PerlInterpreter* my_perl,
                         nxt_unit_request_info_t* req);
nxt_inline int
nxt_perl_psgi_add_sptr(PerlInterpreter* my_perl,
                       HV* hash_env,
                       const char* name,
                       uint32_t name_len,
                       ...
                       uint32_t len);
nxt_inline int
nxt_perl_psgi_add_str(PerlInterpreter* my_perl,
                      HV* hash_env,
                      const char* name,
                      uint32_t name_len,
                      const char* str,
                      uint32_t len);
nxt_inline int
nxt_perl_psgi_add_value(PerlInterpreter* my_perl,
                        HV* hash_env,
                        const char* name,
                        uint32_t name_len,
                        void* value);
...
static int
nxt_perl_psgi_result_head(PerlInterpreter* my_perl,
                          SV* sv_head,
                          nxt_unit_request_info_t* req,
                          uint16_t status);
static int
nxt_perl_psgi_result_body(PerlInterpreter* my_perl,
                          SV* result,
                          nxt_unit_request_info_t* req);
static int
nxt_perl_psgi_result_body_ref(PerlInterpreter* my_perl,
                              SV* sv_body,
                              nxt_unit_request_info_t* req);
static int
nxt_perl_psgi_result_body_fh(PerlInterpreter* my_perl,
                             SV* sv_body,
                             nxt_unit_request_info_t* req);
static ssize_t
nxt_perl_psgi_io_read(nxt_unit_read_info_t* read_info, void* dst, size_t size);
static int
nxt_perl_psgi_result_array(PerlInterpreter* my_perl,
                           SV* result,
                           nxt_unit_request_info_t* req);
static void
nxt_perl_psgi_result_cb(PerlInterpreter* my_perl,
                        SV* result,
                        nxt_unit_request_info_t* req);
...
nxt_perl_psgi_xs_init(pTHX)
{
  ...
  */
  ...
  nxt_perl_psgi_write = newXS(
    "NGINX::Unit::Sandbox::write", XS_NGINX__Unit__Sandbox_write, __FILE__);

  nxt_perl_psgi_close = newXS(
    "NGINX::Unit::Sandbox::close", XS_NGINX__Unit__Sandbox_close, __FILE__);
                                   ...
                                   nxt_unit_request_info_t* req)
{
  ...
  if (SvTRUE(ERRSV)) {
    nxt_unit_req_error(
      req, "PSGI: Failed to run Perl Application: \n%s", SvPV_nolen(ERRSV));
  }
...
nxt_perl_psgi_ctx_init(const char* script, nxt_perl_psgi_ctx_t* pctx)
{
  ...
  if (SvTRUE(ERRSV)) {
    nxt_unit_alert(
      NULL, "PSGI: Failed to parse script: %s\n%s", script, SvPV_nolen(ERRSV));
    goto fail;
  }
...
#define NL(S) (S), sizeof(S) - 1
  ...
  RC(nxt_perl_psgi_add_str(my_perl,
                           hash_env,
                           NL("SERVER_SOFTWARE"),
                           (char*)nxt_server.start,
                           nxt_server.length));

  RC(nxt_perl_psgi_add_sptr(
    my_perl, hash_env, NL("REQUEST_METHOD"), &r->method, r->method_length));
  RC(nxt_perl_psgi_add_sptr(
    my_perl, hash_env, NL("REQUEST_URI"), &r->target, r->target_length));
  RC(nxt_perl_psgi_add_sptr(
    my_perl, hash_env, NL("PATH_INFO"), &r->path, r->path_length));
  ...
  RC(nxt_perl_psgi_add_value(
    my_perl, hash_env, NL("psgi.version"), newRV_noinc((SV*)array_version)));

  RC(
    nxt_perl_psgi_add_value(my_perl,
                            hash_env,
                            NL("psgi.url_scheme"),
  ...
  RC(nxt_perl_psgi_add_value(
    my_perl, hash_env, NL("psgi.input"), SvREFCNT_inc(pctx->arg_input.io)));

  RC(nxt_perl_psgi_io_init(my_perl, &pctx->arg_error, "w", req));
  RC(nxt_perl_psgi_add_value(
    my_perl, hash_env, NL("psgi.errors"), SvREFCNT_inc(pctx->arg_error.io)));

  RC(nxt_perl_psgi_add_value(my_perl,
                             hash_env,
                             NL("psgi.multithread"),
                             nxt_perl_psgi_ctxs != NULL ? &PL_sv_yes
                                                        : &PL_sv_no));
  RC(nxt_perl_psgi_add_value(
    my_perl, hash_env, NL("psgi.multiprocess"), &PL_sv_yes));
  RC(
    nxt_perl_psgi_add_value(my_perl, hash_env, NL("psgi.run_once"), &PL_sv_no));
  RC(nxt_perl_psgi_add_value(
    my_perl, hash_env, NL("psgi.nonblocking"), &PL_sv_no));
  RC(nxt_perl_psgi_add_value(
    my_perl, hash_env, NL("psgi.streaming"), &PL_sv_yes));

  RC(nxt_perl_psgi_add_sptr(
    my_perl, hash_env, NL("QUERY_STRING"), &r->query, r->query_length));
  RC(nxt_perl_psgi_add_sptr(
    my_perl, hash_env, NL("SERVER_PROTOCOL"), &r->version, r->version_length));
  RC(nxt_perl_psgi_add_sptr(
    my_perl, hash_env, NL("REMOTE_ADDR"), &r->remote, r->remote_length));
  RC(nxt_perl_psgi_add_sptr(my_perl,
                            hash_env,
                            NL("SERVER_ADDR"),
                            &r->local_addr,
                            r->local_addr_length));
  ...
  if (r->content_length_field != NXT_UNIT_NONE_FIELD) {
    ...
    RC(nxt_perl_psgi_add_sptr(
      my_perl, hash_env, NL("CONTENT_LENGTH"), &f->value, f->value_length));
  }
  ...
  if (r->content_type_field != NXT_UNIT_NONE_FIELD) {
    ...
    RC(nxt_perl_psgi_add_sptr(
      my_perl, hash_env, NL("CONTENT_TYPE"), &f->value, f->value_length));
  }
...
nxt_perl_psgi_add_sptr(PerlInterpreter* my_perl,
                       ...
                       uint32_t len)
{
  return nxt_perl_psgi_add_str(
    my_perl, hash_env, name, name_len, nxt_unit_sptr_get(sptr), len);
}
                        ...
                        nxt_unit_request_info_t* req)
{
  ...
  if (SvTRUE(ERRSV)) {
    nxt_unit_error(
      NULL, "PSGI: Failed to execute result callback: \n%s", SvPV_nolen(ERRSV));
...
nxt_perl_psgi_ready_handler(nxt_unit_ctx_t* ctx)
{
  ...
  for (i = 0; i < c->threads - 1; i++) {
    ...
    res = pthread_create(&pctx->thread,
                         nxt_perl_psgi_thread_attr,
                         nxt_perl_psgi_thread_func,
                         pctx);
...
nxt_perl_psgi_init_threads(nxt_perl_app_conf_t* c)
{
  ...
  if (c->thread_stack_size > 0) {
    ...
    if (nxt_slow_path(rc != 0)) {
      nxt_unit_alert(
        NULL, "thread attr init failed: %s (%d)", strerror(rc), rc);
    ...
    if (nxt_slow_path(rc != 0)) {
      nxt_unit_alert(
        NULL, "thread attr set stack size failed: %s (%d)", strerror(rc), rc);
