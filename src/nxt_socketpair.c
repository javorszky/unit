...
nxt_socketpair_create(nxt_task_t* task, nxt_socket_t* pair)
{
  ...
  if (nxt_slow_path(setsockopt(pair[0],
                               SOL_SOCKET,
                               SO_PASSCRED,
                               &enable_creds,
  ...
  if (nxt_slow_path(setsockopt(pair[1],
                               SOL_SOCKET,
                               SO_PASSCRED,
                               &enable_creds,
                               sizeof(enable_creds)) == -1)) {
...
ssize_t
nxt_socketpair_send(nxt_fd_event_t* ev,
                    nxt_fd_t* fd,
                    nxt_iobuf_t* iob,
                    nxt_uint_t niob)
{
...
ssize_t
nxt_socketpair_recv(nxt_fd_event_t* ev,
                    nxt_iobuf_t* iob,
                    nxt_uint_t niob,
                    void* oob)
{
