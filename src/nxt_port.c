...
void
nxt_port_enable(nxt_task_t* task,
                nxt_port_t* port,
...
nxt_port_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  if (nxt_fast_path(msg->port_msg.type < NXT_PORT_MSG_MAX)) {
    ...
    nxt_debug(task,
              "port %d: message type:%uD fds:%d,%d",
              msg->port->socket.fd,
              msg->port_msg.type,
              msg->fd[0],
              msg->fd[1]);
  ...
  nxt_alert(task,
            "port %d: unknown message type:%uD",
            msg->port->socket.fd,
            msg->port_msg.type);
}
...
nxt_inline void
nxt_port_send_new_port(nxt_task_t* task,
                       nxt_runtime_t* rt,
                       nxt_port_t* new_port,
                       uint32_t stream)
{
  ...
  nxt_debug(
    task, "new port %d for process %PI", new_port->pair[1], new_port->pid);
...
nxt_int_t
nxt_port_send_port(nxt_task_t* task,
                   nxt_port_t* port,
                   nxt_port_t* new_port,
                   uint32_t stream)
{
  ...
  b = nxt_buf_mem_ts_alloc(
    task, task->thread->engine->mem_pool, sizeof(nxt_port_data_t));
  ...
  return nxt_port_socket_write2(task,
                                port,
...
nxt_port_new_port_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  nxt_debug(task,
            "new port %d received for process %PI:%d",
            msg->fd[0],
            new_port_msg->pid,
            new_port_msg->id);
  ...
  if (port != NULL) {
    nxt_debug(
      task, "port %PI:%d already exists", new_port_msg->pid, new_port_msg->id);
...
nxt_port_mmap_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  if (nxt_slow_path(process == NULL)) {
    nxt_log(
      task, NXT_LOG_WARN, "failed to get process #%PI", msg->port_msg.pid);
...
void
nxt_port_change_log_file(nxt_task_t* task,
                         nxt_runtime_t* rt,
                         nxt_uint_t slot,
                         nxt_fd_t fd)
{
  ...
  nxt_runtime_process_each(rt, process)
  {
    ...
    b =
      nxt_buf_mem_alloc(task->thread->engine->mem_pool, sizeof(nxt_uint_t), 0);
    if (nxt_slow_path(b == NULL)) {
      continue;
    }
    ...
    (void)nxt_port_socket_write(
      task, port, NXT_PORT_MSG_CHANGE_FILE, fd, 0, 0, b);
  }
  nxt_runtime_process_loop;
}
...
nxt_port_remove_notify_others(nxt_task_t* task, nxt_process_t* process)
{
  ...
  nxt_runtime_process_each(rt, p)
  {
    ...
    nxt_port_socket_write(
      task, port, NXT_PORT_MSG_REMOVE_PID, -1, process->stream, 0, buf);
  }
  nxt_runtime_process_loop;
}
