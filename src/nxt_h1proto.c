...
nxt_h1p_init(nxt_task_t *task) {
    ...
    if (nxt_fast_path(ret == NXT_OK)) {
        ret = nxt_http_fields_hash(&nxt_h1p_peer_fields_hash,
            nxt_h1p_peer_fields, nxt_nitems(nxt_h1p_peer_fields));
    }
...
static const nxt_conn_state_t nxt_h1p_peer_header_body_send_state nxt_aligned(
    64)
    = {
        .ready_handler = nxt_h1p_peer_header_sent,
        .error_handler = nxt_h1p_peer_error,
...
static const nxt_conn_state_t nxt_h1p_peer_header_read_timer_state nxt_aligned(
    64)
    = {
        .ready_handler = nxt_h1p_peer_header_read_done,
        .close_handler = nxt_h1p_peer_closed,
...
nxt_h1p_peer_header_parse(nxt_http_peer_t *peer, nxt_buf_mem_t *bm) {
    ...
    if (peer->status < 0) {
        ...
        if (nxt_slow_path(
                memcmp(p, "HTTP/1.", 7) != 0 || (p[7] != '0' && p[7] != '1'))) {
            return NXT_ERROR;
        }
