...
static nxt_int_t
nxt_h1p_header_process(nxt_task_t* task,
                       nxt_h1proto_t* h1p,
                       nxt_http_request_t* r);
static nxt_int_t
nxt_h1p_header_buffer_test(nxt_task_t* task,
                           nxt_h1proto_t* h1p,
                           nxt_conn_t* c,
...
static void
nxt_h1p_request_header_send(nxt_task_t* task,
                            nxt_http_request_t* r,
...
static void
nxt_h1p_request_discard(nxt_task_t* task,
                        nxt_http_request_t* r,
                        nxt_buf_t* last);
static void
...
nxt_inline void
nxt_h1p_request_error(nxt_task_t* task,
                      nxt_h1proto_t* h1p,
                      nxt_http_request_t* r);
static void
nxt_h1p_request_close(nxt_task_t* task,
...
static void
nxt_h1p_peer_body_process(nxt_task_t* task,
...
static nxt_int_t
nxt_h1p_peer_transfer_encoding(void* ctx,
...
nxt_h1p_init(nxt_task_t* task)
{
  ...
  ret = nxt_http_fields_hash(
    &nxt_h1p_fields_hash, nxt_h1p_fields, nxt_nitems(nxt_h1p_fields));
  ...
  switch (ret) {
    ...
    case NXT_DONE:
      ...
      if (nxt_slow_path(r->log_route)) {
        nxt_log(
          task, NXT_LOG_NOTICE, "http request line \"%V\"", &r->request_line);
      }
                       ...
                       nxt_http_request_t* r)
{
  ...
  if (h1p->connection_upgrade && h1p->upgrade_websocket) {
    ...
    if (nxt_slow_path(h1p->websocket_version_ok == 0)) {
      nxt_log(
        task, NXT_LOG_INFO, "h1p upgrade: bad or absent websocket version");
...
nxt_h1p_request_body_read(nxt_task_t* task, nxt_http_request_t* r)
{
  ...
  if (body_length > body_buffer_size) {
    ...
    memcpy(tmp_name.start + tmp_path->length,
           tmp_name_pattern.start,
                            ...
                            void* data)
{
  ...
  if (body_handler != NULL) {
     ...
     */
    nxt_work_queue_add(
      &task->thread->engine->fast_work_queue, body_handler, task, r, data);
...
nxt_h1p_idle_response(nxt_task_t* task, nxt_conn_t* c)
{
  ...
  p = nxt_cpymem(
    out->mem.free, NXT_H1P_IDLE_TIMEOUT, nxt_length(NXT_H1P_IDLE_TIMEOUT));
...
nxt_h1p_peer_request_target(nxt_http_request_t* r, nxt_str_t* target)
{
  ...
  if (r->quoted_target) {
    p = (u_char*)nxt_encode_complex_uri(
      target->start, r->path->start, r->path->length);
