...
static int
nxt_ruby_rack_result_headers(nxt_unit_request_info_t* req,
                             VALUE result,
                             nxt_int_t status);
static int
...
static void
nxt_ruby_exception_log(nxt_unit_request_info_t* req,
                       uint32_t level,
                       const char* desc);
...
nxt_ruby_init_strings(void)
{
  ...
  while (pstr->string.start != NULL) {
    ...
    if (nxt_slow_path(v == Qnil)) {
      nxt_unit_alert(NULL,
                     "Ruby: failed to create const string '%.*s'",
...
nxt_ruby_hook_procs_load(VALUE path)
{
  ...
  rb_define_module_function(
    module, "on_worker_boot", &nxt_ruby_hook_register, 0);
  rb_define_module_function(
    module, "on_worker_shutdown", &nxt_ruby_hook_register, 0);
  rb_define_module_function(
    module, "on_thread_boot", &nxt_ruby_hook_register, 0);
  rb_define_module_function(
    module, "on_thread_shutdown", &nxt_ruby_hook_register, 0);

  file = rb_const_get(rb_cObject, rb_intern("File"));
  file_obj = rb_funcall(file, rb_intern("read"), 1, path);

  return rb_funcall(
    module, rb_intern("module_eval"), 3, file_obj, path, INT2NUM(1));
}
...
nxt_ruby_start(nxt_task_t* task, nxt_process_data_t* data)
{
  ...
  if (nxt_slow_path(res == Qnil || state != 0)) {
    nxt_ruby_exception_log(
      NULL, NXT_LOG_ALERT, "Failed to init basic variables");
    return NXT_ERROR;
  }
  ...
  if (nxt_ruby_hook_procs != Qnil) {
    ...
    if (nxt_slow_path(state != 0)) {
      nxt_ruby_exception_log(
        NULL, NXT_LOG_ERR, "Failed to call on_worker_boot()");
      return NXT_ERROR;
    }
  }
  ...
  if (nxt_slow_path(ruby_ctx.env == Qnil || state != 0)) {
    nxt_ruby_exception_log(
      NULL, NXT_LOG_ALERT, "Failed to create 'environ' variable");
    goto fail;
  }
  ...
  if (nxt_ruby_hook_procs != Qnil) {
    ...
    if (nxt_slow_path(state != 0)) {
      nxt_ruby_exception_log(
        NULL, NXT_LOG_ERR, "Failed to call on_thread_boot()");
    }
  }

  rc = (intptr_t)rb_thread_call_without_gvl2(
    nxt_ruby_unit_run, unit_ctx, nxt_ruby_ubf, unit_ctx);

  if (nxt_ruby_hook_procs != Qnil) {
    rb_protect(nxt_ruby_hook_call, nxt_rb_on_thread_shutdown, &state);
    if (nxt_slow_path(state != 0)) {
      nxt_ruby_exception_log(
        NULL, NXT_LOG_ERR, "Failed to call on_thread_shutdown()");
    }
  }
  ...
  if (nxt_ruby_hook_procs != Qnil) {
    ...
    if (nxt_slow_path(state != 0)) {
      nxt_ruby_exception_log(
        NULL, NXT_LOG_ERR, "Failed to call on_worker_shutdown()");
    }
  }
...
nxt_ruby_init_basic(VALUE arg)
{
  ...
  rb_funcall(
    rb_cObject, rb_intern("require"), 1, rb_str_new2("enc/trans/transdb"));
...
nxt_ruby_rack_init(nxt_ruby_rack_init_t* rack_init)
{
  ...
  if (nxt_slow_path(state != 0)) {
    nxt_ruby_exception_log(
      NULL, NXT_LOG_ALERT, "Failed to require 'rubygems' package");
    return Qnil;
  }
  ...
  if (state != 0) {
    ...
    if (rb_obj_is_kind_of(err, rb_eLoadError) == Qfalse) {
      nxt_ruby_exception_log(
        NULL, NXT_LOG_ALERT, "Failed to require 'bundler/setup' package");
      return Qnil;
    }
  ...
  if (nxt_slow_path(state != 0)) {
    nxt_ruby_exception_log(
      NULL, NXT_LOG_ALERT, "Failed to require 'rack' package");
    return Qnil;
  }

  rackup =
    rb_protect(nxt_ruby_rack_parse_script, (VALUE)(uintptr_t)rack_init, &state);
...
{
  return rb_funcall(
    rb_cObject, rb_intern("require"), 1, rb_str_new2("rubygems"));
}
...
{
  return rb_funcall(
    rb_cObject, rb_intern("require"), 1, rb_str_new2("bundler/setup"));
}
...
nxt_ruby_read_request(nxt_unit_request_info_t* req, VALUE hash_env)
{
  ...
  nxt_ruby_add_sptr(
    hash_env, nxt_rb_request_uri_str, &r->target, r->target_length);
  nxt_ruby_add_sptr(hash_env, nxt_rb_path_info_str, &r->path, r->path_length);
  nxt_ruby_add_sptr(
    hash_env, nxt_rb_query_string_str, &r->query, r->query_length);
  nxt_ruby_add_sptr(
    hash_env, nxt_rb_server_protocol_str, &r->version, r->version_length);
  nxt_ruby_add_sptr(
    hash_env, nxt_rb_remote_addr_str, &r->remote, r->remote_length);
  nxt_ruby_add_sptr(
    hash_env, nxt_rb_server_addr_str, &r->local_addr, r->local_addr_length);
  nxt_ruby_add_sptr(
    hash_env, nxt_rb_server_name_str, &r->server_name, r->server_name_length);

  rb_hash_aset(hash_env, nxt_rb_server_port_str, nxt_rb_80_str);

  rb_hash_aset(hash_env,
               nxt_rb_rack_url_scheme_str,
  ...
  if (r->content_length_field != NXT_UNIT_NONE_FIELD) {
    ...
    nxt_ruby_add_sptr(
      hash_env, nxt_rb_content_length_str, &f->value, f->value_length);
  }
  ...
  if (r->content_type_field != NXT_UNIT_NONE_FIELD) {
    ...
    nxt_ruby_add_sptr(
      hash_env, nxt_rb_content_type_str, &f->value, f->value_length);
  }
                             ...
                             nxt_int_t status)
{
  ...
  rc =
    nxt_unit_response_init(req, status, headers_info.fields, headers_info.size);
  if (nxt_slow_path(rc != NXT_UNIT_OK)) {
    return rc;
  }
...
nxt_ruby_hash_add(VALUE r_key, VALUE r_value, VALUE arg)
{
  ...
  if (value <= value_end) {
    *rc = nxt_unit_response_add_field(
      headers_info->req, RSTRING_PTR(r_key), key_len, value, value_end - value);
    if (nxt_slow_path(*rc != NXT_UNIT_OK)) {
      goto fail;
    }
  }
                                     ...
                                     VALUE filepath)
{
  ...
  rc = (intptr_t)rb_thread_call_without_gvl(
    nxt_ruby_response_write_cb, &ri, nxt_ruby_ubf, req->ctx);
                               ...
                               VALUE blockarg)
{
  ...
  (void)rb_thread_call_without_gvl(
    nxt_ruby_response_write, (void*)(uintptr_t)&wi, nxt_ruby_ubf, wi.req->ctx);
...
nxt_ruby_response_write(void* data)
{
  ...
  rc = nxt_unit_response_write(
    wi->req, RSTRING_PTR(wi->body), RSTRING_LEN(wi->body));
  if (nxt_slow_path(rc != NXT_UNIT_OK)) {
                       ...
                       const char* desc)
{
  ...
  if (RARRAY_LEN(ary) == 0) {
    nxt_unit_req_log(
      req, level, "Ruby: %s (%s)", RSTRING_PTR(msg), RSTRING_PTR(eclass));

    return;
  }

  nxt_unit_req_log(req,
                   level,
                   "Ruby: %s: %s (%s)",
