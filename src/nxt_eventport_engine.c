...
static nxt_int_t
nxt_eventport_create(nxt_event_engine_t* engine,
                     nxt_uint_t mchanges,
                     nxt_uint_t mevents);
static void
...
static void
nxt_eventport_enable_event(nxt_event_engine_t* engine,
                           nxt_fd_event_t* ev,
                           nxt_uint_t events);
static void
                           ...
                           nxt_uint_t events)
{
  ...
  nxt_debug(ev->task,
            "port %d set event: fd:%d ev:%04XD u:%p",
            engine->u.eventport.fd,
            ev->fd,
            events,
            ev);
...
nxt_eventport_disable_event(nxt_event_engine_t* engine, nxt_fd_event_t* ev)
{
  ...
  nxt_debug(
    ev->task, "port %d disable event : fd:%d", engine->u.eventport.fd, ev->fd);
...
nxt_eventport_commit_changes(nxt_event_engine_t* engine)
{
  ...
  do {
    ...
    if (change->events != 0) {
      nxt_debug(ev->task,
                "port_associate(%d): fd:%d ev:%04XD u:%p",
                port,
                ev->fd,
                change->events,
                ev);
...
nxt_eventport_poll(nxt_event_engine_t* engine, nxt_msec_t timeout)
{
  ...
  nevents = 1;
  n = port_getn(engine->u.eventport.fd,
                engine->u.eventport.events,
                engine->u.eventport.mevents,
                &nevents,
                tp);
  ...
  nxt_debug(
    &engine->task, "port_getn(%d) events: %d", engine->u.eventport.fd, nevents);
  ...
  for (i = 0; i < nevents; i++) {
    ...
    switch (event->portev_source) {
      ...
      case PORT_SOURCE_FD:
        ...
        nxt_debug(ev->task,
                  "eventport: fd:%d ev:%04Xd u:%p rd:%d wr:%d",
                  event->portev_object,
                  events,
                  ev,
                  ev->read,
