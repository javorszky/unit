...
static nxt_int_t
nxt_java_setup(nxt_task_t* task,
...
nxt_java_websocket_handler(nxt_unit_websocket_frame_t* ws)
{
  ...
  if (b != NULL) {
    ...
    if (jbuf != NULL) {
      nxt_java_Request_websocket(
        env, data->jreq, jbuf, ws->header->opcode, ws->header->fin);
...
nxt_java_ready_handler(nxt_unit_ctx_t* ctx)
{
  ...
  for (i = 0; i < c->threads - 1; i++) {
    res = pthread_create(
      &nxt_java_threads[i], nxt_java_thread_attr, nxt_java_thread_func, ctx);
...
nxt_java_join_threads(nxt_unit_ctx_t* ctx, nxt_java_app_conf_t* c)
{
  ...
  for (i = 0; i < c->threads - 1; i++) {
    ...
    if (nxt_fast_path(res == 0)) {
      ...
      nxt_unit_alert(
        ctx, "thread #%d join failed: %s (%d)", (int)i, strerror(res), res);
    }
  }
