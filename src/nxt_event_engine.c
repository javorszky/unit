...
nxt_event_engine_queue_pop(nxt_event_engine_t* engine,
                           nxt_task_t** task,
                           void** obj,
                           void** data);
...
nxt_event_engine_signal_pipe_error(nxt_task_t* task, void* obj, void* data)
{
  ...
  nxt_alert(
    task, "engine pipe(%FD:%FD) event error", pipe->fds[0], pipe->fds[1]);
...
void*
nxt_event_engine_mem_alloc(nxt_event_engine_t* engine,
                           uint8_t* hint,
                           size_t size)
{
  uint32_t n;
...
void
nxt_event_engine_mem_free(nxt_event_engine_t* engine,
                          uint8_t hint,
                          void* p,
                          size_t size)
{
  ...
  if (nxt_slow_path(n == NXT_EVENT_ENGINE_NO_MEM_HINT)) {
    ...
    if (size != 0) {
      ...
      nxt_alert(
        &engine->task, "event engine mem free(%p, %z) not found", p, size);
    }
