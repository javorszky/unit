...
#endif
static void*
nxt_mp_alloc_large(nxt_mp_t* mp,
                   size_t alignment,
                   size_t size,
                   nxt_bool_t freeable);
static intptr_t
...
nxt_mp_t*
nxt_mp_create(size_t cluster_size,
              ...
              size_t min_chunk_size)
{
  ...
  nxt_debug_alloc("mp %p create(%uz, %uz, %uz, %uz)",
                  mp,
                  cluster_size,
                  page_alignment,
                  page_size,
...
nxt_bool_t
nxt_mp_test_sizes(size_t cluster_size,
                  size_t page_alignment,
                  size_t page_size,
                  size_t min_chunk_size)
{
  ...
  valid = nxt_expect(1,
                     (nxt_is_power_of_two(page_alignment) &&
  ...
  valid = nxt_expect(
    1,
    (page_size >= 64 && page_size >= page_alignment &&
...
nxt_mp_alloc_small(nxt_mp_t* mp, size_t size)
{
  ...
  nxt_debug_alloc(
    "mp %p chunk:%uz alloc: %p", mp, page->size << mp->chunk_size_shift, p);
