...
nxt_conn_accept(nxt_task_t* task, nxt_listen_event_t* lev, nxt_conn_t* c)
{
  ...
  if (lev->listen->read_after_accept) {
    ...
    nxt_work_queue_add(
      c->read_work_queue, lev->listen->handler, &c->task, c, lev);

  } else {
    nxt_work_queue_add(
      c->write_work_queue, lev->listen->handler, &c->task, c, lev);
  }
  ...
  if (next != NULL && lev->socket.read_ready) {
    nxt_work_queue_add(
      lev->socket.read_work_queue, lev->accept, task, lev, next);
  }
}
...
nxt_conn_accept_close_idle(nxt_task_t* task, nxt_listen_event_t* lev)
{
  ...
  nxt_work_queue_add(&engine->close_work_queue,
                     nxt_conn_accept_close_idle_handler,
                     task,
                     NULL,
                     NULL);
...
nxt_conn_accept_close_idle_handler(nxt_task_t* task, void* obj, void* data)
{
  ...
  for (link = nxt_queue_last(idle); link != nxt_queue_head(idle); link = next) {
    ...
    nxt_debug(c->socket.task,
              "idle connection: %d rdy:%d",
              c->socket.fd,
              c->socket.read_ready);

    if (!c->socket.read_ready) {
      nxt_log_moderate(&nxt_idle_close_log_moderation,
                       NXT_LOG_INFO,
                       task->log,
                      ...
                      nxt_err_t err)
{
  ...
  switch (err) {
    ...
    case ECONNABORTED:
      nxt_log_moderate(&nxt_accept_log_moderation,
                       NXT_LOG_WARN,
                       task->log,
                       "%s(%d) failed %E",
                       accept_syscall,
