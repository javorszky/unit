...
ssize_t
nxt_hpux_event_conn_io_sendfile(nxt_event_conn_t* c,
                                nxt_buf_t* b,
                                size_t limit);

static ssize_t
nxt_sys_sendfile(int s,
                 int fd,
                 off_t offset,
                 size_t nbytes,
...
nxt_hpux_event_conn_io_sendfile(nxt_event_conn_t* c, nxt_buf_t* b, size_t limit)
{
  ...
  nxt_debug(c->socket.task,
            "sendfile(%d, %FD, @%O, %uz) hd:%ui tr:%ui",
            c->socket.fd,
            fb->file->fd,
            fb->file_pos,
            file_size,
  ...
  n = nxt_sys_sendfile(
    c->socket.fd, fb->file->fd, fb->file_pos, file_size, hdtrl, 0);
