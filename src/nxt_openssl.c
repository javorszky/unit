...
#endif
static nxt_int_t
nxt_openssl_server_init(nxt_task_t* task,
                        nxt_mp_t* mp,
                        nxt_tls_init_t* tls_init,
                        nxt_bool_t last);
static nxt_int_t
nxt_openssl_chain_file(nxt_task_t* task,
                       SSL_CTX* ctx,
                       nxt_tls_conf_t* conf,
                       nxt_mp_t* mp,
                       nxt_bool_t single);
...
static nxt_int_t
nxt_ssl_conf_commands(nxt_task_t* task,
                      SSL_CTX* ctx,
...
static nxt_int_t
nxt_tls_ticket_keys(nxt_task_t* task,
                    SSL_CTX* ctx,
                    nxt_tls_init_t* tls_init,
                    nxt_mp_t* mp);
static int
nxt_tls_ticket_key_callback(SSL* s,
                            unsigned char* name,
                            unsigned char* iv,
                            EVP_CIPHER_CTX* ectx,
                            HMAC_CTX* hctx,
...
static nxt_uint_t
nxt_openssl_cert_get_names(nxt_task_t* task,
                           X509* cert,
                           nxt_tls_conf_t* conf,
                           nxt_mp_t* mp);
static nxt_int_t
...
static nxt_int_t
nxt_openssl_bundle_hash_insert(nxt_task_t* task,
                               nxt_lvlhsh_t* lvlhsh,
...
static ssize_t
nxt_openssl_conn_io_send(nxt_task_t* task,
                         nxt_sendbuf_t* sb,
                         void* buf,
                         size_t size);
static void
...
static nxt_int_t
nxt_openssl_conn_test_error(nxt_task_t* task,
                            nxt_conn_t* c,
                            int ret,
                            nxt_err_t sys_err,
                            nxt_openssl_io_t io);
                        ...
                        nxt_bool_t last)
{
  ...
  if (nxt_openssl_chain_file(
        task, ctx, conf, mp, last && bundle->next == NULL) != NXT_OK) {
    goto fail;
  }
  /*
                       ...
                       nxt_bool_t single)
{
  ...
  if (ret != NXT_OK) {
    nxt_openssl_log_error(
      task, NXT_LOG_ALERT, "nxt_openssl_chain_file() failed");
  }
                            ...
                            int enc)
{
  ...
  if (enc == 1) {
    ...
    if (RAND_bytes(iv, EVP_CIPHER_iv_length(cipher)) != 1) {
      nxt_openssl_log_error(
        c->socket.task, NXT_LOG_ALERT, "RAND_bytes() failed");
      return -1;
    }
    ...
    if (EVP_EncryptInit_ex(ectx, cipher, NULL, ticket[0].aes_key, iv) != 1) {
      nxt_openssl_log_error(
        c->socket.task, NXT_LOG_ALERT, "EVP_EncryptInit_ex() failed");
      return -1;
    }
  ...
  found:
    ...
    nxt_debug(
      c->socket.task, "TLS session ticket decrypt, key number: \"%d\"", i);
    ...
    if (EVP_DecryptInit_ex(ectx, cipher, NULL, ticket[i].aes_key, iv) != 1) {
      nxt_openssl_log_error(
        c->socket.task, NXT_LOG_ALERT, "EVP_DecryptInit_ex() failed");
      return -1;
    }
  }
  ...
  if (HMAC_Init_ex(hctx, ticket[i].hmac_key, ticket[i].size, digest, NULL) !=
      1) {
    nxt_openssl_log_error(
      c->socket.task, NXT_LOG_ALERT, "HMAC_Init_ex() failed");
    return -1;
  }
                               ...
                               nxt_mp_t* mp)
{
  ...
  if (nxt_fast_path(ret == NXT_OK)) {
    nxt_debug(task,
              "name \"%V\" for certificate \"%V\" is inserted",
              &str,
              &item->bundle->name);
    return NXT_OK;
  }
  ...
  if (bundle != NULL) {
    ...
    if (bundle != conf->bundle) {
      if (SSL_set_SSL_CTX(s, bundle->ctx) == NULL) {
        nxt_openssl_log_error(
          c->socket.task, NXT_LOG_ALERT, "SSL_set_SSL_CTX() failed");
...
nxt_openssl_conn_init(nxt_task_t* task, nxt_tls_conf_t* conf, nxt_conn_t* c)
{
  ...
  if (ret == 0) {
    nxt_openssl_log_error(
      task, NXT_LOG_ALERT, "SSL_set_fd(%d) failed", c->socket.fd);
    goto fail;
  }
...
fail:
  ...
  nxt_work_queue_add(
    c->read_work_queue, c->read_state->error_handler, task, c, c->socket.data);
}
...
nxt_openssl_conn_handshake(nxt_task_t* task, void* obj, void* data)
{
  ...
  if (ret > 0) {
    ...
    switch (n) {
      ...
      default:
      case NXT_ERROR:
        nxt_openssl_conn_error(
          task, err, "SSL_do_handshake(%d) failed", c->socket.fd);
...
nxt_openssl_conn_io_recvbuf(nxt_conn_t* c, nxt_buf_t* b)
{
  ...
  n =
    nxt_openssl_conn_test_error(c->socket.task, c, ret, err, NXT_OPENSSL_READ);
  if (n == NXT_ERROR) {
    nxt_openssl_conn_error(c->socket.task,
                           err,
                         ...
                         size_t size)
{
  ...
  nxt_debug(
    task, "SSL_write(%d, %p, %uz): %d err:%d", sb->socket, buf, size, ret, err);
  ...
  if (n == NXT_ERROR) {
    ...
    nxt_openssl_conn_error(
      task, err, "SSL_write(%d, %p, %uz) failed", sb->socket, buf, size);
  }
...
nxt_openssl_conn_io_shutdown(nxt_task_t* task, void* obj, void* data)
{
  ...
  switch (n) {
    ...
    case NXT_ERROR:
      nxt_openssl_conn_error(
        task, err, "SSL_shutdown(%d) failed", c->socket.fd);
