...
nxt_int_t
nxt_listen_socket_create(nxt_task_t* task,
                         nxt_mp_t* mp,
                         nxt_listen_socket_t* ls)
{
  ...
  if (listen(s, ls->backlog) != 0) {
    nxt_alert(
      task, "listen(%d, %d) failed %E", s, ls->backlog, nxt_socket_errno);
    goto listen_fail;
  }
  ...
  if (orig_sa != NULL) {
    ...
    if (ret == 0) {
      nxt_alert(task,
                "connect(%d, %*s) socket already in use",
                ts,
                (size_t)orig_sa->length,
...
nxt_int_t
nxt_listen_socket_update(nxt_task_t* task,
                         nxt_listen_socket_t* ls,
...
nxt_listen_socket_log_handler(void* ctx, u_char* pos, u_char* end)
{
  ...
  return nxt_sprintf(pos,
                     end,
                     " while creating listening socket on %*s",
                     (size_t)sa->length,
