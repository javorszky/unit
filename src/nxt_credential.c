...
nxt_int_t
nxt_credential_get(nxt_task_t* task,
                   nxt_mp_t* mp,
                   nxt_credential_t* uc,
                   const char* group)
{
  ...
  nxt_debug(task,
            "about to get \"%s\" groups (uid:%d, base gid:%d)",
            uc->user,
            uc->uid,
            uc->base_gid);
...
nxt_credential_setgids(nxt_task_t* task, nxt_credential_t* uc)
{
  ...
  nxt_debug(task,
            "user cred set gids: base gid:%d, ngroups: %d",
            uc->base_gid,
            uc->ngroups);
