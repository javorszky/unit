            ...
            void* ctx)
{
  ...
  ret = njs_vm_external_create(
    vm, njs_value_arg(&opaque_value), nxt_js_proto_id, ctx, 0);
  if (nxt_slow_path(ret != NJS_OK)) {
    return NXT_ERROR;
  }
  ...
  for (i = 0; i < n; i++) {
    value = njs_vm_object_prop(
      vm, njs_value_arg(&opaque_value), &js_args[i], &arguments[i]);
  ...
  ret = njs_vm_invoke(
    vm, func, njs_value_arg(&arguments), n, njs_value_arg(&retval));
