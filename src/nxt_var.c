...
static nxt_str_t*
nxt_var_cache_value(nxt_task_t* task,
  ...
  while (p < end) {
    ...
    if (next == NULL) {
      nxt_sprintf(error,
                  ...
                  "Invalid variable at position %uz%Z",
                  p - str->start);
    ...
    if (part.start != NULL) {
      ...
      if (ref == NULL) {
        nxt_sprintf(
          error, error + NXT_MAX_ERROR_STR, "Unknown variable \"%V\"%Z", &part);
...
nxt_int_t
nxt_var_interpreter(nxt_task_t* task,
                    nxt_tstr_state_t* state,
                    nxt_var_cache_t* cache,
                    nxt_var_t* var,
                    nxt_str_t* str,
