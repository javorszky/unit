...
static nxt_int_t
nxt_epoll_edge_create(nxt_event_engine_t* engine,
                      nxt_uint_t mchanges,
                      nxt_uint_t mevents);
#endif
static nxt_int_t
nxt_epoll_level_create(nxt_event_engine_t* engine,
                       nxt_uint_t mchanges,
                       nxt_uint_t mevents);
static nxt_int_t
nxt_epoll_create(nxt_event_engine_t* engine,
                 nxt_uint_t mchanges,
                 nxt_uint_t mevents,
                 nxt_conn_io_t* io,
                 uint32_t mode);
...
static void
nxt_epoll_change(nxt_event_engine_t* engine,
                 nxt_fd_event_t* ev,
                 int op,
                 uint32_t events);
static void
...
nxt_epoll_edge_create(nxt_event_engine_t* engine,
                      ...
                      nxt_uint_t mevents)
{
  return nxt_epoll_create(
    engine, mchanges, mevents, &nxt_epoll_edge_conn_io, EPOLLET | EPOLLRDHUP);
}
...
nxt_epoll_enable(nxt_event_engine_t* engine, nxt_fd_event_t* ev)
{
  ...
  nxt_epoll_change(
    engine, ev, EPOLL_CTL_ADD, EPOLLIN | EPOLLOUT | engine->u.epoll.mode);
}
                 ...
                 uint32_t events)
{
  ...
  nxt_debug(ev->task,
            "epoll %d set event: fd:%d op:%d ev:%XD",
            engine->u.epoll.fd,
            ev->fd,
            op,
            events);
...
nxt_epoll_enable_post(nxt_event_engine_t* engine, nxt_work_handler_t handler)
{
  ...
  ret = epoll_ctl(
    engine->u.epoll.fd, EPOLL_CTL_ADD, engine->u.epoll.eventfd.fd, &ee);
...
nxt_epoll_eventfd_handler(nxt_task_t* task, void* obj, void* data)
{
  ...
  if (engine->u.epoll.neventfd++ >= 0xFFFFFFFE) {
    ...
    nxt_debug(
      task, "read(%d): %d events:%uL", engine->u.epoll.eventfd.fd, n, events);

    if (n != sizeof(uint64_t)) {
      nxt_alert(task,
                "read eventfd(%d) failed %E",
                engine->u.epoll.eventfd.fd,
                nxt_errno);
    }
  }
...
nxt_epoll_poll(nxt_event_engine_t* engine, nxt_msec_t timeout)
{
  ...
  nxt_debug(
    &engine->task, "epoll_wait(%d) timeout:%M", engine->u.epoll.fd, timeout);

  nevents = epoll_wait(engine->u.epoll.fd,
                       engine->u.epoll.events,
                       engine->u.epoll.mevents,
                       timeout);
  ...
  for (i = 0; i < nevents; i++) {
    ...
    nxt_debug(ev->task,
              "epoll: fd:%d ev:%04XD d:%p rd:%d wr:%d",
              ev->fd,
              events,
              ev,
              ev->read,
    ...
    if ((events & EPOLLIN) != 0) {
      ...
      if (ev->read != NXT_EVENT_BLOCKED) {
        ...
        nxt_work_queue_add(
          ev->read_work_queue, ev->read_handler, ev->task, ev, ev->data);
    ...
    if ((events & EPOLLOUT) != 0) {
      ...
      if (ev->write != NXT_EVENT_BLOCKED) {
        ...
        nxt_work_queue_add(
          ev->write_work_queue, ev->write_handler, ev->task, ev, ev->data);
...
nxt_epoll_edge_conn_connected(nxt_task_t* task, void* obj, void* data)
{
  ...
  if (!c->socket.epoll_error) {
    ...
    nxt_work_queue_add(
      c->write_work_queue, c->write_state->ready_handler, task, c, data);
    return;
  }
