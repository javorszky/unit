...
static nxt_int_t
nxt_discovery_module(nxt_task_t* task,
                     nxt_mp_t* mp,
                     nxt_array_t* modules,
                     const char* name);
static void
...
nxt_discovery_start(nxt_task_t* task, nxt_process_data_t* data)
{
  ...
  ret = nxt_port_socket_write(
    task, main_port, NXT_PORT_MSG_MODULES, -1, stream, discovery_port->id, b);
...
nxt_proto_setup(nxt_task_t* task, nxt_process_t* process)
{
  ...
  if (nxt_app == NULL) {
    nxt_debug(task,
              "application language module: %s \"%s\"",
              lang->version,
              lang->file);
...
nxt_proto_sigchld_handler(nxt_task_t* task, void* obj, void* data)
{
  ...
  nxt_debug(
    task, "proto sigchld handler signo:%d (%s)", (int)(uintptr_t)obj, data);
  ...
  for (;;) {
    ...
    if (WTERMSIG(status)) {
      if (rt->is_pid_isolated) {
        ...
        nxt_alert(task,
                  "app process %PI exited on signal %d%s",
                  pid,
                  WTERMSIG(status),
                  NXT_WCOREDUMP(status) ? " (core dumped)" : "");
      }
      ...
      if (rt->is_pid_isolated) {
        ...
        nxt_trace(task,
                  "app process %PI exited with code %d",
                  pid,
                  WEXITSTATUS(status));
      }
    }
...
nxt_app_module_load(nxt_task_t* task, const char* name)
{
  ...
  if (nxt_slow_path(app == NULL)) {
    err = dlerror();
    nxt_alert(task,
              "dlsym(\"%s\", \"nxt_app_module\") failed: \"%s\"",
              name,
              err != NULL ? err : "(null)");
...
nxt_int_t
nxt_unit_default_init(nxt_task_t* task,
...
nxt_proto_process_add(nxt_task_t* task, nxt_process_t* process)
{
  ...
  switch (nxt_lvlhsh_insert(&nxt_proto_processes, &lhq)) {
    ...
    default:
      nxt_alert(
        task, "process (isolated %PI) failed to add", process->isolated_pid);
      break;
  }
}
