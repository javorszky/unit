...
static int
nxt_unit_ctx_init(nxt_unit_impl_t* lib,
...
static int
nxt_unit_ready(nxt_unit_ctx_t* ctx,
               int ready_fd,
               uint32_t stream,
               int queue_fd);
static int
...
nxt_unit_mmap_get(nxt_unit_ctx_t* ctx,
...
static int
nxt_unit_get_outgoing_buf(nxt_unit_ctx_t* ctx,
                          nxt_unit_port_t* port,
                          uint32_t size,
                          uint32_t min_size,
                          nxt_unit_mmap_buf_t* mmap_buf,
...
static int
nxt_unit_check_rbuf_mmap(nxt_unit_ctx_t* ctx,
                         ...
                         nxt_unit_read_buf_t* rbuf);
static int
nxt_unit_mmap_read(nxt_unit_ctx_t* ctx,
                   nxt_unit_recv_msg_t* recv_msg,
                   nxt_unit_read_buf_t* rbuf);
static int
...
static int
nxt_unit_send_port(nxt_unit_ctx_t* ctx,
                   nxt_unit_port_t* dst,
                   nxt_unit_port_t* port,
                   int queue_fd);
...
static void
nxt_unit_remove_port(nxt_unit_impl_t* lib,
                     nxt_unit_ctx_t* ctx,
                     nxt_unit_port_id_t* port_id);
static nxt_unit_port_t*
...
static ssize_t
nxt_unit_port_send(nxt_unit_ctx_t* ctx,
                   nxt_unit_port_t* port,
                   const void* buf,
                   size_t buf_size,
                   const nxt_send_oob_t* oob);
static ssize_t
nxt_unit_sendmsg(nxt_unit_ctx_t* ctx,
                 int fd,
                 const void* buf,
                 size_t buf_size,
                 const nxt_send_oob_t* oob);
static int
nxt_unit_ctx_port_recv(nxt_unit_ctx_t* ctx,
                       nxt_unit_port_t* port,
                       nxt_unit_read_buf_t* rbuf);
nxt_inline void
nxt_unit_rbuf_cpy(nxt_unit_read_buf_t* dst, nxt_unit_read_buf_t* src);
static int
nxt_unit_shared_port_recv(nxt_unit_ctx_t* ctx,
                          nxt_unit_port_t* port,
                          nxt_unit_read_buf_t* rbuf);
static int
nxt_unit_port_recv(nxt_unit_ctx_t* ctx,
                   nxt_unit_port_t* port,
                   nxt_unit_read_buf_t* rbuf);
static int
nxt_unit_port_queue_recv(nxt_unit_port_t* port, nxt_unit_read_buf_t* rbuf);
static int
nxt_unit_app_queue_recv(nxt_unit_ctx_t* ctx,
                        nxt_unit_port_t* port,
                        nxt_unit_read_buf_t* rbuf);
nxt_inline int
...
static nxt_unit_port_t*
nxt_unit_port_hash_find(nxt_lvlhsh_t* port_hash,
                        nxt_unit_port_id_t* port_id,
                        int remove);
...
nxt_unit_init(nxt_unit_init_t* init)
{
  ...
  if (init->ready_port.id.pid != 0 && init->ready_stream != 0 &&
    ...
    nxt_unit_port_id_init(
      &router_port.id, router_port.id.pid, router_port.id.id);
    ...
    rc = nxt_unit_read_env(&ready_port,
                           &router_port,
                           &read_port,
                           &shared_port.in_fd,
                           &shared_queue_fd,
  ...
  if (nxt_slow_path(mem == MAP_FAILED)) {
    nxt_unit_alert(
      ctx, "mmap(%d) failed: %s (%d)", queue_fd, strerror(errno), errno);
  ...
  nxt_unit_port_id_init(
    &shared_port.id, read_port.id.pid, NXT_UNIT_SHARED_PORT_ID);
  ...
  if (nxt_slow_path(mem == MAP_FAILED)) {
    nxt_unit_alert(
      ctx, "mmap(%d) failed: %s (%d)", shared_queue_fd, strerror(errno), errno);
                  ...
                  uint32_t* request_limit)
{
  ...
  if (nxt_slow_path(unit_init == NULL)) {
    nxt_unit_alert(
      NULL, "%s is not in the current environment", NXT_UNIT_INIT_ENV);
                     ...
                     nxt_unit_request_info_t** preq)
{
  ...
  nxt_unit_debug(ctx,
                 "#%" PRIu32 ": process message %d fd[0] %d fd[1] %d",
                 port_msg->stream,
                 (int)port_msg->type,
                 recv_msg.fd[0],
                 recv_msg.fd[1]);
  ...
  if (nxt_slow_path(port_msg->nf != 0 || port_msg->mf != 0)) {
    nxt_unit_alert(ctx,
                   "#%" PRIu32 ": fragmented message type (%d)",
                   port_msg->stream,
                   (int)port_msg->type);
    rc = NXT_UNIT_ERROR;
    goto done;
  ...
  switch (port_msg->type) {
    ...
    case _NXT_PORT_MSG_CHANGE_FILE:
      nxt_unit_debug(ctx,
                     "#%" PRIu32 ": change_file: fd %d",
                     port_msg->stream,
                     recv_msg.fd[0]);

      if (dup2(recv_msg.fd[0], lib->log_fd) == -1) {
        nxt_unit_alert(ctx,
                       "#%" PRIu32 ": dup2(%d, %d) failed: %s (%d)",
                       port_msg->stream,
                       recv_msg.fd[0],
                       lib->log_fd,
                       strerror(errno),
    ...
    case _NXT_PORT_MSG_REMOVE_PID:
      ...
      nxt_unit_debug(
        ctx, "#%" PRIu32 ": remove_pid: %d", port_msg->stream, (int)pid);
...
nxt_unit_process_new_port(nxt_unit_ctx_t* ctx, nxt_unit_recv_msg_t* recv_msg)
{
  ...
  if (nxt_slow_path(recv_msg->fd[0] < 0)) {
    nxt_unit_alert(ctx,
                   "#%" PRIu32 ": invalid fd %d for new port",
                   recv_msg->stream,
                   recv_msg->fd[0]);
  ...
  nxt_unit_debug(ctx,
                 "#%" PRIu32 ": new_port: port{%d,%d} fd[0] %d fd[1] %d",
                 recv_msg->stream,
                 (int)new_port_msg->pid,
                 (int)new_port_msg->id,
                 recv_msg->fd[0],
                 recv_msg->fd[1]);
  ...
  if (nxt_slow_path(mem == MAP_FAILED)) {
    nxt_unit_alert(
      ctx, "mmap(%d) failed: %s (%d)", recv_msg->fd[1], strerror(errno), errno);
                             ...
                             nxt_unit_request_info_t** preq)
{
  ...
  if (nxt_slow_path(recv_msg->mmap == 0)) {
    nxt_unit_warn(
      ctx, "#%" PRIu32 ": data is not in shared memory", recv_msg->stream);
  ...
  if (nxt_slow_path(req_impl == NULL)) {
    nxt_unit_warn(
      ctx, "#%" PRIu32 ": request info allocation failed", recv_msg->stream);
                                     ...
                                     nxt_unit_port_id_t* port_id)
{
  ...
  if (nxt_fast_path(port != NULL)) {
    ...
    if (nxt_fast_path(port_impl->ready)) {
      ...
      nxt_unit_debug(ctx,
                     "check_response_port: found port{%d,%d}",
                     (int)port->id.pid,
                     (int)port->id.id);
  ...
  if (nxt_slow_path(port_impl == NULL)) {
    nxt_unit_alert(ctx,
                   "check_response_port: malloc(%d) failed",
  ...
  if (nxt_slow_path(res != NXT_UNIT_OK)) {
    nxt_unit_alert(ctx,
                   "check_response_port: %d,%d hash_add failed",
                   port->id.pid,
                   port->id.id);
  ...
  if (nxt_slow_path(process == NULL)) {
    nxt_unit_alert(
      ctx, "check_response_port: process %d not found", port->id.pid);
  ...
  if (cb->websocket_handler && recv_msg->size >= 2) {
    ...
    if (recv_msg->mmap) {
      ...
      if (nxt_slow_path(b == NULL)) {
        nxt_unit_alert(
          ctx, "#%" PRIu32 ": failed to allocate buf", req_impl->stream);
    ...
    switch (fields[i].hash) {
      case NXT_UNIT_HASH_CONTENT_LENGTH:
        if (fields[i].name_length == content_length.length &&
            nxt_unit_memcasecmp(
              name, content_length.start, content_length.length) == 0) {
      ...
      case NXT_UNIT_HASH_CONTENT_TYPE:
        if (fields[i].name_length == content_type.length &&
            nxt_unit_memcasecmp(
              name, content_type.start, content_type.length) == 0) {
                       ...
                       uint32_t max_fields_size)
{
  ...
  nxt_unit_req_debug(req,
                     "init: %d, max fields %d/%d",
                     (int)status,
                     (int)max_fields_count,
                     (int)max_fields_size);
                          ...
                          uint32_t max_fields_size)
{
  ...
  if (req->response->piggyback_content_length > 0) {
    ...
    p = nxt_cpymem(p,
                   nxt_unit_sptr_get(&req->response->piggyback_content),
                   req->response->piggyback_content_length);
  }
                            ...
                            uint32_t value_length)
{
  ...
  if (nxt_slow_path(resp->fields_count >= req->response_max_fields)) {
    nxt_unit_req_warn(
      req, "add_field: too many response fields (%d)", (int)resp->fields_count);
...
nxt_unit_response_buf_alloc(nxt_unit_request_info_t* req, uint32_t size)
{
  ...
  rc = nxt_unit_get_outgoing_buf(
    req->ctx, req->response_port, size, size, mmap_buf, NULL);
  if (nxt_slow_path(rc != NXT_UNIT_OK)) {
                       ...
                       int last)
{
  ...
  if (m.msg.mmap) {
    ...
    nxt_unit_debug(req->ctx,
                   "#%" PRIu32 ": send mmap: (%d,%d,%d)",
                   req_impl->stream,
    ...
    nxt_unit_debug(
      req->ctx, "allocated_chunks %d", (int)lib->outgoing.allocated_chunks);
    ...
    res = nxt_unit_port_send(req->ctx,
                             req->response_port,
...
int
nxt_unit_response_write(nxt_unit_request_info_t* req,
                        const void* start,
                        size_t size)
{
  ssize_t res;
...
ssize_t
nxt_unit_response_write_nb(nxt_unit_request_info_t* req,
                           const void* start,
                           size_t size,
                           size_t min_size)
{
  ...
  while (size > 0) {
    ...
    rc = nxt_unit_get_outgoing_buf(req->ctx,
                                   req->response_port,
                                   part_size,
                                   min_part_size,
                                   &mmap_buf,
                           ...
                           nxt_unit_read_info_t* read_info)
{
  ...
  while (!read_info->eof) {
    ...
    rc = nxt_unit_get_outgoing_buf(
      req->ctx, req->response_port, buf_size, buf_size, &mmap_buf, local_buf);
    if (nxt_slow_path(rc != NXT_UNIT_OK)) {
      return rc;
    }
...
nxt_unit_request_read(nxt_unit_request_info_t* req, void* dst, size_t size)
{
  ...
  if (buf_res < (ssize_t)size && req->content_fd != -1) {
    ...
    if (nxt_slow_path(res < 0)) {
      nxt_unit_req_alert(
        req, "failed to read content: %s (%d)", strerror(errno), errno);
...
skip_response_send:
  ...
  (void)nxt_unit_port_send(
    req->ctx, req->response_port, &msg, sizeof(msg), NULL);

  nxt_unit_request_info_release(req);
}

int
nxt_unit_websocket_send(nxt_unit_request_info_t* req,
                        uint8_t opcode,
                        uint8_t last,
                        const void* start,
                        size_t size)
...
int
nxt_unit_websocket_sendv(nxt_unit_request_info_t* req,
                         uint8_t opcode,
                         uint8_t last,
                         ...
                         int iovcnt)
{
  ...
  rc = nxt_unit_get_outgoing_buf(
    req->ctx, req->response_port, alloc_size, alloc_size, &mmap_buf, local_buf);
  if (nxt_slow_path(rc != NXT_UNIT_OK)) {
    return rc;
  }
...
unlock:
  ...
  nxt_unit_debug(
    ctx, "allocated_chunks %d", (int)lib->outgoing.allocated_chunks);
...
nxt_unit_new_mmap(nxt_unit_ctx_t* ctx, nxt_unit_port_t* port, int n)
{
  ...
  if (nxt_slow_path(rc != NXT_UNIT_OK)) {
    ...
    nxt_unit_debug(ctx,
                   "new mmap #%" PRIu32 " created for %d -> %d",
                   hdr->id,
                   (int)lib->pid,
                   (int)port->id.pid);
  }
...
nxt_unit_shm_open(nxt_unit_ctx_t* ctx, size_t size)
{
  ...
  if (nxt_slow_path(fd == -1)) {
    nxt_unit_alert(
      ctx, "memfd_create(%s) failed: %s (%d)", name, strerror(errno), errno);
  ...
  if (nxt_slow_path(fd == -1)) {
    nxt_unit_alert(
      ctx, "shm_open(SHM_ANON) failed: %s (%d)", strerror(errno), errno);
  ...
  if (nxt_slow_path(fd == -1)) {
    nxt_unit_alert(
      ctx, "shm_open(%s) failed: %s (%d)", name, strerror(errno), errno);
  ...
  if (nxt_slow_path(shm_unlink(name) == -1)) {
    nxt_unit_alert(
      ctx, "shm_unlink(%s) failed: %s (%d)", name, strerror(errno), errno);
  }
...
#error No working shared memory implementation.
  ...
  if (nxt_slow_path(ftruncate(fd, size) == -1)) {
    nxt_unit_alert(
      ctx, "ftruncate(%d) failed: %s (%d)", fd, strerror(errno), errno);
                          ...
                          char* local_buf)
{
  ...
  if (size <= NXT_UNIT_MAX_PLAIN_SIZE) {
    ...
    nxt_unit_debug(ctx,
                   "outgoing plain buffer allocation: (%p, %d)",
                   mmap_buf->buf.start,
                   (int)size);
  ...
  nxt_unit_debug(ctx,
                 "outgoing mmap allocation: (%d,%d,%d)",
                 (int)hdr->id,
                 (int)c,
...
nxt_unit_incoming_mmap(nxt_unit_ctx_t* ctx, pid_t pid, int fd)
{
  ...
  if (fstat(fd, &mmap_stat) == -1) {
    nxt_unit_alert(ctx,
                   "incoming_mmap: fstat(%d) failed: %s (%d)",
                   fd,
                   strerror(errno),
                   errno);
  ...
  if (nxt_slow_path(mem == MAP_FAILED)) {
    nxt_unit_alert(
      ctx, "incoming_mmap: mmap() failed: %s (%d)", strerror(errno), errno);
                   ...
                   nxt_unit_read_buf_t* rbuf)
{
  ...
  if (nxt_slow_path(recv_msg->size < sizeof(nxt_port_mmap_msg_t))) {
    nxt_unit_warn(ctx,
                  "#%" PRIu32 ": mmap_read: too small message (%d)",
                  recv_msg->stream,
                  (int)recv_msg->size);
  ...
  for (; mmap_msg < end; mmap_msg++) {
    res = nxt_unit_check_rbuf_mmap(
      ctx, mmaps, recv_msg->pid, mmap_msg->mmap_id, &hdr, rbuf);
...
retry:
  ...
  if (port_impl->from_socket == 0) {
    ...
    if (res == NXT_UNIT_OK) {
      if (nxt_unit_is_read_socket(rbuf)) {
        ...
        nxt_unit_debug(ctx,
                       "port{%d,%d} dequeue 1 read_socket %d",
  ...
  nxt_unit_alert(ctx,
                 "poll(%d,%d): %d unexpected revents [%04uXi, %04uXi]",
                 fds[0].fd,
                 fds[1].fd,
                 nevents,
                 fds[0].revents,
...
nxt_unit_process_ready_req(nxt_unit_ctx_t* ctx)
{
  ...
  nxt_queue_each(
    req_impl, &ready_req, nxt_unit_request_info_impl_t, port_wait_link)
  {
...
nxt_unit_ctx_alloc(nxt_unit_ctx_t* ctx, void* data)
{
  ...
  new_ctx =
    nxt_unit_malloc(ctx, sizeof(nxt_unit_ctx_impl_t) + lib->request_data_size);
...
nxt_unit_ctx_free(nxt_unit_ctx_impl_t* ctx_impl)
{
  ...
  nxt_queue_each(
    req_impl, &ctx_impl->active_req, nxt_unit_request_info_impl_t, link)
  {
  ...
  nxt_queue_each(
    req_impl, &ctx_impl->free_req, nxt_unit_request_info_impl_t, link)
  {
  ...
  nxt_queue_each(
    ws_impl, &ctx_impl->free_ws, nxt_unit_websocket_frame_impl_t, link)
  {
...
nxt_unit_create_port(nxt_unit_ctx_t* ctx)
{
  ...
  if (nxt_slow_path(rc != 0)) {
    nxt_unit_warn(
      ctx, "create_port: socketpair() failed: %s (%d)", strerror(errno), errno);
  ...
  if (nxt_slow_path(setsockopt(port_sockets[0],
                               SOL_SOCKET,
                               SO_PASSCRED,
                               &enable_creds,
  ...
  if (nxt_slow_path(setsockopt(port_sockets[1],
                               SOL_SOCKET,
                               SO_PASSCRED,
                               &enable_creds,
                               sizeof(enable_creds)) == -1)) {
  ...
  nxt_unit_debug(ctx,
                 "create_port: new socketpair: %d->%d",
                 port_sockets[0],
                 port_sockets[1]);
...
nxt_unit_port_release(nxt_unit_port_t* port)
{
  ...
  if (c == 1) {
    nxt_unit_debug(NULL,
                   "destroy port{%d,%d} in_fd %d out_fd %d",
                   (int)port->id.pid,
                   (int)port->id.id,
                   port->in_fd,
                   port->out_fd);
    ...
    if (port_impl->queue != NULL) {
      munmap(port_impl->queue,
             (port->id.id == NXT_UNIT_SHARED_PORT_ID)
               ? sizeof(nxt_app_queue_t)
               : sizeof(nxt_port_queue_t));
    }
...
nxt_unit_add_port(nxt_unit_ctx_t* ctx, nxt_unit_port_t* port, void* queue)
{
  ...
  nxt_unit_debug(ctx,
                 "add_port: port{%d,%d} in_fd %d out_fd %d queue %p",
                 port->id.pid,
                 port->id.id,
                 port->in_fd,
                 port->out_fd,
  ...
  if (nxt_slow_path(new_port == NULL)) {
    nxt_unit_alert(
      ctx, "add_port: %d,%d malloc() failed", port->id.pid, port->id.id);
  ...
  if (nxt_slow_path(rc != NXT_UNIT_OK)) {
    nxt_unit_alert(
      ctx, "add_port: %d,%d hash_add failed", port->id.pid, port->id.id);
...
nxt_unit_process_awaiting_req(nxt_unit_ctx_t* ctx, nxt_queue_t* awaiting_req)
{
  ...
  nxt_queue_each(
    req_impl, awaiting_req, nxt_unit_request_info_impl_t, port_wait_link)
  {
...
nxt_unit_remove_port_unsafe(nxt_unit_impl_t* lib, nxt_unit_port_id_t* port_id)
{
  ...
  if (nxt_slow_path(port == NULL)) {
    nxt_unit_debug(NULL,
                   "remove_port: port{%d,%d} not found",
                   (int)port_id->pid,
                   (int)port_id->id);
  ...
  nxt_unit_debug(NULL,
                 "remove_port: port{%d,%d}, fds %d,%d, data %p",
                 (int)port_id->pid,
                 (int)port_id->id,
                 port->in_fd,
                 port->out_fd,
...
nxt_unit_quit(nxt_unit_ctx_t* ctx, uint8_t quit_param)
{
  ...
  nxt_unit_debug(
    ctx, "quit: %d/%d/%d", (int)quit_param, ctx_impl->ready, ctx_impl->online);
                   ...
                   const nxt_send_oob_t* oob)
{
  ...
  if (port_impl->queue != NULL && (oob == NULL || oob->size == 0) &&
    ...
    if (nxt_slow_path(rc != NXT_OK)) {
      nxt_unit_alert(ctx,
                     "port_send: port %d,%d queue overflow",
                     (int)port->id.pid,
                     (int)port->id.id);
    ...
    if (notify) {
      ...
      if (lib->callbacks.port_send == NULL) {
        ret = nxt_unit_sendmsg(
          ctx, port->out_fd, &msg, sizeof(nxt_port_msg_t), NULL);

        nxt_unit_debug(ctx,
                       "port{%d,%d} send %d read_queue",
                       (int)port->id.pid,
                       (int)port->id.id,
                       (int)ret);
  ...
  if (port_impl->queue != NULL) {
    ...
    nxt_unit_debug(ctx,
                   "port{%d,%d} enqueue 1 read_socket notify %d",
                   (int)port->id.pid,
                   (int)port->id.id,
                   notify);
  }
  ...
  if (lib->callbacks.port_send != NULL) {
    ret = lib->callbacks.port_send(ctx,
                                   port,
                                   buf,
                                   buf_size,
...
retry:
  ...
  if (port_impl->from_socket > 0) {
    if (port_impl->socket_rbuf != NULL && port_impl->socket_rbuf->size > 0) {
      ...
      nxt_unit_debug(ctx,
                     "port{%d,%d} use suspended message %d",
                     (int)port->id.pid,
                     (int)port->id.id,
                     (int)rbuf->size);
...
retry:
  ...
  if (nxt_slow_path(rbuf->size == -1)) {
    ...
    if (err == EAGAIN) {
      nxt_unit_debug(
        ctx, "recvmsg(%d) failed: %s (%d)", fd, strerror(err), err);
    ...
    if (nxt_app_queue_cancel(queue, cookie, port_msg->stream)) {
      ...
      if (lib->request_limit != 0) {
        ...
        if (nxt_slow_path(lib->request_count >= lib->request_limit)) {
          ...
          (void)nxt_unit_port_send(
            ctx, lib->main_ctx.read_port, &m, sizeof(m), NULL);
        }
      }
...
nxt_unit_close(int fd)
{
  ...
  if (nxt_slow_path(res == -1)) {
    nxt_unit_alert(
      NULL, "close(%d) failed: %s (%d)", fd, strerror(errno), errno);
                       ...
                       nxt_unit_port_id_t* port_id)
{
  ...
  if (nxt_fast_path(port_id->hash != 0)) {
    ...
    nxt_unit_debug(NULL,
                   "calculate hash for port_id (%d, %d): %04X",
                   (int)port_id->pid,
                   (int)port_id->id,
                   (int)port_id->hash);
  }
...
nxt_unit_malloc(nxt_unit_ctx_t* ctx, size_t size)
{
  ...
  if (nxt_fast_path(p != NULL)) {
    ...
    nxt_unit_alert(
      ctx, "malloc(%d) failed: %s (%d)", (int)size, strerror(errno), errno);
  }
