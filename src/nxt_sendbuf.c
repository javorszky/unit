...
static nxt_buf_t*
nxt_sendbuf_coalesce_completion(nxt_task_t* task,
                                nxt_work_queue_t* wq,
                                nxt_buf_t* start);

nxt_uint_t
nxt_sendbuf_mem_coalesce0(nxt_task_t* task,
                          nxt_sendbuf_t* sb,
                          struct iovec* iov,
                          nxt_uint_t niov_max)
{
  ...
  for (b = sb->buf; b != NULL && total < sb->limit; b = b->next) {
    ...
    if (nxt_buf_is_mem(b)) {
      ...
      if (size != 0) {
        ...
        nxt_debug(
          task, "sendbuf: %ui, %p, %uz", n, iov[n].iov_base, iov[n].iov_len);
...
ssize_t
nxt_sendbuf_copy_coalesce(nxt_conn_t* c,
                          nxt_buf_mem_t* bm,
                          nxt_buf_t* b,
                          size_t limit)
{
