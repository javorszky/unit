...
nxt_script_t*
nxt_script_new(nxt_task_t* task,
               nxt_str_t* name,
               u_char* data,
               size_t size,
               u_char* error)
...
void
nxt_script_store_get(nxt_task_t* task,
                     nxt_str_t* name,
                     nxt_mp_t* mp,
                     ...
                     void* ctx)
{
  ...
  *b->mem.free++ = '\0';
  ...
  ret = nxt_port_socket_write(
    task, main_port, NXT_PORT_MSG_SCRIPT_GET, -1, stream, recv_port->id, b);
...
nxt_script_store_delete(nxt_task_t* task, nxt_str_t* name, nxt_mp_t* mp)
{
  ...
  if (nxt_fast_path(b != NULL)) {
    ...
    *b->mem.free++ = '\0';
    ...
    (void)nxt_port_socket_write(
      task, main_port, NXT_PORT_MSG_SCRIPT_DELETE, -1, 0, 0, b);
  }
}
...
nxt_script_store_delete_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  if (nxt_slow_path(nxt_recv_msg_cmsg_pid(msg) != ctl_port->pid)) {
    nxt_alert(
      task, "process %PI cannot delete scripts", nxt_recv_msg_cmsg_pid(msg));
    return;
  }
