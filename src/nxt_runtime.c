...
static void
nxt_runtime_thread_pool_destroy(nxt_task_t* task,
                                nxt_runtime_t* rt,
...
nxt_runtime_create(nxt_task_t* task)
{
  ...
  nxt_work_queue_add(
    &task->thread->engine->fast_work_queue, nxt_runtime_start, task, rt, NULL);
...
nxt_runtime_quit(nxt_task_t* task, nxt_uint_t status)
{
  ...
  if (done) {
    nxt_work_queue_add(
      &engine->fast_work_queue, nxt_runtime_exit, task, rt, engine);
  }
}
...
nxt_int_t
nxt_runtime_thread_pool_create(nxt_thread_t* thr,
                               nxt_runtime_t* rt,
                               ...
                               nxt_nsec_t timeout)
{
  ...
  thread_pool = nxt_thread_pool_create(max_threads,
                                       timeout,
...
nxt_runtime_conf_init(nxt_task_t* task, nxt_runtime_t* rt)
{
  ...
  ret = nxt_file_name_create(
    rt->mem_pool, &file_name, "%s%s*.unit.so%Z", rt->modules, slash);
  if (nxt_slow_path(ret != NXT_OK)) {
    return NXT_ERROR;
  }
  ...
  ret = nxt_file_name_create(
    rt->mem_pool, &file_name, "%s%sversion%Z", rt->state, slash);
  if (nxt_slow_path(ret != NXT_OK)) {
    return NXT_ERROR;
  }
  ...
  ret = nxt_file_name_create(
    rt->mem_pool, &file_name, "%s%sconf.json%Z", rt->state, slash);
  if (nxt_slow_path(ret != NXT_OK)) {
    return NXT_ERROR;
  }
  ...
  ret = nxt_file_name_create(
    rt->mem_pool, &file_name, "%s%scerts/%Z", rt->state, slash);
  if (nxt_slow_path(ret != NXT_OK)) {
    return NXT_ERROR;
  }
  ...
  ret = nxt_file_name_create(
    rt->mem_pool, &file_name, "%s%sscripts/%Z", rt->state, slash);
  if (nxt_slow_path(ret != NXT_OK)) {
    return NXT_ERROR;
  }
...
nxt_runtime_pid_file_create(nxt_task_t* task, nxt_file_name_t* pid_file)
{
  ...
  n = nxt_file_open(
    task, &file, O_WRONLY, O_CREAT | O_TRUNC, NXT_FILE_DEFAULT_ACCESS);
...
nxt_port_t*
nxt_runtime_process_port_create(nxt_task_t* task,
                                nxt_runtime_t* rt,
                                nxt_pid_t pid,
                                nxt_port_id_t id,
