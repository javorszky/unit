...
static nxt_int_t
nxt_select_create(nxt_event_engine_t* engine,
                  nxt_uint_t mchanges,
                  nxt_uint_t mevents);
static void
...
nxt_select_poll(nxt_event_engine_t* engine, nxt_msec_t timeout)
{
  ...
  nevents = select(nfds,
                   &engine->u.select.work_read_fd_set,
                   &engine->u.select.work_write_fd_set,
                   NULL,
                   tp);
  ...
  for (fd = 0; fd < (nxt_uint_t)nfds && nevents != 0; fd++) {
    ...
    if (FD_ISSET(fd, &engine->u.select.work_read_fd_set)) {
      ...
      nxt_debug(
        ev->task, "select() fd:%ui read rd:%d wr:%d", fd, ev->read, ev->write);
    ...
    if (FD_ISSET(fd, &engine->u.select.work_write_fd_set)) {
      ...
      nxt_debug(
        ev->task, "select() fd:%ui write rd:%d wr:%d", fd, ev->read, ev->write);
