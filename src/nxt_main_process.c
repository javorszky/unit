...
static nxt_int_t
nxt_main_file_store(nxt_task_t* task,
                    const char* tmp_name,
                    const char* name,
                    u_char* buf,
                    size_t size);
...
nxt_main_data_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  nxt_debug(task,
            "main data: %*s",
            nxt_buf_mem_used_size(&msg->buf->mem),
            msg->buf->mem.pos);
}
...
nxt_main_start_process_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  if (nxt_slow_path(port->pid != nxt_recv_msg_cmsg_pid(msg))) {
    nxt_alert(
      task, "process %PI cannot start processes", nxt_recv_msg_cmsg_pid(msg));
  ...
  *init = nxt_proto_process;
  ...
  nxt_debug(
    task, "main start prototype: %*s", b->mem.free - b->mem.pos, b->mem.pos);
  ...
  *p = '\0';
  ...
  ret = nxt_conf_map_object(process->mem_pool,
                            conf,
                            nxt_common_app_conf,
  ...
  ret = nxt_conf_map_object(process->mem_pool,
                            conf,
                            nxt_app_maps[idx].map,
                            nxt_app_maps[idx].size,
                            app_conf);
...
nxt_main_process_created_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  if (nxt_is_clone_flag_set(process->isolation.clone.flags, NEWUSER)) {
    if (nxt_slow_path(nxt_clone_credential_map(task,
                                               process->pid,
                                               process->user_cred,
  ...
  (void)nxt_port_socket_write(
    task, port, NXT_PORT_MSG_RPC_READY_LAST, -1, msg->port_msg.stream, 0, NULL);
}
...
nxt_main_process_whoami_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  (void)nxt_port_socket_write(
    task, port, NXT_PORT_MSG_RPC_READY_LAST, -1, msg->port_msg.stream, 0, buf);
...
nxt_main_process_title(nxt_task_t* task)
{
  ...
  p = nxt_sprintf(
    title, end, "unit: main v" NXT_VERSION " [%s", nxt_process_argv[0]);
...
nxt_main_process_sigusr1_handler(nxt_task_t* task, void* obj, void* data)
{
  ...
  if (nxt_fast_path(port != NULL)) {
    (void)nxt_port_socket_write(
      task, port, NXT_PORT_MSG_ACCESS_LOG, -1, 0, 0, NULL);
  }
  ...
  nxt_list_each(file, rt->log_files)
  {
    ...
    ret = nxt_file_open(
      task, new_file, O_WRONLY | O_APPEND, O_CREAT, NXT_FILE_OWNER_ACCESS);
...
nxt_main_process_sigchld_handler(nxt_task_t* task, void* obj, void* data)
{
  ...
  for (;;) {
    ...
    if (WTERMSIG(status)) {
#ifdef WCOREDUMP
      nxt_alert(task,
                "process %PI exited on signal %d%s",
                pid,
                WTERMSIG(status),
      ...
      nxt_trace(
        task, "process %PI exited with code %d", pid, WEXITSTATUS(status));
    }
    ...
    if (process != NULL) {
      ...
      if (!nxt_queue_is_empty(&process->children)) {
        ...
        nxt_queue_each(child, &children, nxt_process_t, link)
        {
          ...
          (void)nxt_port_socket_write(
            task, port, NXT_PORT_MSG_QUIT, -1, 0, 0, NULL);
        }
        nxt_queue_loop;
      }
...
{
  nxt_trace(
    task, "signal signo:%d (%s) received, ignored", (int)(uintptr_t)obj, data);
}
...
nxt_main_port_socket_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  if (nxt_slow_path(port->type != NXT_PROCESS_ROUTER)) {
    nxt_alert(
      task, "process %PI cannot create listener sockets", msg->port_msg.pid);
  ...
  nxt_port_socket_write(
    task, port, type, ls.socket, msg->port_msg.stream, 0, out);
}
...
nxt_main_listening_socket(nxt_sockaddr_t* sa, nxt_listening_socket_t* ls)
{
  ...
  if (sa->u.sockaddr.sa_family == AF_UNIX &&
    ...
    if (chmod(filename, 0666) != 0) {
      ls->end = nxt_sprintf(
        ls->start, ls->end, "chmod(\\\"%s\\\") failed %E", filename, nxt_errno);
      goto fail;
    }
...
nxt_main_port_modules_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  for (index = 0; /* void */; index++) {
    ...
    ret = nxt_conf_map_object(rt->mem_pool,
                              value,
                              nxt_app_lang_module_map,
                              nxt_nitems(nxt_app_lang_module_map),
                              lang);
    ...
    for (jindex = 0; /* */; jindex++) {
      ...
      ret = nxt_conf_map_object(rt->mem_pool,
                                ...
                                nxt_nitems(nxt_app_lang_mounts_map),
                                mnt);
  ...
  qsort(rt->languages->elts,
        rt->languages->nelts,
        sizeof(nxt_app_lang_module_t),
        nxt_app_lang_compare);
...
nxt_main_port_conf_store_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  if (nxt_buf_mem_used_size(&msg->buf->mem) != sizeof(size_t)) {
    nxt_alert(task,
              "conf_store_handler: unexpected buffer size (%d)",
                    ...
                    size_t size)
{
  ...
  ret = nxt_file_open(
    task, &file, NXT_FILE_WRONLY, NXT_FILE_TRUNCATE, NXT_FILE_OWNER_ACCESS);
  if (nxt_slow_path(ret != NXT_OK)) {
    return NXT_ERROR;
  }
...
nxt_main_port_access_log_handler(nxt_task_t* task, nxt_port_recv_msg_t* msg)
{
  ...
  ret = nxt_file_open(
    task, &file, O_WRONLY | O_APPEND, O_CREAT, NXT_FILE_OWNER_ACCESS);
  ...
  if (nxt_fast_path(port != NULL)) {
    (void)nxt_port_socket_write(
      task, port, type, file.fd, msg->port_msg.stream, 0, NULL);
  }
}
