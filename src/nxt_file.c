              ...
              nxt_file_access_t access)
{
  ...
  nxt_debug(task,
            "open(\"%FN\", 0x%uXi, 0x%uXi): %FD err:%d",
            file->name,
            mode,
            access,
            file->fd,
...
nxt_int_t
nxt_file_openat2(nxt_task_t* task,
                 nxt_file_t* file,
                 nxt_uint_t mode,
                 nxt_uint_t create,
                 nxt_file_access_t access,
...
nxt_file_close(nxt_task_t* task, nxt_file_t* file)
{
  ...
  if (close(file->fd) != 0) {
    nxt_alert(
      task, "close(%FD, \"%FN\") failed %E", file->fd, file->name, nxt_errno);
  }
}

ssize_t
nxt_file_write(nxt_file_t* file,
               const u_char* buf,
               size_t size,
               nxt_off_t offset)
{
  ...
  nxt_log_debug(
    thr->log, "pwrite(%FD, %p, %uz, %O): %z", file->fd, buf, size, offset, n);
...
nxt_file_read(nxt_file_t* file, u_char* buf, size_t size, nxt_off_t offset)
{
  ...
  nxt_log_debug(
    thr->log, "pread(%FD, %p, %uz, %O): %z", file->fd, buf, size, offset, n);
  ...
  if (file->fd == NXT_FILE_INVALID) {
    ...
    if (file->log_level != 0) {
      nxt_thread_log_error(
        file->log_level, "stat(\"%FN\") failed %E", file->name, file->error);
    }
    ...
    nxt_thread_log_alert(
      "fstat(%FD, \"%FN\") failed %E", file->fd, file->name, file->error);
...
nxt_file_chown(nxt_file_name_t* name, const char* owner, const char* group)
{
  ...
  nxt_thread_log_alert("chown(\"%FN\", %l, %l) failed %E",
                       name,
                       owner != NULL ? (long)uid : -1,
                       group != NULL ? (long)gid : -1,
                       nxt_errno);
...
nxt_file_rename(nxt_file_name_t* old_name, nxt_file_name_t* new_name)
{
  ...
  nxt_thread_log_alert(
    "rename(\"%FN\", \"%FN\") failed %E", old_name, new_name, nxt_errno);
...
nxt_file_fopen(nxt_task_t* task, const char* pathname, const char* mode)
{
  ...
  nxt_debug(
    task, "fopen(\"%s\", \"%s\"): fp:%p err:%d", pathname, mode, fp, err);
...
nxt_file_redirect(nxt_file_t* file, nxt_fd_t fd)
{
  ...
  if (dup2(fd, file->fd) == -1) {
    nxt_thread_log_alert(
      "dup2(%FD, %FD, \"%FN\") failed %E", fd, file->fd, file->name, nxt_errno);
    return NXT_ERROR;
  }

  if (close(fd) != 0) {
    nxt_thread_log_alert(
      "close(%FD, \"%FN\") failed %E", fd, file->name, nxt_errno);
    return NXT_ERROR;
  }
...
nxt_file_stdout(nxt_file_t* file)
{
  nxt_thread_log_debug(
    "dup2(%FD, %FD, \"%FN\")", file->fd, STDOUT_FILENO, file->name);
...
{
  nxt_thread_log_debug(
    "dup2(%FD, %FD, \"%FN\")", file->fd, STDERR_FILENO, file->name);
...
nxt_int_t
nxt_pipe_create(nxt_task_t* task,
                nxt_fd_t* pp,
                nxt_bool_t nbread,
                nxt_bool_t nbwrite)
{
