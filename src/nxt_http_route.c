...
nxt_http_route_create(nxt_task_t* task,
                      nxt_router_temp_conf_t* tmcf,
                      nxt_conf_value_t* cv);
static nxt_http_route_match_t*
nxt_http_route_match_create(nxt_task_t* task,
                            nxt_router_temp_conf_t* tmcf,
                            nxt_conf_value_t* cv);
static nxt_http_route_table_t*
nxt_http_route_table_create(nxt_task_t* task,
                            nxt_mp_t* mp,
...
nxt_http_route_ruleset_create(nxt_task_t* task,
                              nxt_mp_t* mp,
...
nxt_http_route_rule_name_create(nxt_task_t* task,
                                nxt_mp_t* mp,
                                ...
                                nxt_http_uri_encoding_t encoding);
static nxt_http_route_rule_t*
nxt_http_route_rule_create(nxt_task_t* task,
                           nxt_mp_t* mp,
                           nxt_conf_value_t* cv,
                           nxt_bool_t case_sensitive,
...
static nxt_int_t
nxt_http_route_pattern_create(nxt_task_t* task,
                              nxt_mp_t* mp,
                              nxt_conf_value_t* cv,
                              nxt_http_route_pattern_t* pattern,
                              nxt_http_route_pattern_case_t pattern_case,
...
static nxt_int_t
nxt_http_route_resolve(nxt_task_t* task,
                       nxt_router_temp_conf_t* tmcf,
                       nxt_http_route_t* route);
static nxt_int_t
nxt_http_action_resolve(nxt_task_t* task,
                        nxt_router_temp_conf_t* tmcf,
                        nxt_http_action_t* action);
static nxt_http_action_t*
nxt_http_pass_var(nxt_task_t* task,
                  nxt_http_request_t* r,
                  nxt_http_action_t* action);
static void
nxt_http_pass_query(nxt_task_t* task,
                    nxt_http_request_t* r,
                    nxt_http_action_t* action);
static nxt_int_t
nxt_http_pass_find(nxt_mp_t* mp,
                   nxt_router_conf_t* rtcf,
                   nxt_str_t* pass,
                   nxt_http_action_t* action);
static nxt_int_t
nxt_http_route_find(nxt_http_routes_t* routes,
                    nxt_str_t* name,
                    nxt_http_action_t* action);

static nxt_http_action_t*
nxt_http_route_handler(nxt_task_t* task,
                       nxt_http_request_t* r,
                       nxt_http_action_t* start);
static nxt_http_action_t*
nxt_http_route_match(nxt_task_t* task,
                     nxt_http_request_t* r,
                     nxt_http_route_match_t* match);
static nxt_int_t
...
nxt_http_route_pattern(nxt_http_request_t* r,
                       ...
                       size_t length);
static nxt_int_t
nxt_http_route_memcmp(u_char* start,
                      u_char* test,
                      size_t length,
                      nxt_bool_t case_sensitive);

nxt_http_routes_t*
nxt_http_routes_create(nxt_task_t* task,
                       nxt_router_temp_conf_t* tmcf,
                            ...
                            nxt_conf_value_t* cv)
{
  ...
  ret = nxt_conf_map_object(tmcf->mem_pool,
                            ...
                            nxt_nitems(nxt_http_route_match_conf),
                            &mtcf);
  if (ret != NXT_OK) {
    return NULL;
  }
  ...
  if (mtcf.scheme != NULL) {
    rule = nxt_http_route_rule_create(task,
                                      mp,
                                      mtcf.scheme,
                                      1,
  ...
  if (mtcf.headers != NULL) {
    table = nxt_http_route_table_create(task,
                                        mp,
                                        mtcf.headers,
                                        NXT_HTTP_ROUTE_HEADER,
                                        0,
...
nxt_int_t
nxt_http_action_init(nxt_task_t* task,
                     nxt_router_temp_conf_t* tmcf,
                     nxt_conf_value_t* cv,
                     nxt_http_action_t* action)
{
  ...
  ret = nxt_conf_map_object(tmcf->mem_pool,
                            cv,
                            nxt_http_route_action_conf,
                            nxt_nitems(nxt_http_route_action_conf),
                            &acf);
  if (ret != NXT_OK) {
    return ret;
  }
                            ...
                            nxt_http_uri_encoding_t encoding)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    ruleset = nxt_http_route_ruleset_create(
      task, mp, ruleset_cv, object, case_sensitive, encoding);
                              ...
                              nxt_http_uri_encoding_t encoding)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    rule = nxt_http_route_rule_name_create(
      task, mp, rule_cv, &name, case_sensitive, encoding);
                                ...
                                nxt_http_uri_encoding_t encoding)
{
  ...
  rule = nxt_http_route_rule_create(
    task, mp, rule_cv, case_sensitive, NXT_HTTP_ROUTE_PATTERN_NOCASE, encoding);
  if (nxt_slow_path(rule == NULL)) {
    return NULL;
  }
                           ...
                           nxt_http_uri_encoding_t encoding)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    ret = nxt_http_route_pattern_create(
      task, mp, value, &pattern[i], pattern_case, encoding);
    if (nxt_slow_path(ret != NXT_OK)) {
      return NULL;
    }
  }
...
nxt_http_route_addr_rule_create(nxt_task_t* task,
                                nxt_mp_t* mp,
                                nxt_conf_value_t* cv)
{
  size_t size;
  ...
  if (n > 1) {
    nxt_qsort(addr_rule->addr_pattern,
              addr_rule->items,
...
nxt_http_route_types_rule_create(nxt_task_t* task,
                                 nxt_mp_t* mp,
                                 nxt_conf_value_t* types)
{
  return nxt_http_route_rule_create(task,
                                    mp,
                                    types,
                                    0,
                                    NXT_HTTP_ROUTE_PATTERN_LOWCASE,
                              ...
                              nxt_http_uri_encoding_t encoding)
{
  ...
  if (test.length > 0 && test.start[0] == '~') {
    ...
    if (nxt_slow_path(re == NULL)) {
      if (err.offset < test.length) {
        nxt_alert(task,
                  "nxt_regex_compile(%V) failed: %s at offset %d",
                  &test,
                  err.msg,
                  (int)err.offset);
        return NXT_ERROR;
  ...
  if (type == NXT_HTTP_ROUTE_PATTERN_EXACT) {
    ...
    ret =
      nxt_http_route_pattern_slice(slices, &tmp, type, encoding, pattern_case);
    if (nxt_slow_path(ret != NXT_OK)) {
      return ret;
    }
                  ...
                  nxt_http_action_t* action)
{
  ...
  ret = nxt_tstr_query_init(
    &r->tstr_query, rtcf->tstr_state, &r->tstr_cache, r, r->mem_pool);
  if (nxt_slow_path(ret != NXT_OK)) {
    goto fail;
  }
                   ...
                   nxt_http_action_t* action)
{
  ...
  if (nxt_str_eq(&segments[0], "applications", 12)) {
    return nxt_router_application_init(
      rtcf, &segments[1], &segments[2], action);
  }
...
nxt_int_t
nxt_http_pass_segments(nxt_mp_t* mp,
                       nxt_str_t* pass,
                       nxt_str_t* segments,
                       nxt_uint_t n)
{
...
nxt_http_action_t*
nxt_http_action_create(nxt_task_t* task,
                       nxt_router_temp_conf_t* tmcf,
                       nxt_str_t* pass)
{
  nxt_mp_t* mp;
...
nxt_http_action_t*
nxt_http_pass_application(nxt_task_t* task,
                          nxt_router_conf_t* rtcf,
                          nxt_str_t* name)
{
  nxt_http_action_t* action;
    ...
    case AF_INET:
      ...
      switch (base->match_type) {
        ...
        case NXT_HTTP_ROUTE_ADDR_EXACT:
          match =
            (memcmp(
               &sin->sin_addr, &p->addr.v4.start, sizeof(struct in_addr)) == 0);
          break;
        ...
        case NXT_HTTP_ROUTE_ADDR_RANGE:
          match =
            (memcmp(&sin->sin_addr,
             ...
             memcmp(&sin->sin_addr, &p->addr.v4.end, sizeof(struct in_addr)) <=
               0);
          break;
...
nxt_int_t
nxt_http_route_test_rule(nxt_http_request_t* r,
                         nxt_http_route_rule_t* rule,
                         u_char* start,
                         size_t length)
{
    ...
    switch (pattern_slice->type) {
      case NXT_HTTP_ROUTE_PATTERN_EXACT:
        return ((length == pattern->min_length) &&
                nxt_http_route_memcmp(
                  start, test, test_length, pattern->case_sensitive));

      case NXT_HTTP_ROUTE_PATTERN_BEGIN:
        if (nxt_http_route_memcmp(
              start, test, test_length, pattern->case_sensitive)) {
          start += test_length;
          break;
        }
      ...
      case NXT_HTTP_ROUTE_PATTERN_END:
        ...
        if (nxt_http_route_memcmp(
              p, test, test_length, pattern->case_sensitive)) {
          end = p;
          break;
        }
