...
nxt_http_route_pattern_slice(nxt_array_t *slices, nxt_str_t *test,
    nxt_http_route_pattern_type_t type, nxt_http_uri_encoding_t encoding,
    nxt_http_route_pattern_case_t pattern_case);
...
nxt_http_route_match_create(nxt_task_t *task, nxt_router_temp_conf_t *tmcf,
    ...
    ret = nxt_conf_map_object(tmcf->mem_pool, match_conf,
        nxt_http_route_match_conf, nxt_nitems(nxt_http_route_match_conf),
        &mtcf);
    if (ret != NXT_OK) {
        return NULL;
    }
    ...
    if (mtcf.scheme != NULL) {
        rule = nxt_http_route_rule_create(task, mp, mtcf.scheme, 1,
            NXT_HTTP_ROUTE_PATTERN_NOCASE, NXT_HTTP_URI_ENCODING_NONE);
        if (rule == NULL) {
            return NULL;
        }
    ...
    if (mtcf.host != NULL) {
        rule = nxt_http_route_rule_create(task, mp, mtcf.host, 1,
            NXT_HTTP_ROUTE_PATTERN_LOWCASE, NXT_HTTP_URI_ENCODING_NONE);
        if (rule == NULL) {
            return NULL;
        }
    ...
    if (mtcf.uri != NULL) {
        rule = nxt_http_route_rule_create(task, mp, mtcf.uri, 1,
            NXT_HTTP_ROUTE_PATTERN_NOCASE, NXT_HTTP_URI_ENCODING);
        if (rule == NULL) {
            return NULL;
        }
    ...
    if (mtcf.method != NULL) {
        rule = nxt_http_route_rule_create(task, mp, mtcf.method, 1,
            NXT_HTTP_ROUTE_PATTERN_UPCASE, NXT_HTTP_URI_ENCODING_NONE);
        if (rule == NULL) {
            return NULL;
        }
    ...
    if (mtcf.headers != NULL) {
        table = nxt_http_route_table_create(task, mp, mtcf.headers,
            NXT_HTTP_ROUTE_HEADER, 0, NXT_HTTP_URI_ENCODING_NONE);
        if (table == NULL) {
            return NULL;
        }
    ...
    if (mtcf.arguments != NULL) {
        table = nxt_http_route_table_create(task, mp, mtcf.arguments,
            NXT_HTTP_ROUTE_ARGUMENT, 1, NXT_HTTP_URI_ENCODING_PLUS);
        if (table == NULL) {
            return NULL;
        }
    ...
    if (mtcf.cookies != NULL) {
        table = nxt_http_route_table_create(task, mp, mtcf.cookies,
            NXT_HTTP_ROUTE_COOKIE, 1, NXT_HTTP_URI_ENCODING_NONE);
        if (table == NULL) {
            return NULL;
        }
    ...
    if (mtcf.query != NULL) {
        rule = nxt_http_route_rule_create(task, mp, mtcf.query, 1,
            NXT_HTTP_ROUTE_PATTERN_NOCASE, NXT_HTTP_URI_ENCODING_PLUS);
        if (rule == NULL) {
            return NULL;
        }
...
nxt_http_route_pattern_create(nxt_task_t *task, nxt_mp_t *mp,
    ...
    if (test.length != 0 && end[-1] != '*') {
        ...
        ret = nxt_http_route_pattern_slice(slices, &tmp,
            NXT_HTTP_ROUTE_PATTERN_END, encoding, pattern_case);
        if (nxt_slow_path(ret != NXT_OK)) {
            return ret;
        }
    ...
    while (p != end) {
        ...
        ret = nxt_http_route_pattern_slice(slices, &tmp,
            NXT_HTTP_ROUTE_PATTERN_SUBSTRING, encoding, pattern_case);
        if (nxt_slow_path(ret != NXT_OK)) {
            return ret;
        }
